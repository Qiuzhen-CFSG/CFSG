...
/-- If two involutions have odd-order product, an element of the cyclic
subgroup generated by that product conjugates the first involution to the second. -/
public theorem exists_zpowers_conjugator_of_odd_product
    {G : Type*} [Group G] {s v : G}
    (hs : IsInvolution s) (hv : IsInvolution v)
    (hodd : Odd (orderOf (s * v))) :
    ∃ g : G, g ∈ Subgroup.zpowers (s * v) ∧
      rightConjugateElem s g = v := by
  rcases hodd with ⟨m, hm⟩
  let r : G := s * v
  let k : ℕ := m + 1
  have hsInv : s⁻¹ = s := hs.inv_eq_self
  have hvInv : v⁻¹ = v := hv.inv_eq_self
  have hss : s * s = 1 := by
    simpa [pow_two] using hs.sq_eq_one
  have hsem : SemiconjBy s r r⁻¹ := by
    change s * (s * v) = (s * v)⁻¹ * s
    rw [mul_inv_rev, hsInv, hvInv, ← mul_assoc, hss, one_mul, mul_assoc,
      hss, mul_one]
  have hsrk : s * r ^ k = (r ^ k)⁻¹ * s := by
    have h := hsem.pow_right k
    simpa [inv_pow] using h.eq
  have hpowOrder : r ^ (2 * m + 1) = 1 := by
    have h := pow_orderOf_eq_one r
    simpa [r, hm] using h
  have htwoK : 2 * k = (2 * m + 1) + 1 := by
    dsimp [k]
    omega
  have hpowTwoK : r ^ (2 * k) = r := by
    rw [htwoK, pow_succ, hpowOrder, one_mul]
  refine ⟨r ^ k, (Subgroup.zpowers r).pow_mem (Subgroup.mem_zpowers r) k, ?_⟩
  dsimp [rightConjugateElem]
  calc
    (r ^ k)⁻¹ * s * r ^ k = (s * r ^ k) * r ^ k := by rw [hsrk]
    _ = s * (r ^ k * r ^ k) := by rw [mul_assoc]
    _ = s * r ^ (2 * k) := by
      rw [← pow_add]
      congr 2
      omega
    _ = s * r := by rw [hpowTwoK]
    _ = v := by
      dsimp [r]
