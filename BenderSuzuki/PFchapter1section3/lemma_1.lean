...
private theorem twoPrimeResidual_eq_closure_rightConjugates_of_sylow
    {G : Type*} [Group G] [Finite G] (Q : Subgroup G)
    (hQ_sylow : ∃ P : Sylow 2 G, Q = (P : Subgroup G)) :
    twoPrimeResidual G =
      Subgroup.closure {x : G | ∃ g y : G, y ∈ Q ∧ x = rightConjugateElem y g} := by
  classical
  haveI : Fact (Nat.Prime 2) := ⟨Nat.prime_two⟩
  rcases hQ_sylow with ⟨P₀, rfl⟩
  apply le_antisymm
  · rw [twoPrimeResidual]
    refine iSup_le ?_
    intro P x hxP
    obtain ⟨g, hg⟩ := MulAction.exists_smul_eq G P₀ P
    have hx_smul : x ∈ ((g • P₀ : Sylow 2 G) : Subgroup G) := by
      simpa [hg] using hxP
    rw [Sylow.coe_subgroup_smul] at hx_smul
    have hyP₀ : (MulAut.conj g)⁻¹ x ∈ (P₀ : Subgroup G) :=
      (Subgroup.mem_pointwise_smul_iff_inv_smul_mem
        (a := MulAut.conj g) (S := (P₀ : Subgroup G)) (x := x)).mp hx_smul
    have hx_gen : x ∈ {x : G | ∃ g y : G,
        y ∈ (P₀ : Subgroup G) ∧ x = rightConjugateElem y g} := by
      refine ⟨g⁻¹, (MulAut.conj g)⁻¹ x, hyP₀, ?_⟩
      simp [rightConjugateElem, mul_assoc]
    exact Subgroup.subset_closure hx_gen
  · rw [Subgroup.closure_le]
    intro x hx
    rcases hx with ⟨g, y, hyP₀, rfl⟩
    rw [twoPrimeResidual]
    have hmem : rightConjugateElem y g ∈ ((g⁻¹ • P₀ : Sylow 2 G) : Subgroup G) := by
      rw [Sylow.coe_subgroup_smul]
      simpa [rightConjugateElem] using
        (Subgroup.smul_mem_pointwise_smul y (MulAut.conj g⁻¹)
          (P₀ : Subgroup G) hyP₀)
    exact (le_iSup (fun S : Sylow 2 G => (S : Subgroup G)) (g⁻¹ • P₀)) hmem

private theorem section3_q_eq_sylow_of_prime_power_order
    {G Ω : Type*} [Group G] [Finite G] [MulAction G Ω] [Finite Ω]
    (H D Q K V W Q0 S Q1 : Subgroup G) (t s : G)
    (hsec : ((_root_.BenderSuzuki.PFchapter1section1.HypothesisA G Ω H D Q t ∧
  K ≤ D ∧
