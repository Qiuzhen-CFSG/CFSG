...
/-- Hall Theorem 14.4.1 (P. Hall).  In Hall's setup with `G₀ = u_p(G)`,
`H = G₀ ∩ H₁`, and `P = G₀ ∩ P₁`, the residuals of `H₁` and `H` agree in the
ambient group.  If `u_p(H)` is proper, then Hall chooses finitely many
`z_i ∈ P₁` from the nontrivial double-coset permutation representations, and
`H` is generated from `H₀ = H^p (H,H₁)u_p(H)` by adjoining conjugates in `H`
of Engel elements `e_p(u,z_i)`. -/
public theorem hall_theorem_14_4_1_p_hall
    {G : Type u} [Group G] [Finite G] (p : ℕ) [Fact p.Prime]
    (P₁ : Sylow p G) (N₁ H₁ G₀ H P : Subgroup G)
  ...
  have hdata := hall_lemma_14_4_5_cycle_factor_engel_congruence
    (G := G) p P₁ N₁ H₁ G₀ H P H₀ hN₁ hN₁_le_H₁ hG₀ hH hP rfl
