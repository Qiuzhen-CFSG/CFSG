...
private theorem standard_hermitian_isotropic_fixing_mem_center
    ...
    (M : GL (Fin 3) K)
    (hfix : ∀ (v : Fin 3 → K) (_hv : v ≠ 0),
      dotProduct (fun i => J.conj (v i)) (J.form.mulVec v) = 0 →
        ∃ c : K, c • v =
          ((M : Matrix (Fin 3) (Fin 3) K).mulVec v)) :
    M ∈ Subgroup.center (GL (Fin 3) K) := by
  classical
  ...
  have he2iso :
    ...
    rw [hiso_formula]
    simp [e2]
  obtain ⟨c0, hc0⟩ := hfix e0 he0 he0iso
  ...
  have hKfintype_card : Fintype.card K = q ^ 2 := by
    rw [← Nat.card_eq_fintype_card, hKcard]
  have hKcard_three : 2 < Fintype.card K := by
    rw [hKfintype_card]
    nlinarith
  obtain ⟨x, y, z, hxy, hxz, hyz⟩ :=
    Fintype.two_lt_card_iff.mp hKcard_three
  let a : K := (z - x) / (y - x)
  have ha0 : a ≠ 0 := by
    exact div_ne_zero (sub_ne_zero.mpr hxz.symm)
      (sub_ne_zero.mpr hxy.symm)
  have ha1 : a ≠ 1 := by
    intro ha
    have heq : z - x = y - x :=
      (div_eq_one_iff_eq (sub_ne_zero.mpr hxy.symm)).mp ha
    exact hyz (sub_left_injective heq).symm
  have hnegNorm_fixed (r : K) :
      J.conj (-(r * J.conj r)) = -(r * J.conj r) := by
    rw [map_neg, map_mul, J.conj_involutive]
    ring
  obtain ⟨b1, hb1⟩ := huppert_II_10_4_trace_surjective
    J q hKcard hfixed_card (-(1 * J.conj 1)) (hnegNorm_fixed 1)
  simp only [map_one, one_mul] at hb1
  obtain ⟨ba, hba⟩ := huppert_II_10_4_trace_surjective
  ...
  have hdelta_trace : delta + J.conj delta = 0 := by
    ...
    abel
  let vaff (r b : K) : Fin 3 → K := ![1, r, b]
  have hvaff (r b : K) : vaff r b ≠ 0 := by
    intro h
    exact one_ne_zero (congrFun h 0)
  have hb1iso :
      dotProduct (fun i => J.conj (vaff 1 b1 i))
          (J.form.mulVec (vaff 1 b1)) = 0 := by
    rw [hiso_formula]
    change J.conj 1 * b1 + J.conj 1 * 1 + J.conj b1 * 1 = 0
    simp only [map_one, one_mul, mul_one]
    calc
      b1 + 1 + J.conj b1 = (b1 + J.conj b1) + 1 := by ring
      _ = -1 + 1 := by rw [hb1]
      _ = 0 := by ring
  have hb1delta_iso :
      dotProduct (fun i => J.conj (vaff 1 (b1 + delta) i))
          (J.form.mulVec (vaff 1 (b1 + delta))) = 0 := by
    rw [hiso_formula]
    change J.conj 1 * (b1 + delta) + J.conj 1 * 1 +
      J.conj (b1 + delta) * 1 = 0
    simp only [map_one, map_add, one_mul, mul_one]
    calc
      b1 + delta + 1 + (J.conj b1 + J.conj delta) =
          (b1 + J.conj b1) + 1 + (delta + J.conj delta) := by ring
      _ = -1 + 1 + 0 := by rw [hb1, hdelta_trace]
      _ = 0 := by ring
  have hbaiso :
      dotProduct (fun i => J.conj (vaff a ba i))
          (J.form.mulVec (vaff a ba)) = 0 := by
    rw [hiso_formula]
    change J.conj 1 * ba + J.conj a * a + J.conj ba * 1 = 0
    rw [map_one, one_mul, mul_one]
    calc
      ba + J.conj a * a + J.conj ba =
          (ba + J.conj ba) + a * J.conj a := by ring
      _ = -(a * J.conj a) + a * J.conj a := by rw [hba]
      _ = 0 := by ring
  obtain ⟨d1, hd1⟩ := hfix (vaff 1 b1) (hvaff 1 b1) hb1iso
  obtain ⟨d1', hd1'⟩ :=
    hfix (vaff 1 (b1 + delta)) (hvaff 1 (b1 + delta)) hb1delta_iso
  obtain ⟨da, hda⟩ := hfix (vaff a ba) (hvaff a ba) hbaiso
  have hmulVec_aff (r b : K) (i : Fin 3) :
      Mmat.mulVec (vaff r b) i =
        Mmat i 0 + Mmat i 1 * r + Mmat i 2 * b := by
    simp [vaff, Matrix.mulVec, dotProduct, Fin.sum_univ_three]
  have hd1_0 : d1 = Mmat 0 0 + Mmat 0 1 := by
    have h := congrFun hd1 0
    change d1 * 1 = Mmat.mulVec (vaff 1 b1) 0 at h
    rw [hmulVec_aff] at h
    simpa [h02] using h
  have hd1_1 : d1 = Mmat 1 1 := by
    have h := congrFun hd1 1
    change d1 * 1 = Mmat.mulVec (vaff 1 b1) 1 at h
    rw [hmulVec_aff] at h
    simpa [h10, h12] using h
  have hd1_2 : d1 * b1 = Mmat 2 1 + Mmat 2 2 * b1 := by
    have h := congrFun hd1 2
    change d1 * b1 = Mmat.mulVec (vaff 1 b1) 2 at h
    rw [hmulVec_aff] at h
    simpa [h20] using h
  have hd1'_1 : d1' = Mmat 1 1 := by
    have h := congrFun hd1' 1
    change d1' * 1 = Mmat.mulVec (vaff 1 (b1 + delta)) 1 at h
    rw [hmulVec_aff] at h
    simpa [h10, h12] using h
  have hd1'_2 : d1' * (b1 + delta) =
      Mmat 2 1 + Mmat 2 2 * (b1 + delta) := by
    have h := congrFun hd1' 2
    change d1' * (b1 + delta) =
      Mmat.mulVec (vaff 1 (b1 + delta)) 2 at h
    rw [hmulVec_aff] at h
    simpa [h20] using h
  have hda_0 : da = Mmat 0 0 + Mmat 0 1 * a := by
    have h := congrFun hda 0
    change da * 1 = Mmat.mulVec (vaff a ba) 0 at h
    rw [hmulVec_aff] at h
    simpa [h02] using h
  have hda_1 : da * a = Mmat 1 1 * a := by
    have h := congrFun hda 1
    change da * a = Mmat.mulVec (vaff a ba) 1 at h
    rw [hmulVec_aff] at h
    simpa [h10, h12] using h
  have hda_eq : da = Mmat 1 1 := by
    exact mul_right_cancel₀ ha0 hda_1
  have h01prod : Mmat 0 1 * (1 - a) = 0 := by
    rw [hd1_1] at hd1_0
    rw [hda_eq] at hda_0
    calc
      Mmat 0 1 * (1 - a) =
          (Mmat 0 0 + Mmat 0 1) -
            (Mmat 0 0 + Mmat 0 1 * a) := by ring
      _ = 0 := by rw [← hd1_0, ← hda_0, sub_self]
  have h01 : Mmat 0 1 = 0 := by
    exact (mul_eq_zero.mp h01prod).resolve_right
      (sub_ne_zero.mpr ha1.symm)
  have h11 : Mmat 1 1 = Mmat 0 0 := by
    rw [h01, add_zero] at hd1_0
    exact hd1_1.symm.trans hd1_0
  have h22prod : (Mmat 2 2 - Mmat 0 0) * delta = 0 := by
    rw [hd1_1, h11] at hd1_2
    rw [hd1'_1, h11] at hd1'_2
    have hdelta_mul : Mmat 0 0 * delta = Mmat 2 2 * delta := by
      linear_combination hd1'_2 - hd1_2
    calc
      (Mmat 2 2 - Mmat 0 0) * delta =
          Mmat 2 2 * delta - Mmat 0 0 * delta := by ring
      _ = 0 := sub_eq_zero.mpr hdelta_mul.symm
  have h22 : Mmat 2 2 = Mmat 0 0 := by
    exact sub_eq_zero.mp <|
      (mul_eq_zero.mp h22prod).resolve_right hdelta0
  have h21 : Mmat 2 1 = 0 := by
    rw [hd1_1, h11, h22] at hd1_2
    linear_combination -hd1_2
  apply Matrix.GeneralLinearGroup.mem_center_iff_val_mem_range_scalar.mpr
  refine ⟨Mmat 0 0, ?_⟩
  symm
  ext i j
  fin_cases i <;> fin_cases j <;>
    simp [Matrix.scalar_apply, Mmat, h10, h20, h02, h12, h01, h11, h21, h22]
...
public def hermitianUnipotentCenter
    {K : Type u} [Field K] {n : ℕ} (J : HermitianForm n K) :
    Subgroup (hermitianUnipotentCoord J) where
  carrier := {z | z.1.1 = 0}
  one_mem' := by
    change (hermitianUnipotentOne J).1.1 = 0
    rfl
  mul_mem' := by
    intro z w hz hw
    change z.1.1 = 0 at hz
    change w.1.1 = 0 at hw
    change (hermitianUnipotentMul J z w).1.1 = 0
    change z.1.1 + w.1.1 = 0
    rw [hz, hw, add_zero]
  inv_mem' := by
    intro z hz
    change z.1.1 = 0 at hz
    change (hermitianUnipotentInv J z).1.1 = 0
    change -z.1.1 = 0
    rw [hz, neg_zero]

/-- A Heisenberg coordinate with zero first component is central. -/
public theorem hermitianUnipotentCenter_le_center
    {K : Type u} [Field K] {n : ℕ} (J : HermitianForm n K) :
    hermitianUnipotentCenter J ≤ Subgroup.center (hermitianUnipotentCoord J) := by
  intro z hz
  rw [Subgroup.mem_center_iff]
  intro w
  change hermitianUnipotentMul J w z = hermitianUnipotentMul J z w
  apply Subtype.ext
  apply Prod.ext
  · simp only [hermitianUnipotentMul]
    exact add_comm _ _
