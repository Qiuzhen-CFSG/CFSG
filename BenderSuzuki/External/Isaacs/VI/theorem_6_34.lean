...
A representation-theoretic statement of Isaacs, *Character Theory of Finite
Groups*, Theorem 6.34, for ordinary complex irreducible representations. The
inertia assertion `Z_G(phi)=N` is expanded as a pointwise iff.
-/
...
theorem isaacs_6_34_inertia
  ...
  · intro hequiv
    by_contra hg
    let chi : Representation.ClassFunction N :=
      Representation.characterClassFunction phi
    ...
    have hchiIrr : Representation.IsIrreducibleCharacter chi :=
      Representation.isIrreducibleCharacter_characterClassFunction phi hphi
    have htauIrr :
        Representation.IsIrreducibleCharacter
          (Representation.characterClassFunction tau) :=
      Representation.isIrreducibleCharacter_characterClassFunction tau htau
    have hcharNe :
        chi ≠ Representation.characterClassFunction tau := by
      intro hchars
      apply hnonprincipal
      ...
      rcases Representation.equiv_of_irreducible_char_eq
          (ρ := tau) (σ := phi) hcard (by
            funext x
            have hx := congrFun (show
              Representation.characterClassFunction phi =
                Representation.characterClassFunction tau from hchars) (ConjClasses.mk x)
            exact hx.symm) with ⟨he⟩
      exact ⟨Representation.RepEquiv.mk he.toLinearEquiv he.isIntertwining'⟩
    have hchiFix :
        Representation.classFunctionConjLinearEquiv N g⁻¹ chi = chi := by
      dsimp [chi]
    ...
    have htauFix :
      ...
      rcases ConjClasses.exists_rep c with ⟨x, rfl⟩
      rfl
    rcases Representation.exists_nontrivial_fixed_conjClass_of_two_fixed_irreducible
        N g⁻¹ hchiIrr htauIrr hchiFix htauFix hcharNe with
      ⟨x, hxne, hxconj⟩
    rcases isConj_iff.mp hxconj with ⟨n, hn⟩
    have hnG :
        (n : G) * (g⁻¹ * (x : G) * g) * (n : G)⁻¹ = (x : G) := by
      simpa [Representation.normalSubgroupConjMulEquiv] using congrArg Subtype.val hn
    have ha :
