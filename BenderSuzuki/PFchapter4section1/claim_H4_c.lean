...
/-! # Peterfalvi, Part II, Chapter IV, Section 1, (H4)(c) -/

public theorem claim_H4_c
    ...
    ∀ x : L, x ∈ Q → x ≠ 1 → h (f x) = (h x)⁻¹ := by
  intro x hxQ hx1
  exact (canonical_compare_f M Q D t f g h htwo_transitive hpoint_stabilizer ht_involution ht_not_mem_M hD_eq hQ_normal_in_M hQ_disjoint_D hQ_sup_D hf_mem hg_mem hh_mem hcanonical_eq hxQ hx1).2.1
