...
/-- Appendix III, Lemma 1(b). A quadratic map is realized by a central
extension; the extension data are quantified directly rather than bundled. -/
public theorem lemma1b_exists_central_extension
    {V : Type v} {W : Type w}
    ...
    ∃ groupP : Group (V × W),
      letI := groupP
      ∃ iota : W → V × W, ∃ pi : V × W → V,
        Function.Injective iota ∧ Function.Surjective pi ∧
        iota 0 = 1 ∧ pi 1 = 0 ∧
        (∀ z₁ z₂, iota (z₁ + z₂) = iota z₁ * iota z₂) ∧
        (∀ x y, pi (x * y) = pi x + pi y) ∧
        (∀ x, pi x = 0 ↔ ∃ z, iota z = x) ∧
        (∀ z x, iota z * x = x * iota z) ∧
        ∀ x, iota (q (pi x)) = x ^ 2 := by
  classical
  let basis := Module.finBasis (ZMod 2) V
  let b : LinearMap.BilinMap (ZMod 2) V W := q.toBilin basis
  have hb_diag (v : V) : b v v = q v := by
    change b.toQuadraticMap v = q v
    rw [QuadraticMap.toQuadraticMap_toBilin q basis]
  let mulP : (V × W) → (V × W) → (V × W) := fun x y =>
    (x.1 + y.1, b x.1 y.1 + x.2 + y.2)
  let oneP : V × W := (0, 0)
  let invP : (V × W) → (V × W) := fun x =>
    (-x.1, -x.2 - b (-x.1) x.1)
  letI : Mul (V × W) := ⟨mulP⟩
  letI : One (V × W) := ⟨oneP⟩
  letI : Inv (V × W) := ⟨invP⟩
  have hmul_assoc (x y z : V × W) : (x * y) * z = x * (y * z) := by
    apply Prod.ext
    · change (x.1 + y.1) + z.1 = x.1 + (y.1 + z.1)
      exact add_assoc _ _ _
    · change
        b (x.1 + y.1) z.1 + (b x.1 y.1 + x.2 + y.2) + z.2 =
          b x.1 (y.1 + z.1) + x.2 + (b y.1 z.1 + y.2 + z.2)
      simp only [map_add, LinearMap.add_apply]
      abel
  have hone_mul (x : V × W) : 1 * x = x := by
    apply Prod.ext
    · change 0 + x.1 = x.1
      simp
    · change b 0 x.1 + 0 + x.2 = x.2
      simp
  have hinv_mul (x : V × W) : x⁻¹ * x = 1 := by
    apply Prod.ext
    · change -x.1 + x.1 = 0
      exact neg_add_cancel x.1
    · change b (-x.1) x.1 + (-x.2 - b (-x.1) x.1) + x.2 = 0
      abel
  let groupP : Group (V × W) :=
    Group.ofLeftAxioms hmul_assoc hone_mul hinv_mul
  letI : Group (V × W) := groupP
  let iota : W → V × W := fun z => (0, z)
  let pi : V × W → V := fun x => x.1
  have hiota_injective : Function.Injective iota := by
    intro z₁ z₂ h
    change (0, z₁) = (0, z₂) at h
    exact congrArg Prod.snd h
  have hpi_surjective : Function.Surjective pi := by
    intro v
    exact ⟨(v, 0), rfl⟩
  have hiota_zero : iota 0 = 1 := by
    rfl
  have hpi_one : pi 1 = 0 := by
    rfl
  have hiota_add (z₁ z₂ : W) : iota (z₁ + z₂) = iota z₁ * iota z₂ := by
    apply Prod.ext
    · change 0 = 0 + 0
      simp
    · change z₁ + z₂ = b 0 0 + z₁ + z₂
      simp
  have hpi_mul (x y : V × W) : pi (x * y) = pi x + pi y := by
    rfl
  have hexact (x : V × W) : pi x = 0 ↔ ∃ z, iota z = x := by
    constructor
    · intro hx
      change x.1 = 0 at hx
      refine ⟨x.2, ?_⟩
  ...
  have hcentral (z : W) (x : V × W) : iota z * x = x * iota z := by
    apply Prod.ext
    · change 0 + x.1 = x.1 + 0
      simp
    · change b 0 x.1 + z + x.2 = b x.1 0 + x.2 + z
      simp [add_comm]
  have hsquare (x : V × W) : iota (q (pi x)) = x ^ 2 := by
    have hV_two (v : V) : v + v = 0 := by
      exact ZModModule.add_self v
    have hW_two (w : W) : w + w = 0 := by
      exact ZModModule.add_self w
    rw [pow_two]
    apply Prod.ext
    · change 0 = x.1 + x.1
      exact (hV_two x.1).symm
    · change q x.1 = b x.1 x.1 + x.2 + x.2
      rw [hb_diag]
      calc
        q x.1 = q x.1 + 0 := (add_zero _).symm
        _ = q x.1 + (x.2 + x.2) := by rw [hW_two]
        _ = q x.1 + x.2 + x.2 := (add_assoc _ _ _).symm
  exact ⟨groupP, iota, pi, hiota_injective, hpi_surjective, hiota_zero, hpi_one,
    hiota_add, hpi_mul, hexact, hcentral, hsquare⟩
