...
private theorem actsRegularly_quotient_of_solvable_coprime_appendixIV
    ...
    (N : Subgroup M) [N.Normal] (hNinv : FTIsInvariant A M N)
  ...
  have hNinvA' : FTIsInvariant A' M N := by
    refine ⟨?_⟩
    intro b x
    have h := FTIsInvariant.invariant (A := A) (G := M) (H := N) (b : A) x
...
private theorem feitSibley_step1_regular_index_contradiction_core
  ...
  have hN2inv : FTIsInvariant d.D Z3 N2 := by
    refine ⟨?_⟩
    intro a z
    constructor
    · intro hz
  ...
  have hNZinv : FTIsInvariant d.D d.Q1 NZ := by
    refine ⟨?_⟩
    intro a q
    constructor
    · intro hq
...
private theorem feitSibley_step2_center_card_lower_bound_nilpotent_core
  ...
  letI : MulDistribMulAction d.D d.Q1 :=
    Subgroup.conjMulDistribMulActionOfLeNormalizer (G := d.H) d.D d.Q1
      (Subgroup.le_normalizer_of_normal (H := d.Q1))
  letI : FTIsInvariant d.D d.Q1 (Subgroup.center d.Q1) := by
    infer_instance
