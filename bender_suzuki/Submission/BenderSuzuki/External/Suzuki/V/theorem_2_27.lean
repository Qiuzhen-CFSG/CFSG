...
private theorem focalSubgroupOf_lt_top_of_controlled_fusion
  ...
  have hFmap :
    ...
    rw [Subgroup.focalSubgroup_def, Subgroup.map_le_iff_le_comap,
      Subgroup.closure_le]
    rintro g ⟨hg, x, hx, u, hgen⟩
    let a : H := x⁻¹
    let b : H := u * x⁻¹ * u⁻¹
    have ha : a ∈ (T : Subgroup H) := by
      exact (T : Subgroup H).inv_mem hx
    have hb : b ∈ (T : Subgroup H) := by
      have hmul : x⁻¹ * g ∈ (T : Subgroup H) :=
        (T : Subgroup H).mul_mem ((T : Subgroup H).inv_mem hx) hg
      simpa [a, b, commutatorElement_def, hgen, mul_assoc] using hmul
    have hab : (u : H) * a * (u : H)⁻¹ = b := by
      simp [a, b, mul_assoc]
    have hfaP : f a ∈ (P : Subgroup G) :=
      hTmap (Subgroup.mem_map_of_mem f ha)
    have hfbP : f b ∈ (P : Subgroup G) :=
      hTmap (Subgroup.mem_map_of_mem f hb)
    obtain ⟨z, hz⟩ := hfusion hfaP hfbP
      ⟨f (u : H), by simpa using congrArg f hab⟩
    have hgT : g ∈ (T : Subgroup H) := by
      exact hg
    have hfg : f g = ⁅(f a)⁻¹, (z : G)⁆ := by
      calc
        f g = (f a)⁻¹ * f b := by
          simp [a, b, commutatorElement_def, hgen, mul_assoc]
        _ = (f a)⁻¹ * ((z : G) * f a * (z : G)⁻¹) := by rw [hz]
        _ = ⁅(f a)⁻¹, (z : G)⁆ := by
          simp only [commutatorElement_def]
          group
    have hjg :
        j ⟨g, hgT⟩ =
          ⁅j ⟨a⁻¹, (T : Subgroup H).inv_mem ha⟩, z⁆ := by
      apply Subtype.ext
      have hright :
          ((⁅j ⟨a⁻¹, (T : Subgroup H).inv_mem ha⟩, z⁆ : P) : G) =
            ⁅(f a)⁻¹, (z : G)⁆ := by
        calc
          ((⁅j ⟨a⁻¹, (T : Subgroup H).inv_mem ha⟩, z⁆ : P) : G) =
              ⁅((j ⟨a⁻¹, (T : Subgroup H).inv_mem ha⟩ : P) : G), (z : G)⁆ :=
            map_commutatorElement (P : Subgroup G).subtype
              (j ⟨a⁻¹, (T : Subgroup H).inv_mem ha⟩) z
          _ = ⁅f (a⁻¹), (z : G)⁆ := rfl
          _ = ⁅(f a)⁻¹, (z : G)⁆ := by rw [map_inv]
      calc
        ((j ⟨g, hgT⟩ : P) : G) = f g := rfl
        _ = ⁅(f a)⁻¹, (z : G)⁆ := hfg
        _ = ((⁅j ⟨a⁻¹, (T : Subgroup H).inv_mem ha⟩, z⁆ : P) : G) := hright.symm
    have hC :
        j ⟨g, hgT⟩ ∈ C := by
      rw [hjg]
      exact Subgroup.commutator_mem_commutator
        (Subgroup.subset_normalClosure
          ⟨⟨a⁻¹, (T : Subgroup H).inv_mem ha⟩, rfl⟩)
        (Subgroup.mem_top z)
...
private theorem hasNormalPComplement_of_controlled_fusion_embedding
  ...
  let H0 : Subgroup H := T
  let V : H →* H0 ⧸ H0.focalSubgroupOf := H0.ftTransferFocal
  ...
  have hVsurj : Function.Surjective V := by
    intro y
    obtain ⟨x, hx⟩ := hkt_ftTransferFocal_restrict_surjective (S := T) y
    exact ⟨x, by simpa [V, H0, MonoidHom.restrict_apply] using hx⟩
  have hkerinf : K ⊓ H0 = H0.focalSubgroup := by
    simpa [K, V, H0] using
      (Subgroup.ker_ftTransferFocal_inf_eq_focalSubgroup (P := T))
