...
private theorem lemma13_omegaLength_three_of_chain
    {X P : Type u} [Group X] [Group P] [MulDistribMulAction X P]
    {H : Subgroup P} [FTIsInvariant X P H]
    (Phi C : Subgroup H)
...
private theorem lemma13_three_chain_data_of_ambient_chain
    {X P : Type u} [Group X] [Group P] [MulDistribMulAction X P]
    {H Phi C : Subgroup P}
    [FTIsInvariant X P H] [FTIsInvariant X P Phi] [FTIsInvariant X P C]
  ...
  have hlengthFourImpossible :
    ...
    have hPhi_le_A : PhiTop ≤ A := by
      ...
      have hD_X : IsXInvariantSubgroup X D := by
        letI : FTIsInvariant X Q A := ⟨hupper.2.2.2.2.1⟩
        letI : FTIsInvariant X Q PhiTop := ⟨hPhiX⟩
    ...
    have hX_odd : Odd (Fintype.card X) := by
      ...
      have he_pos : 0 < 2 ^ e := by positivity
      omega
    letI : FTIsInvariant X Q (frattini Q) := ⟨by
      have hPhiX' := hPhiX
    ...
    letI : FTIsInvariant X Q A := ⟨hupper.2.2.2.2.1⟩
    letI : FTIsInvariant X (Q ⧸ frattini Q) AQ := by
      dsimp [AQ]
      exact isInvariant_map_quotient A
    obtain ⟨WQ, hAQWQ, hWQ_forward⟩ :=
      exists_isCompl_invariant_subgroup_of_odd_group hX_odd AQ (by
        intro k q hq
        exact (FTIsInvariant.invariant (A := X) (G := Q ⧸ frattini Q)
          (H := AQ) k q).1 hq)
    letI : FTIsInvariant X (Q ⧸ frattini Q) WQ := by
      refine ⟨?_⟩
      intro k q
      constructor
    ...
    · have hPhi_eq_C : PhiTop = C := by
      ...
      let QuotientIrreducible (U : Subgroup (Q ⧸ frattini Q)) : Prop :=
        ∀ L : Subgroup (Q ⧸ frattini Q),
          FTIsInvariant X (Q ⧸ frattini Q) L → L ≤ U → L = ⊥ ∨ L = U
      have hExponentTwoQuotientSplit :
          ∃ UQ VQ WQ'': Subgroup (Q ⧸ frattini Q),
            FTIsInvariant X (Q ⧸ frattini Q) UQ ∧
            FTIsInvariant X (Q ⧸ frattini Q) VQ ∧
            FTIsInvariant X (Q ⧸ frattini Q) WQ'' ∧
            UQ ⊓ VQ = ⊥ ∧ UQ ⊔ VQ = AQ ∧
            AQ ⊓ WQ'' = ⊥ ∧ AQ ⊔ WQ'' = ⊤ ∧
        ...
        letI : FTIsInvariant X Q B := ⟨hmiddle.2.2.2.2.1⟩
        letI : FTIsInvariant X (Q ⧸ frattini Q) BQ := by
          dsimp [BQ]
        ...
        obtain ⟨VQ, hVQ_le_AQ, hVQ_forward,
          ...
          exists_isCompl_invariant_subgroup_within hX_odd AQ BQ hBQ_le_AQ
            ...
            (by
              ...
              exact (FTIsInvariant.invariant (A := X) (G := Q ⧸ frattini Q)
                (H := BQ) k q).1 hq)
        have hVQ_invariant : FTIsInvariant X (Q ⧸ frattini Q) VQ := ⟨by
          intro k q
          constructor
        ...
        have hBQ_irreducible : QuotientIrreducible BQ := by
          ...
          have hDnormal : D.Normal := by
            dsimp [D]
            infer_instance
          letI : FTIsInvariant X (Q ⧸ frattini Q) L := hLinv
        ...
        have hVQ_irreducible : QuotientIrreducible VQ := by
          ...
          have hDnormal : D.Normal := by
            ...
            infer_instance
          letI : FTIsInvariant X (Q ⧸ frattini Q) L := hLinv
          letI : FTIsInvariant X (Q ⧸ frattini Q) (BQ ⊔ L) :=
        ...
        have hWQ_irreducible : QuotientIrreducible WQ := by
          ...
          have hDnormal : D.Normal := by
            ...
            infer_instance
          letI : FTIsInvariant X (Q ⧸ frattini Q) L := hLinv
          letI : FTIsInvariant X (Q ⧸ frattini Q) (AQ ⊔ L) :=
      ...
      have hexponentTwoCase : False := by
        ...
        have hLayerCard :
            ∀ (U : Subgroup (Q ⧸ frattini Q)),
              FTIsInvariant X (Q ⧸ frattini Q) U →
              QuotientIrreducible U → U ≠ ⊥ →
              Nat.card U = 2 ^ nC := by
          intro U hUinv hUirr hU_ne
          letI : FTIsInvariant X (Q ⧸ frattini Q) U := hUinv
          ...
          have hT_irreducible :
            ...
            have hL_forward :
              ...
              let gu : U := ⟨g ^ j • (u : Q ⧸ frattini Q),
                (FTIsInvariant.invariant (A := X) (G := Q ⧸ frattini Q)
              ...
              exact ⟨gu, hgu, rfl⟩
            have hLinv : FTIsInvariant X (Q ⧸ frattini Q) L := by
              refine ⟨?_⟩
              intro k x
              constructor
    ...
    · have hPhiOrderFour : ∃ x : PhiTop, x ^ 2 ≠ 1 ∧ x ^ 4 = 1 := by
      ...
      have hC_le_center : C ≤ Subgroup.center Q := by
        ...
        have hD_normal : D.Normal := by
          ...
          exact hconj
        have hD_X : IsXInvariantSubgroup X D := by
          letI : FTIsInvariant X Q C := ⟨hbottom.2.2.2.1⟩
          letI : FTIsInvariant X Q (⊤ : Subgroup Q) := ⟨by
            intro x q
            simp⟩
      ...
      letI : FTIsInvariant X PhiTop (C.subgroupOf PhiTop) :=
      ...
      letI : FTIsInvariant X Q PhiTop := ⟨hPhiX⟩
      letI : FTIsInvariant X Q C := ⟨hbottom.2.2.2.1⟩
      letI : FTIsInvariant X Q Ylift := ⟨hYlift_X⟩
