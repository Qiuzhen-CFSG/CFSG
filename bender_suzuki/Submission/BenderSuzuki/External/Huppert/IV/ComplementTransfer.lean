...
public theorem hkt_ftTransferFocal_restrict_surjective
    {Q : Type u} [Group Q] [Finite Q] {q : ℕ} [Fact q.Prime]
    (S : Sylow q Q) :
    Function.Surjective
      (((S : Subgroup Q).ftTransferFocal).restrict (S : Subgroup Q)) := by
  classical
  let H : Subgroup Q := S
  intro y
  ...
  refine ⟨s, ?_⟩
  rw [MonoidHom.restrict_apply, Subgroup.ftTransferFocal_eq_pow]
  exact hz
