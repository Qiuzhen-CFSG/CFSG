...
public theorem hkt_maximal_invariant_quotient_exists_isElementaryAbelian
    ...
    (hN_ne_top : N ≠ ⊤)
    (hNmax :
      ∀ K : Subgroup Q, K.Normal → FTIsInvariant (Subgroup.zpowers φ) Q K →
        N ≤ K → K ≠ ⊤ → K = N)
  ...
  haveI : M.Normal := by
    dsimp [M]
    infer_instance
  have hMinv : FTIsInvariant (Subgroup.zpowers ψ) (Q ⧸ N) M := by
    refine ⟨?_⟩
    intro a x
    simp [M]
  have hM_ne_bot : M ≠ ⊥ := by
  ...
  have hMmin :
      ∀ K : Subgroup (Q ⧸ N), K.Normal →
        FTIsInvariant (Subgroup.zpowers ψ) (Q ⧸ N) K →
          K ≠ ⊥ → K ≤ M → K = M := by
...
public theorem hkt_maximal_lower_nilpotent_exists_isPGroup
    {Q : Type u} [Group Q] [Finite Q] (φ : MulAut Q)
    (N : Subgroup Q) [N.Normal]
    (hNinv : FTIsInvariant (Subgroup.zpowers φ) Q N)
  ...
  have hcardN_power : Nat.card N = r ^ (Nat.card N).primeFactorsList.length := by
    ...
    have hRamb_inv : FTIsInvariant (Subgroup.zpowers φ) Q Ramb := by
      haveI : FTIsInvariant (Subgroup.zpowers φ) Q N := hNinv
      have hR_inv_N : FTIsInvariant (Subgroup.zpowers φ) N (R : Subgroup N) := by
        haveI : (R : Subgroup N).Characteristic := hR_char
        simpa using isInvariant_of_characteristic (A := Subgroup.zpowers φ) (G := N)
          (R : Subgroup N)
      haveI : FTIsInvariant (Subgroup.zpowers φ) N (R : Subgroup N) := hR_inv_N
      simpa [Ramb] using
        isInvariant_map_subtype (A := Subgroup.zpowers φ) (G := Q) N (R : Subgroup N)
    have hSamb_inv : FTIsInvariant (Subgroup.zpowers φ) Q Samb := by
      haveI : FTIsInvariant (Subgroup.zpowers φ) Q N := hNinv
      have hS_inv_N : FTIsInvariant (Subgroup.zpowers φ) N (S : Subgroup N) := by
        haveI : (S : Subgroup N).Characteristic := hS_char
        simpa using isInvariant_of_characteristic (A := Subgroup.zpowers φ) (G := N)
          (S : Subgroup N)
      haveI : FTIsInvariant (Subgroup.zpowers φ) N (S : Subgroup N) := hS_inv_N
      simpa [Samb] using
...
public theorem huppertV813_lower_frattini_ambient_facts
    ...
    Φamb.Normal ∧
      FTIsInvariant (Subgroup.zpowers φ) Q Φamb ∧
      Φamb ≤ N ∧
      IsPGroup q Φamb ∧
  ...
  have hΦamb_invariant : FTIsInvariant (Subgroup.zpowers φ) Q Φamb := by
    haveI : FTIsInvariant (Subgroup.zpowers φ) Q N := hNinv
    have hΦ_invariant_N : FTIsInvariant (Subgroup.zpowers φ) N Φ := by
      haveI : Φ.Characteristic := hΦ_char
      simpa [Φ] using
        isInvariant_of_characteristic (A := Subgroup.zpowers φ) (G := N) Φ
    haveI : FTIsInvariant (Subgroup.zpowers φ) N Φ := hΦ_invariant_N
    simpa [Φamb] using
...
public theorem hkt_maximal_lower_frattini_ambient_eq_bot
    ...
    (hΦ_invariant :
      FTIsInvariant (Subgroup.zpowers φ) Q ((frattini N).map N.subtype))
...
public theorem hkt_centralizer_lower_eq_self_of_maximal_elementary_branch
  ...
  have hC_invariant : FTIsInvariant (Subgroup.zpowers φ) Q C := by
...
public theorem hkt_fixedPointSubgroup_invariant_of_commutative_operators
    ...
    [IsMulCommutative A] :
    FTIsInvariant A G (fixedPointSubgroup A G) := by
  refine ⟨?_⟩
  intro a x
  constructor
  · intro hx
...
public theorem hkt_quotient_fixedPointSubgroup_eq_top_of_ne_bot
  ...
  have hC_invariant_ψ :
      FTIsInvariant (Subgroup.zpowers ψ) (Q ⧸ N) C := by
  ...
  have hC_invariant_hkt :
      FTIsInvariant (Subgroup.zpowers (invariantQuotientAut φ N hNφ)) (Q ⧸ N) C := by
    subst hψ
...
public theorem hkt_quotient_isCyclic_of_fixedPointSubgroup_eq_top
  ...
  have hLφψ : ∀ y : Q ⧸ N, y ∈ L ↔ ψ y ∈ L := by
    ...
    simp [hy_fixed]
  have hL_invariant_ψ :
      FTIsInvariant (Subgroup.zpowers ψ) (Q ⧸ N) L :=
    hkt_zpowers_invariant_of_generator ψ L hLφψ
  have hL_invariant_hkt :
      FTIsInvariant (Subgroup.zpowers (invariantQuotientAut φ N hNφ)) (Q ⧸ N) L := by
    subst hψ
