...
public theorem hkt_zpowers_invariant_generator
    {Q : Type u} [Group Q] (φ : MulAut Q) (N : Subgroup Q)
    [FTIsInvariant (Subgroup.zpowers φ) Q N] :
    ∀ q : Q, q ∈ N ↔ φ q ∈ N := by
  intro q
  have h :=
    FTIsInvariant.invariant (A := Subgroup.zpowers φ) (G := Q) (H := N)
...
public theorem hkt_zpowers_invariant_of_generator
    {Q : Type u} [Group Q] (φ : MulAut Q) (N : Subgroup Q)
    (hN : ∀ q : Q, q ∈ N ↔ φ q ∈ N) :
    FTIsInvariant (Subgroup.zpowers φ) Q N := by
...
public theorem hkt_exists_minimal_invariant_normal
    ...
    (hnon_nil : ¬ Group.IsNilpotent Q) :
    ∃ M : Subgroup Q,
      M.Normal ∧ FTIsInvariant (Subgroup.zpowers φ) Q M ∧ M ≠ ⊥ ∧
...
public theorem hkt_exists_maximal_proper_invariant_normal
    ...
    (hnon_nil : ¬ Group.IsNilpotent Q) :
    ∃ N : Subgroup Q,
      N.Normal ∧ FTIsInvariant (Subgroup.zpowers φ) Q N ∧ N ≠ ⊤ ∧
        (∀ K : Subgroup Q, K.Normal → FTIsInvariant (Subgroup.zpowers φ) Q K →
          N ≤ K → K ≠ ⊤ → K = N) := by
  classical
  letI : Nontrivial Q := hkt_nontrivial_of_not_nilpotent hnon_nil
  ...
  let S : Set (Subgroup Q) :=
    {N | N.Normal ∧ FTIsInvariant A Q N ∧ N ≠ ⊤}
...
public theorem hkt_quotient_no_proper_nontrivial_invariant_normal
    ...
    ∀ L : Subgroup (Q ⧸ N), L.Normal →
      FTIsInvariant (Subgroup.zpowers (invariantQuotientAut φ N hNφ)) (Q ⧸ N) L →
      L ≠ ⊥ → L = ⊤ := by
  ...
  · let π : Q →* Q ⧸ N := QuotientGroup.mk' N
    ...
    have hLφbar : ∀ x : Q ⧸ N,
      ...
      haveI : FTIsInvariant (Subgroup.zpowers (invariantQuotientAut φ N hNφ))
          (Q ⧸ N) L := hLinv
    ...
    have hKinv : FTIsInvariant (Subgroup.zpowers φ) Q K :=
...
public theorem hkt_minimal_invariant_normal_ne_top_of_solvable_branch
  ...
  have hD_invariant : FTIsInvariant (Subgroup.zpowers φ) Q D :=
...
public theorem hkt_minimal_invariant_nilpotent_exists_isPGroup
    {Q : Type u} [Group Q] [Finite Q] (φ : MulAut Q)
    (M : Subgroup Q) [M.Normal]
    (hMinv : FTIsInvariant (Subgroup.zpowers φ) Q M)
    (hM_ne_bot : M ≠ ⊥)
    (hMmin :
      ∀ K : Subgroup Q, K.Normal → FTIsInvariant (Subgroup.zpowers φ) Q K →
        K ≠ ⊥ → K ≤ M → K = M)
  ...
  have hPamb_invariant : FTIsInvariant (Subgroup.zpowers φ) Q Pamb := by
    haveI : FTIsInvariant (Subgroup.zpowers φ) Q M := hMinv
    haveI : (P : Subgroup M).Characteristic := hP_char
    have hP_invariant_M : FTIsInvariant (Subgroup.zpowers φ) M (P : Subgroup M) := by
      simpa using
        isInvariant_of_characteristic (A := Subgroup.zpowers φ) (G := M) (P : Subgroup M)
    haveI : FTIsInvariant (Subgroup.zpowers φ) M (P : Subgroup M) := hP_invariant_M
    simpa [Pamb] using
...
public theorem hkt_minimal_invariant_nilpotent_frattini_eq_bot
  ...
  have hΦamb_invariant : FTIsInvariant (Subgroup.zpowers φ) Q Φamb := by
    haveI : FTIsInvariant (Subgroup.zpowers φ) Q M := hMinv
    have hΦ_invariant_M : FTIsInvariant (Subgroup.zpowers φ) M Φ := by
      haveI : Φ.Characteristic := hΦ_char
      simpa [Φ] using isInvariant_of_characteristic (A := Subgroup.zpowers φ) (G := M) Φ
    haveI : FTIsInvariant (Subgroup.zpowers φ) M Φ := hΦ_invariant_M
