...
import Submission.BenderSuzuki.SE.StrongEmbeddingIntersections
...
import Submission.FeitThompson.BGsection1.proposition_1_5
import Submission.FeitThompson.BGsection7.Defs
...
public theorem theorem4b_exists_invariant_sylow_containing
  ...
  have hPinv : FTIsInvariant (↥A) (↥D) (P.subgroupOf D) :=
  ...
  letI : FTIsInvariant (↥A) (↥D) H := hHinv
  have hA_normQ : A ≤ Subgroup.normalizer (Q : Set X) := by
    ...
    have hyInv : a • y ∈ H :=
      (FTIsInvariant.invariant (A := ↥A) (G := ↥D) (H := H) a y).1 hyH
    apply Subgroup.mem_map.mpr
    refine ⟨a • y, hyInv, ?_⟩
...
public theorem theorem4b_exists_centralizer_sylow_le_of_invariant_sylow
  ...
  have hA_normR : A ≤ Subgroup.normalizer (R : Set X) := by
    rw [Subgroup.zpowers_le]
    exact htNormR
  have hQinvSub : FTIsInvariant A D (Q.subgroupOf D) :=
    isInvariant_subgroupOf_of_le_normalizer
      hA_normD hA_normQ hQD
  have hRinvSub : FTIsInvariant A D (R.subgroupOf D) :=
    isInvariant_subgroupOf_of_le_normalizer
  ...
  have hQinv : FTIsInvariant A D (Qd : Subgroup D) := by
    simpa only [hQsubEq] using hQinvSub
  have hRinv : FTIsInvariant A D (Rd : Subgroup D) := by
