...
import Submission.Theory.Representation.TwoDimensionalOddOrder
...
public theorem quotient_scalar_coordinates_of_isomorphic_summands
  ...
  letI : FTIsInvariant K E U := ⟨hUinv⟩
  ...
  letI : FTIsInvariant K E V := ⟨hVinv⟩
...
private theorem lemma_5_W_cyclic_and_divides_obligation
  ...
  letI : FTIsInvariant K Q (Subgroup.center Q) := center_isInvariant
  let quotientActionK : MulDistribMulAction K E :=
    quotientMulDistribMulAction (A := K) (G := Q) (Subgroup.center Q)
      (inferInstance : FTIsInvariant K Q (Subgroup.center Q))
  ...
  letI : FTIsInvariant W Q (Subgroup.center Q) := center_isInvariant
  let quotientActionW : MulDistribMulAction W E :=
    quotientMulDistribMulAction (A := W) (G := Q) (Subgroup.center Q)
      (inferInstance : FTIsInvariant W Q (Subgroup.center Q))
  ...
  have hKfixedFree :
    ...
    letI : FTIsInvariant A Q (Subgroup.center Q) := center_isInvariant
    let quotientActionA : MulDistribMulAction A E :=
      quotientMulDistribMulAction (A := A) (G := Q) (Subgroup.center Q)
        (inferInstance : FTIsInvariant A Q (Subgroup.center Q))
    ...
    have hfixedEq :=
      fixedPointSubgroup_quotient_eq_map_of_solvable_coprime_action
        ...
        (Subgroup.center Q) (inferInstance : FTIsInvariant A Q (Subgroup.center Q))
  ...
  have hprimeMovesLine :
    ...
    have hPlaneInv : FTIsInvariant P Q Plane := by
      refine ⟨?_⟩
      intro c x
    ...
    letI : FTIsInvariant P Q Plane := hPlaneInv
