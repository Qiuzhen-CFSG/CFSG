...
import Submission.BenderSuzuki.PFchapter2.claim_11
...
private theorem chapter2_claim12_le_normalizer_map_of_isInvariant
    ...
    (K : Subgroup R) (hK : FTIsInvariant A R K)
    (hsmul_coe : ∀ (a : A) (x : R),
      (((a • x : R) : G)) = (a : G) * (x : G) * (a : G)⁻¹) :
  ...
  have hforward (a : A) (x : G)
    ...
    have hxInv : a • xR ∈ K :=
      (FTIsInvariant.invariant (A := A) (G := R) (H := K) a xR).1 hxK
...
private theorem chapter2_claim12_delta_mem_right_complement
    ...
    [IsMulCommutative U] (F J : Subgroup U) [FTIsInvariant A U J]
  ...
  have hcj : a • (j : U) ∈ J :=
    (FTIsInvariant.invariant (A := A) (G := U) (H := J) a (j : U)).1
      j.property
...
private theorem chapter2_claim12_le_normalizer_map_comap_quotient
    ...
    (q : R →* U) (J : Subgroup U) [FTIsInvariant A U J]
    (hcompat : ∀ (a : A) (x : R),
      a • q x = q (a • x))
    (hsmul_coe : ∀ (a : A) (x : R),
      (((a • x : R) : G)) = (a : G) * (x : G) * (a : G)⁻¹) :
  ...
  have hInv : FTIsInvariant A R (J.comap q) := by
    constructor
    intro a x
    change q x ∈ J ↔ q (a • x) ∈ J
    rw [← hcompat]
    exact FTIsInvariant.invariant (A := A) (G := U) (H := J) a (q x)
...
private theorem chapter2_claim12_local_index_after_sylow
  ...
  have hTR1invA : FTIsInvariant A R1 TR1 :=
  ...
  have hT1RinvA : FTIsInvariant A R1 T1R := by
    exact isInvariant_comap_quotient (A := A) (G := R1)
      (N := TR1) Jcomm hqU_compat
  letI : FTIsInvariant A R1 T1R := hT1RinvA
  have hJcomm_inverted {u : U} (hu : u ∈ Jcomm) : sA • u = u⁻¹ := by
    let v : U := u * (sA • u)
    have hvJ : v ∈ Jcomm := by
      exact Jcomm.mul_mem hu ((FTIsInvariant.invariant sA u).mp hu)
  ...
  have hTR1invH12 : FTIsInvariant H12 R1 TR1 :=
    isInvariant_subgroupOf_of_le_normalizer
  ...
  have hTR1invCQ : FTIsInvariant CQ R1 TR1 :=
    isInvariant_subgroupOf_of_le_normalizer
  ...
  have hJcomm_forward (c : CQ) {u : U} (hu : u ∈ Jcomm) :
    ...
    calc
      ...
      _ = (c • u)⁻¹ := by simp
  have hJcomm_invCQ : FTIsInvariant CQ U Jcomm := by
    refine ⟨?_⟩
    intro c u
    constructor
    ...
    · intro hcu
      ...
      simpa using hback
  letI : FTIsInvariant CQ U Jcomm := hJcomm_invCQ
  letI : FTIsInvariant CQ R1 TR1 := hTR1invCQ
