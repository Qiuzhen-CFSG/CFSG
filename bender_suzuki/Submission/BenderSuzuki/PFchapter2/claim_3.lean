...
import Submission.BenderSuzuki.PFchapter2.claim_3_appendixI
import Submission.Theory.Representation.Clifford
...
import Submission.Theory.Representation.SubrepresentationLattice
...
private theorem claim3_fixedPointSubgroup_exists_injective_to_quotient_QP
    ...
    {r : ℕ} (R : Sylow r Q1)
    (hRinv : FTIsInvariant L Q1 (R : Subgroup Q1))
    (M : Subgroup R) (hMinv : FTIsInvariant L R M)
    (N : Subgroup M) (hNinv : FTIsInvariant L M N)
    (core : Subgroup (Subgroup.centralizer (P : Set G))) [core.Normal]
    (hcore_le_DP : core ≤
      D.comap (Subgroup.centralizer (P : Set G)).subtype) :
    letI : FTIsInvariant L Q1 (R : Subgroup Q1) := hRinv
    letI : FTIsInvariant L R M := hMinv
    letI : FTIsInvariant L M N := hNinv
...
private theorem claim3_fixedPointSubgroup_card_eq_prime_of_propositionOneConclusion
    ...
    (haction : ∀ l : L, ∀ q : Q1,
      ((l • q : Q1) : G) = (l : G) * (q : G) * (l : G)⁻¹)
    {r : ℕ} [Fact r.Prime]
    (R : Sylow r Q1) (hRinv : FTIsInvariant L Q1 (R : Subgroup Q1))
    (M : Subgroup R) (hMinv : FTIsInvariant L R M)
    (N : Subgroup M) (hNinv : FTIsInvariant L M N)
...
private theorem claim3_minimal_invariant_subgroup_representation_irreducible
    ...
    (N : Subgroup M) (hNinv : FTIsInvariant L M N) (hNne : N ≠ ⊥)
    (hmin : ∀ T : Subgroup M, T.Normal → FTIsInvariant L M T →
      T ≠ ⊥ → T ≤ N → T = N) :
    letI : FTIsInvariant L M N := hNinv
  ...
  have hT0inv : FTIsInvariant L N T0 := by
...
private theorem claim3_exists_minimal_invariant_elementaryAbelian
    ...
    ∃ R : Sylow r Q1, ∃ hRinv : FTIsInvariant L Q1 (R : Subgroup Q1),
      ...
      ∃ M : Subgroup R, ∃ hMinv : FTIsInvariant L R M,
        letI : FTIsInvariant L R M := hMinv
        ∃ N : Subgroup M, ∃ _hNinv : FTIsInvariant L M N,
          IsElementaryAbelian r M ∧ IsElementaryAbelian r N ∧ N ≠ ⊥ ∧
            ∀ T : Subgroup M, T.Normal → FTIsInvariant L M T →
              T ≠ ⊥ → T ≤ N → T = N := by
  letI : MulDistribMulAction L Q1 :=
    Subgroup.conjMulDistribMulActionOfLeNormalizer L Q1 hLnorm
  ...
  have hRinv : FTIsInvariant L Q1 (R : Subgroup Q1) :=
