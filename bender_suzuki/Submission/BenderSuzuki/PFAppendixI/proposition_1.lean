...
private theorem peterfalvi_appendixI_proposition_1_fixedPointSubgroup_sup_eq_bot_of_disjoint
    ...
    (U W : Subgroup M) [FTIsInvariant A M U] [FTIsInvariant A M W]
    [FTIsInvariant A M (U ⊔ W)]
    (hdisj : Disjoint U W)
  ...
  have hparts (a : A) : a • u = u ∧ a • w = w := by
    have hau := (FTIsInvariant.invariant (A := A) (G := M) (H := U) a u).1 hu
    have haw := (FTIsInvariant.invariant (A := A) (G := M) (H := W) a w).1 hw
...
private theorem peterfalvi_appendixI_proposition_1_fixedPointSubgroup_family_iSupIndep_of_pairwise_disjoint
  ...
  have hF_inv (i j : ι) : FTIsInvariant (T i) M (F j) := by
  ...
  have hnormalizer (X Y : Subgroup M) : Y ≤ Subgroup.normalizer (X : Set M) := by
    ...
    · intro hx
      ...
      rw [hyx']
      exact hx
  have hsup_inv : ∀ (s : Finset ι) (i : ι), FTIsInvariant (T i) M (s.sup F) := by
    intro s
    ...
    | @insert j s hj ih =>
        intro i
        letI : FTIsInvariant (T i) M (F j) := hF_inv i j
        letI : FTIsInvariant (T i) M (s.sup F) := ih i
        have hinv : FTIsInvariant (T i) M (F j ⊔ s.sup F) :=
  ...
  have hind : ∀ s : Finset ι,
    ...
    | @insert i s hi ih =>
        ...
        letI : FTIsInvariant (T j) M (F i) := hF_inv j i
        letI : FTIsInvariant (T j) M (s.sup F) := hsup_inv s j
        letI : FTIsInvariant (T j) M (F i ⊔ s.sup F) :=
...
private theorem peterfalvi_appendixI_proposition_1_stabilizer_mul_eq_inf
    ...
    (U V : Subgroup E) [FTIsInvariant P E U] [FTIsInvariant P E V]
  ...
  · intro hg
    have hgaU : g • a ∈ U :=
      (FTIsInvariant.invariant (A := P) (G := E) (H := U) g a).mp haU
    have hgbV : g • b ∈ V :=
      (FTIsInvariant.invariant (A := P) (G := E) (H := V) g b).mp hbV
    have hprod : (g • a) * (g • b) = a * b := by simpa using hg
