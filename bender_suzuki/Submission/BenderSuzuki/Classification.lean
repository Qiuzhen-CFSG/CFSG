...
private theorem isInvolution_map_iff {G : Type u} {H : Type v} [Group G] [Group H]
    (e : G ≃* H) (x : G) : _root_.BSIsInvolution (e x) ↔ _root_.BSIsInvolution x := by
  constructor
  · rintro ⟨h1, h2⟩
...
private theorem isStronglyEmbedded_map {G : Type u} {H : Type v} [Group G] [Group H]
    (e : G ≃* H) {M : Subgroup G} (hM : _root_.BSIsStronglyEmbedded M) :
    _root_.BSIsStronglyEmbedded (M.map (e : G →* H)) := by
...
private theorem exists_stronglyEmbedded_psl2Model (n : ℕ) (hn : 2 ≤ n) :
    ∃ M : Subgroup (BSPSL2Model n), _root_.BSIsStronglyEmbedded M :=
...
private theorem exists_stronglyEmbedded_szModel (n : ℕ) (hn : 1 ≤ n) :
    ∃ M : Subgroup (BSSzModel n), _root_.BSIsStronglyEmbedded M := by
  haveI : NeZero n := ⟨by omega⟩
  have e : (BSSzModel n) ≃* (SuzukiMatrixGroup n) :=
...
private theorem exists_stronglyEmbedded_psu3Model (n : ℕ) (hn : 2 ≤ n) :
    ∃ M : Subgroup (BSPSU3Model n), _root_.BSIsStronglyEmbedded M := by
  haveI : NeZero n := ⟨by omega⟩
...
public theorem exists_stronglyEmbedded_of_isSimpleBenderGroup {X : Type u} [Group X]
    [Finite X] (h : _root_.BSIsSimpleBenderGroup X) :
    ∃ M : Subgroup X, _root_.BSIsStronglyEmbedded M := by
...
public theorem isSimpleBenderGroup_iff_exists_stronglyEmbedded {X : Type u} [Group X]
    ...
    (∃ M : Subgroup X, _root_.BSIsStronglyEmbedded M) ↔ _root_.BSIsSimpleBenderGroup X := by
  constructor
  · rintro ⟨M, hM⟩
    exact bender_suzuki_internal M hM
