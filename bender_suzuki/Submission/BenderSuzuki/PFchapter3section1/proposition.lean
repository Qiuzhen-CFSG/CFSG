...
private theorem cubic_line_root_card
  ...
  have hPlaneInv : FTIsInvariant K S Plane := by
    refine ⟨?_⟩
    intro k x
    change qmap x ∈ T ↔ qmap (k • x) ∈ T
    rw [← hquotientAction k x]
    exact hTinv k (qmap x)
  letI : FTIsInvariant K S Plane := hPlaneInv
...
private theorem cubic_linear_contradiction
  ...
  have hCcommNe : Ccomm ≠ ⊥ := by
    ...
    have hPbot : P = ⊥ := by
      ...
      · exact bot_le
    exact hPne hPbot
  have hCcommInv : FTIsInvariant P K Ccomm := by
    simpa [Ccomm] using
      (commutatorAction_isInvariant (G := K) (A := P))
  letI : FTIsInvariant P K Ccomm := hCcommInv
  ...
  have hKfixedFree :
    ...
    have hfixedEq :=
      fixedPointSubgroup_quotient_eq_map_of_solvable_coprime_action
        ...
        Z (inferInstance : FTIsInvariant A S Z)
  ...
  have hfixedEq :=
    fixedPointSubgroup_quotient_eq_map_of_solvable_coprime_action
      ...
      Z (inferInstance : FTIsInvariant P S Z)
...
private theorem cubic_fixed_root
  ...
  have hPlaneInv : FTIsInvariant P S Plane := by
    refine ⟨?_⟩
    intro c x
    ...
    exact hmem
  letI : FTIsInvariant P S Plane := hPlaneInv
    ...
    · rcases hiso with ⟨eUV, heUV⟩
      ...
      have hYdata : ∃ Y : Subgroup E,
        ...
        have hDActionP : ∀ c : PD, ∀ x : E,
          ...
          apply Subtype.ext
          rfl
        have hAinvX : FTIsInvariant A E X := by
          ...
          · intro hax
            ...
            simpa [smul_smul] using hback
        letI : FTIsInvariant A E X := hAinvX
        ...
        have hXinvariant : eta X ∈ ρ.invtSubmodule := by
          ...
          simpa [ρ, eta] using
            (FTIsInvariant.invariant (A := A) (G := E) (H := X)
        ...
        have hYinvA : FTIsInvariant A E Y := by
          refine ⟨?_⟩
          intro a y
          constructor
          · intro hy
          ...
          Subgroup.isComplement'_of_card_mul_and_disjoint
            ...
            (by
              ...
              calc
                ...
                _ = ⊥ := hTVinf)
        have hTInvariant : FTIsInvariant K E T := ⟨hTinv⟩
        letI : FTIsInvariant K E T := hTInvariant
  ...
  have htypeACase :
    ...
    have htypeA_fixed_of_prime_in_W :
      ...
      have hPfixCenter : ∀ c : P, ∀ z : Subgroup.center S,
        ...
        rw [hcComm]
        simp
      letI : FTIsInvariant P S (Subgroup.center S) := center_isInvariant
      letI : MulDistribMulAction P (S ⧸ Subgroup.center S) :=
        quotientMulDistribMulAction (A := P) (G := S)
          (Subgroup.center S) (inferInstance : FTIsInvariant P S (Subgroup.center S))
      ...
      have hfixedQuotientMap :=
        fixedPointSubgroup_quotient_eq_map_of_solvable_coprime_action
          ...
          (Subgroup.center S)
          (inferInstance : FTIsInvariant P S (Subgroup.center S))
