...
private theorem section13_le_normalizer_map_of_isInvariant
    ...
    [hCinv : letI : Subgroup.Normalizes A Q := ⟨hAQ⟩; FTIsInvariant A Q C] :
  ...
  · intro hx
    ...
    · exact (FTIsInvariant.invariant (A := A) (G := Q) (H := C)
        (⟨a, haA⟩ : A) q).1 hqC
...
private theorem section13_theorem_13_16_MaschkeFrobeniusSourceData_of_inputs
  ...
  haveI : FTIsInvariant KW2 Q C := hCinv
  refine ⟨Q1, hQprod, ?_⟩
  intro hKne
