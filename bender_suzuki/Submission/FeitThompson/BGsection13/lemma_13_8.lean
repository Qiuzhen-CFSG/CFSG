...
private theorem section13_lemma_13_8_exists_hall_in_centralizer_containing_piSubgroup
  ...
  have hKinv : FTIsInvariant PUnit.{1} C (K.subgroupOf C) := by
    refine ⟨?_⟩
    intro a x
    cases a
    simp
...
private theorem section13_fixedPointSubgroup_quotient_eq_map_of_solvable_kernel_coprime
    ...
    (K : Subgroup L) [K.Normal] (hKinv : FTIsInvariant A L K)
    (hKsolv : IsSolvable K)
  ...
  classical
  letI : FTIsInvariant A L K := hKinv
...
private theorem section13_lemma_13_8_fixed_point_prime_order_lift_to_normalizer_Q_divisor
    ...
    have hKUinv : FTIsInvariant (↥P) (↥U) KUsub := by
      refine ⟨?_⟩
      intro a x
      change ((x : U) : G) ∈ K ↔
        ((a : G) * ((x : U) : G) * (a : G)⁻¹) ∈ K
    ...
    letI : FTIsInvariant (↥P) (↥U) KUsub := hKUinv
    ...
    have hKsubS_inv : FTIsInvariant (↥P) (↥S) KsubS := by
      simpa [KsubS] using
        section13_isInvariant_subgroupOf_of_le_normalizer
          (G := G) (A := P) (H := S) (K := K)
          hP_norm_S hP_norm_K le_sup_right
    letI : FTIsInvariant (↥P) (↥S) KsubS := hKsubS_inv
