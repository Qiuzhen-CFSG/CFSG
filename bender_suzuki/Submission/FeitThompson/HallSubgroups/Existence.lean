...
public theorem exists_isHallSubgroup_isInvariant_of_isPiSubgroup
    ...
    (π : Set Nat.Primes) :
    ∀ K : Subgroup G, IsPiSubgroup (G := G) π K → FTIsInvariant A G K →
      ∃ H : Subgroup G, IsHallSubgroup π H ∧ FTIsInvariant A G H ∧ K ≤ H := by
  classical
    ...
    ∀ (G' : Type u) [Group G'] [Finite G'] [MulDistribMulAction A G'],
      Nat.card G' = n → IsSolvable G' → Nat.Coprime (Nat.card A) (Nat.card G') →
        ∀ (π' : Set Nat.Primes) (K' : Subgroup G'),
          IsPiSubgroup π' K' → FTIsInvariant A G' K' →
            ∃ H' : Subgroup G', IsHallSubgroup π' H' ∧ FTIsInvariant A G' H' ∧ K' ≤ H'
  have main : ∀ n, P n := by
    intro n
    ...
    haveI : FTIsInvariant A G' N := isInvariant_of_characteristic (A := A) (G := G') N
    ...
    haveI : FTIsInvariant A N (P₀ : Subgroup N) :=
      isInvariant_of_characteristic (A := A) (G := N) (P₀ : Subgroup N)
    haveI : FTIsInvariant A G' Psub := isInvariant_map_subtype (A := A) (G := G') N (P₀ : Subgroup N)
    ...
    letI : MulDistribMulAction A Q :=
      quotientMulDistribMulAction (A := A) (G := G') Psub (inferInstance : FTIsInvariant A G' Psub)
    ...
    have hK'_image_pi : IsPiSubgroup π' K'_image := by
      ...
      exact hp.trans this
    have hK'_image_inv : FTIsInvariant A Q K'_image := by
      refine ⟨?_⟩
      intro a q
      constructor
      · rintro ⟨g, hg, rfl⟩
        refine ⟨a • g, (FTIsInvariant.invariant (A := A) (G := G') (H := K') a g).1 hg, ?_⟩
      ...
      · rintro ⟨g, hg, hq⟩
        ...
        have : f (a⁻¹ • g) = a⁻¹ • f g := by
          exact MulAction.Quotient.smul_mk (H := Psub) (a⁻¹) g
        simp [this, hq]
    haveI : FTIsInvariant A Q K'_image := hK'_image_inv
    ...
    ·
      ...
      haveI : FTIsInvariant A K0 Pk := by
        refine ⟨?_⟩
        intro a x
        change (x.1 ∈ Psub) ↔ (a • x.1 ∈ Psub)
        exact FTIsInvariant.invariant (A := A) (G := G') (H := Psub) a x.1
      ...
      have hKk_coprime_Pk : Nat.Coprime (Nat.card Kk) (Nat.card Pk) := by
        ...
        rw [this] at hq_mem
        exact hq_mem
      haveI : FTIsInvariant A K0 Kk := by
        refine ⟨?_⟩
        intro a x
        change (x.1 ∈ K') ↔ (a • x.1 ∈ K')
        exact FTIsInvariant.invariant (A := A) (G := G') (H := K') a x.1
      ...
      have hB_inv_Pk : FTIsInvariant B K0 Pk := by
        refine ⟨?_⟩
        intro b x
        have h_inr (a : A) (y : K0) :
          ...
          rw [hsmul]
          simpa using (FTIsInvariant.invariant (A := A) (G := K0) (H := Pk) a y)
      ...
      letI : FTIsInvariant B K0 Pk := hB_inv_Pk
      ...
      have hsmul_inl (n : Kk) (x : K0) :
        ...
        calc
          ...
          _ = (n : K0) * x * (n : K0)⁻¹ := by
                simp [conjHomKk, MulAut.conj_apply]
      have hLinvarA : FTIsInvariant A K0 L := by
        refine ⟨?_⟩
        intro a x
        rw [← hsmul_inr (a := a) (x := x)]
        exact FTIsInvariant.invariant (A := B) (G := K0) (H := L) (SemidirectProduct.inr a) x
      have hKk_le_normalizerL : Kk ≤ Subgroup.normalizer L := by
        intro n hn
        rw [Subgroup.mem_normalizer_iff]
        intro y
        rw [← hsmul_inl (n := ⟨n, hn⟩) (x := y)]
        exact FTIsInvariant.invariant (A := B) (G := K0) (H := L) (SemidirectProduct.inl ⟨n, hn⟩) y
      ...
      haveI : FTIsInvariant A K0 L := hLinvarA
      -- Map the complement back into `G'`.
      let L' : Subgroup G' := L.map K0.subtype
      haveI : FTIsInvariant A G' L' := isInvariant_map_subtype (A := A) (G := G') K0 L
      -- Show that K' ≤ L'
...
public theorem exists_isHallSubgroup_isInvariant
    ...
    [MulDistribMulAction A G] (hsolv : IsSolvable G) (hcoprime : Nat.Coprime (Nat.card A) (Nat.card G))
    (π : Set Nat.Primes) :
    ∃ H : Subgroup G, IsHallSubgroup π H ∧ FTIsInvariant A G H := by
  ...
  have hbot_inv : FTIsInvariant A G (⊥ : Subgroup G) :=
