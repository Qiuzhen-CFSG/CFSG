  ...
  have hP : ∀ n, P n := by
    ...
    exact Nat.strong_induction_on (p := P) n <| fun n ih {H} _ _ _ _ hcard hHcop hΩH => by
      ...
      · letI : Nontrivial H := not_subsingleton_iff_nontrivial.mp htriv
        ...
        have hDinv : FTIsInvariant A H D := by
          refine ⟨?_⟩
          intro a x
          constructor
          · exact hDsmul
        ...
        letI : FTIsInvariant A H D := hDinv
        by_cases hDtop : D = ⊤
        · have hC_le_center : C ≤ Subgroup.center H := by
          ...
          have hΦinv : FTIsInvariant A H Φ :=
            isInvariant_of_characteristic (A := A) (G := H) Φ
          letI : FTIsInvariant A H Φ := hΦinv
          by_cases hΦtop : Φ = ⊤
          · exact theorem_1_8 (R := H) (A := A) (p := p) hHcop <| by
              let hΦinv' : FTIsInvariant A H (frattini H) :=
