...
public lemma isInvariant_of_characteristic {G A : Type*} [Group G] [Group A]
    [MulDistribMulAction A G] (H : Subgroup G) [H.Characteristic] : FTIsInvariant A G H := by
  refine ⟨?_⟩
  intro a g
  have hfixed :
...
public lemma isInvariant_normalizer_of_isInvariant {G A : Type*} [Group G] [Group A]
    [MulDistribMulAction A G] (H : Subgroup G) [FTIsInvariant A G H] :
    FTIsInvariant A G (Subgroup.normalizer H) := by
  have hforward : ∀ a : A, ∀ g : G, g ∈ Subgroup.normalizer H → a • g ∈ Subgroup.normalizer H := by
    ...
    calc
      x ∈ H ↔ a⁻¹ • x ∈ H :=
        (FTIsInvariant.invariant (A := A) (G := G) (H := H) a⁻¹ x)
      _ ↔ g * (a⁻¹ • x) * g⁻¹ ∈ H := hg (a⁻¹ • x)
      _ ↔ a • (g * (a⁻¹ • x) * g⁻¹) ∈ H :=
        (FTIsInvariant.invariant (A := A) (G := G) (H := H) a
          (g * (a⁻¹ • x) * g⁻¹))
      _ ↔ (a • g) * x * (a • g)⁻¹ ∈ H := by
...
public lemma isInvariant_map_subtype {G A : Type*} [Group G] [Group A]
    [MulDistribMulAction A G] (H : Subgroup G) [FTIsInvariant A G H] (K : Subgroup H)
    [FTIsInvariant A H K] : FTIsInvariant A G (K.map H.subtype) := by
  -- Use the restricted action on `H`.
  refine ⟨?_⟩
  intro a g
  constructor
  · rintro ⟨x, hx, rfl⟩
    refine ⟨a • x, (FTIsInvariant.invariant (A := A) (G := H) (H := K) a x).1 hx, ?_⟩
    rfl
  · rintro ⟨x, hx, hxg⟩
    refine ⟨a⁻¹ • x, (FTIsInvariant.invariant (A := A) (G := H) (H := K) a⁻¹ x).1 hx, ?_⟩
...
public lemma isInvariant_sup {G A : Type*} [Group G] [Group A]
    ...
    [FTIsInvariant A G H] [FTIsInvariant A G K] :
    FTIsInvariant A G (H ⊔ K) := by
  refine ⟨?_⟩
  intro a g
  constructor
  · intro hg
    ...
    · intro x hx
      ...
      · exact Subgroup.subset_closure (Or.inl ((FTIsInvariant.invariant (A := A) (G := G) (H := H) a x).1 hx))
      · exact Subgroup.subset_closure (Or.inr ((FTIsInvariant.invariant (A := A) (G := G) (H := K) a x).1 hx))
    · simp
    · intro x y _ _ hx hy
...
public lemma isInvariant_commutator {G A : Type*} [Group G] [Group A]
  ...
  have hforward : ∀ a : A, ∀ x : G, x ∈ ⁅H, K⁆ → a • x ∈ ⁅H, K⁆ := by
    ...
    · rintro y ⟨h, hh, k, hk, rfl⟩
      ...
      refine ⟨a • h, (FTIsInvariant.invariant (A := A) (G := G) (H := H) a h).1 hh,
        a • k, (FTIsInvariant.invariant (A := A) (G := G) (H := K) a k).1 hk, ?_⟩
      calc
...
public lemma isInvariant_centralizer {G A : Type*} [Group G] [Group A]
    [MulDistribMulAction A G] (H : Subgroup G) [FTIsInvariant A G H] :
    FTIsInvariant A G (Subgroup.centralizer (H : Set G)) := by
  have hforward : ∀ a : A, ∀ g : G,
    ...
    intro h hh
    have hh' : a⁻¹ • h ∈ H :=
      (FTIsInvariant.invariant (A := A) (G := G) (H := H) a⁻¹ h).1 hh
...
public lemma isInvariant_inf {G A : Type*} [Group G] [Group A]
  ...
  constructor
  · rintro ⟨hgH, hgK⟩
    exact ⟨(FTIsInvariant.invariant (A := A) (G := G) (H := H) a g).1 hgH,
      (FTIsInvariant.invariant (A := A) (G := G) (H := K) a g).1 hgK⟩
  · intro hg
    rcases hg with ⟨hgH, hgK⟩
    have hg' : a⁻¹ • (a • g) ∈ H ⊓ K := by
      refine ⟨(FTIsInvariant.invariant (A := A) (G := G) (H := H) a⁻¹ (a • g)).1 hgH,
        (FTIsInvariant.invariant (A := A) (G := G) (H := K) a⁻¹ (a • g)).1 hgK⟩
    simpa [inv_smul_smul] using hg'

public lemma isInvariant_subgroupOf {G A : Type*} [Group G] [Group A]
    [MulDistribMulAction A G] (H K : Subgroup G)
    [FTIsInvariant A G H] [FTIsInvariant A G K] :
    FTIsInvariant A K (H.subgroupOf K) := by
  refine ⟨?_⟩
  intro a x
  constructor
  · intro hx
    ...
    have hmem : (a • (x : K) : K) ∈ H.subgroupOf K := by
      have : (a • (x : K) : G) ∈ H := by
        exact (FTIsInvariant.invariant (A := A) (G := G) (H := H) a (x : G)).1 hx'
      change a • (x : G) ∈ H
      exact this
    exact hmem
  · intro hx
    ...
    have hx_inv : (x : G) ∈ H := by
      simpa using (FTIsInvariant.invariant (A := A) (G := G) (H := H) a (x : G)).2 hx'
    exact hx_inv
...
public theorem isInvariant_sup_of_le_normalizer
    ...
    [FTIsInvariant A G X] [FTIsInvariant A G Y] :
    FTIsInvariant A G (X ⊔ Y) := by
  ...
  have hforward : ∀ a : A, ∀ g : G, g ∈ X ⊔ Y → a • g ∈ X ⊔ Y := by
    ...
    have hxX : a • ((x : ↥(X ⊔ Y)) : G) ∈ X := by
      exact
        (FTIsInvariant.invariant (A := A) (G := G) (H := X) a (((x : ↥(X ⊔ Y)) : G))).1 <|
          by simpa [Subgroup.mem_subgroupOf] using hx
    have hyY : a • ((y : ↥(X ⊔ Y)) : G) ∈ Y := by
      exact
        (FTIsInvariant.invariant (A := A) (G := G) (H := Y) a (((y : ↥(X ⊔ Y)) : G))).1 <|
