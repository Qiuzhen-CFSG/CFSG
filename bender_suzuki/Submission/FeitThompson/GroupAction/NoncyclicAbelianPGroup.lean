...
theorem proposition_1_16_b_qgroup
  ...
  have hsolv : IsSolvable G := by
    ...
    infer_instance
  let hfrattini_inv : FTIsInvariant A G (frattini G) :=
  ...
  have hKbar_eq : K.map (QuotientGroup.mk' (frattini G)) = Kbar := by
    calc
      ...
      _ = Kbar := by
            ...
            have hsubinv : FTIsInvariant ↥Y.1 G (frattini G) := by
              constructor
              intro z g
...
theorem fixedPointSubgroup_map_subtype_le
    ...
    (H : Subgroup G) [FTIsInvariant A G H] (Y : Subgroup A) :
...
public theorem exists_invariant_sylow
    ...
    ∃ P : Sylow q G, FTIsInvariant A G (P : Subgroup G) := by
  classical
...
public theorem iSup_fixedPointSubgroup_cyclicQuot_eq_top_of_noncyclic_abelian_pGroup_action
  ...
  have hHtop : H = ⊤ := by
    ...
    letI : FTIsInvariant A G (Q : Subgroup G) := hQinv
