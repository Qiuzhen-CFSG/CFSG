...
public instance center_isInvariant {G A : Type*} [Group G] [Group A] [MulDistribMulAction A G] :
    FTIsInvariant A G (Subgroup.center G) where
  invariant a g := by
...
public instance quotientAction_of_invariant {G A : Type*} [Group G] [Group A] [MulDistribMulAction A G]
    (H : Subgroup G) [FTIsInvariant A G H] : MulAction.QuotientAction A H := by
  constructor
  intro b a a' h
  have h_inv : a⁻¹ * a' ∈ H := h
  have h_smul : b • (a⁻¹ * a') ∈ H := (FTIsInvariant.invariant (A := A) (G := G) (H := H) b (a⁻¹ * a')).1 h_inv
...
public instance quotient_mulDistribMulAction {G A : Type*} [Group G] [Group A] [MulDistribMulAction A G]
    (H : Subgroup G) [H.Normal] [FTIsInvariant A G H] : MulDistribMulAction A (G ⧸ H) :=
