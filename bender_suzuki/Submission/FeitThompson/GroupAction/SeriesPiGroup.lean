...
instance FTIsInvariant.subgroup {A G : Type*} [Group A] [Group G] [SMul A G] (H : Subgroup G) [FTIsInvariant A G H]
    (B : Subgroup A) : FTIsInvariant B G H where
  invariant a g := FTIsInvariant.invariant (A := A) (G := G) (H := H) a g
...
public theorem isPiGroup_quotient_fixingSubgroup_of_stabilizesNormalSeries {G A : Type*} [Group G] [Finite G] [Group A] [Finite A] [MulDistribMulAction A G]
  ...
  have smul_eq : ∀ (a : A) (g : G), ((a : Q) • g) = a • g := by
    ...
    calc
      ...
      _ = a • g := rfl
  have hinvQ : ∀ i, FTIsInvariant Q G (Gi i) := by
    intro i
    constructor
    intro aQ g
    ...
    constructor
    · intro hg
      have hg' := (FTIsInvariant.invariant (A := A) (G := G) (H := Gi i) a g).1 hg
      simpa [smul_eq] using hg'
    · intro hg
      have hg' : a • g ∈ Gi i := by simpa [smul_eq] using hg
      exact (FTIsInvariant.invariant (A := A) (G := G) (H := Gi i) a g).2 hg'
  ...
  · by_cases h_not : p ∉ π
    · have h_coprime_G' : ¬ p.val ∣ Nat.card G := not_dvd_card_of_isPiGroup_of_prime_notMem π G hpi p h_not
      ...
      let P' : Subgroup Q := P
      have hinvP : ∀ i, FTIsInvariant P' G (Gi i) := fun i => FTIsInvariant.subgroup (Gi i) P'
      ...
      have h_triv_chain : ∀ k, k ≤ n → ∀ a : P', ∀ g : G, g ∈ Gi (Nat.iterate next k top) → a • g = g := by
        ...
        have step : ∀ k, k < n → (∀ a : P', ∀ g : G, g ∈ Gi (Nat.iterate next (k + 1) top) → a • g = g) →
          ...
          haveI : FTIsInvariant P' G (Gi (Nat.iterate next (k + 1) top)) := hinvP _
          haveI : FTIsInvariant P' G (Gi (Nat.iterate next k top)) := hinvP _
