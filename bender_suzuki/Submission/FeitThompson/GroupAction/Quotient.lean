...
/-- If a subgroup `H` is `A`-invariant (in the sense of `FTIsInvariant`), then the action descends to
`G ⧸ H`. -/
public lemma quotientAction_of_isInvariant (H : Subgroup G) (hH : FTIsInvariant A G H) :
...
@[reducible, expose]
public noncomputable def quotientMulDistribMulAction (H : Subgroup G) (hH : FTIsInvariant A G H)
...
public theorem fixedPointSubgroup_map_mk'_le_fixedPointSubgroup_quotient
    (H : Subgroup G) [H.Normal] (hH : FTIsInvariant A G H) :
    ...
    · haveI : IsSolvable H' := by infer_instance
      ...
      haveI : FTIsInvariant A H' N0 := by
        ...
        · intro hg
          ...
          rw [hfixed] at hg''
          exact hg''
      have hNinv : FTIsInvariant A G' N := by
        refine ⟨?_⟩
        intro a g
        constructor
        · rintro ⟨x, hx, rfl⟩
          refine ⟨a • x, (FTIsInvariant.invariant (A := A) (G := H') (H := N0) a x).1 hx, ?_⟩
          rfl
        · rintro ⟨x, hx, hxg⟩
          refine ⟨a⁻¹ • x, (FTIsInvariant.invariant (A := A) (G := H') (H := N0) a⁻¹ x).1 hx, ?_⟩
          have : ((a⁻¹ • x : H') : G') = g := by
          ...
          simp [this]
      letI : FTIsInvariant A G' N := hNinv
      ...
      have hHbar_inv : FTIsInvariant A Q Hbar := by
        refine ⟨?_⟩
        intro a q
        constructor
        · rintro ⟨g, hg, rfl⟩
          refine ⟨a • g, (FTIsInvariant.invariant (A := A) (G := G') (H := H') a g).1 hg, ?_⟩
        ...
        · rintro ⟨g, hg, hq⟩
          ...
          have : fN (a⁻¹ • g) = a⁻¹ • fN g := by
            ...
            exact MulAction.Quotient.smul_mk (H := N) (a⁻¹) g
          simp [this, hq]
      letI : FTIsInvariant A Q Hbar := hHbar_inv
...
public lemma isInvariant_map_quotient {G A : Type*} [Group G] [Group A] [MulDistribMulAction A G]
    {N : Subgroup G} [N.Normal] [FTIsInvariant A G N]
    (H : Subgroup G) [FTIsInvariant A G H] :
    letI : MulDistribMulAction A (G ⧸ N) :=
      quotientMulDistribMulAction (A := A) (G := G) N inferInstance
    FTIsInvariant A (G ⧸ N) (H.map (mk' N)) := by
...
public lemma isInvariant_comap_quotient {G A : Type*} [Group G] [Group A] [MulDistribMulAction A G]
    {N : Subgroup G} [N.Normal] [FTIsInvariant A G N]
    (H : Subgroup (G ⧸ N))
    [hQ : MulDistribMulAction A (G ⧸ N)] [FTIsInvariant A (G ⧸ N) H]
    (hq : ∀ a : A, ∀ g : G, a • ((mk' N) g) = (mk' N) (a • g)) :
    FTIsInvariant A G (H.comap (mk' N)) := by
  refine ⟨?_⟩
  intro a g
  constructor
  · intro hg
    change (mk' N) (a • g) ∈ H
    rw [← hq a g]
    exact (FTIsInvariant.invariant (A := A) (G := G ⧸ N) (H := H) a ((mk' N) g)).1 hg
  · intro hg
    change (mk' N) g ∈ H
    have hg' : a⁻¹ • ((mk' N) (a • g)) ∈ H :=
      (FTIsInvariant.invariant (A := A) (G := G ⧸ N) (H := H) a⁻¹ ((mk' N) (a • g))).1 hg
