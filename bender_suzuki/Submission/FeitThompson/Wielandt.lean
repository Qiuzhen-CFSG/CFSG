...
public theorem
    ...
    {p : ℕ} [Fact p.Prime] [IsElementaryAbelian p V]
    (hcop : Nat.Coprime (Nat.card V) (Nat.card G))
    (_hminv : ∀ N : Subgroup V, N.Normal → FTIsInvariant G V N → N ≠ ⊥ → N = ⊤)
    {e : ℕ} (he : 1 < e) :
    Nonempty
...
public theorem
  ...
  let hΦinv : FTIsInvariant A L.cover (frattini L.cover) :=
    isInvariant_of_characteristic (frattini L.cover)
  letI : FTIsInvariant A L.cover (frattini L.cover) := hΦinv
...
private noncomputable def
    ...
    let hkerInv : FTIsInvariant A L.cover φ.ker := by
    ...
    letI : FTIsInvariant A L.cover φ.ker := hkerInv
  ...
  haveI : φ.ker.Characteristic := by
    simpa [φ] using powMonoidHom_ker_characteristic (H := L.cover) p
  let hkerInv : FTIsInvariant A L.cover φ.ker := isInvariant_of_characteristic φ.ker
  letI : FTIsInvariant A L.cover φ.ker := hkerInv
  letI : MulDistribMulAction A φ.ker :=
    instMulDistribMulAction_subtype (A := A) (G := L.cover) (H := φ.ker)
  refine {
...
public theorem
  ...
  let hkerInv : FTIsInvariant A L.cover φ.ker := isInvariant_of_characteristic φ.ker
  let hrangeInv : FTIsInvariant A L.cover φ.range := isInvariant_of_characteristic φ.range
  let hΦinv : FTIsInvariant A L.cover (frattini L.cover) :=
    isInvariant_of_characteristic (frattini L.cover)
  letI : FTIsInvariant A L.cover φ.ker := hkerInv
  letI : FTIsInvariant A L.cover φ.range := hrangeInv
  letI : FTIsInvariant A L.cover (frattini L.cover) := hΦinv
  letI : MulDistribMulAction A φ.ker :=
    instMulDistribMulAction_subtype (A := A) (G := L.cover) (H := φ.ker)
