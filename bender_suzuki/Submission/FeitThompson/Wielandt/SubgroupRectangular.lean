...
public noncomputable def fixedPointSubgroupPowQuotientKerEquivRange
    ...
    (p : ℕ) :
    let hker : FTIsInvariant A M ((powMonoidHom p : M →* M).ker) := by
    ...
    let hrange : FTIsInvariant A M ((powMonoidHom p : M →* M).range) := by
    ...
    letI : FTIsInvariant A M ((powMonoidHom p : M →* M).ker) := hker
    letI : FTIsInvariant A M ((powMonoidHom p : M →* M).range) := hrange
  ...
  let hker : FTIsInvariant A M φ.ker :=
    isInvariant_of_characteristic φ.ker
  let hrange : FTIsInvariant A M φ.range :=
    isInvariant_of_characteristic φ.range
  letI : FTIsInvariant A M φ.ker := hker
  letI : FTIsInvariant A M φ.range := hrange
