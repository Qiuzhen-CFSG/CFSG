...
private theorem exists_maximal_cyclic_invariant_subgroup_containing_derived
    ...
    ∃ S : Subgroup R,
      derivedSubgroup R ≤ S ∧ IsCyclic S ∧ FTIsInvariant A R S ∧
        ∀ T : Subgroup R, derivedSubgroup R ≤ T → IsCyclic T → FTIsInvariant A R T →
          S ≤ T → T = S := by
  classical
  let s : Set (Subgroup R) := {S | derivedSubgroup R ≤ S ∧ IsCyclic S ∧ FTIsInvariant A R S}
...
private noncomputable def actionMulAutOfInvariant
    ...
    (S : Subgroup G) [FTIsInvariant A G S] (a : A) : MulAut S :=
  {
    toFun := fun s =>
      ⟨a • (s : G), (FTIsInvariant.invariant (A := A) (G := G) (H := S) a (s : G)).1 s.2⟩
    invFun := fun s =>
      ⟨a⁻¹ • (s : G),
        (FTIsInvariant.invariant (A := A) (G := G) (H := S) a⁻¹ (s : G)).1 s.2⟩
    left_inv := by
      intro s
      ext
      simp
...
private theorem action_commutator_generator_le_centralizer_of_cyclic_invariant
    {G A : Type*} [Group G] [Group A] [MulDistribMulAction A G]
    {S : Subgroup G} [S.Normal] [FTIsInvariant A G S] (hScyc : IsCyclic S)
    (a : A) (g : G) :
...
private theorem le_center_of_commutatorAction_eq_top_of_cyclic_invariant
    ...
    {S : Subgroup R} [S.Normal] [FTIsInvariant A R S] (hScyc : IsCyclic S)
...
public theorem exists_isCompl_isInvariant_of_elementaryAbelian_coprime
    ...
    (hcop : Nat.Coprime p (Nat.card A)) (B : Subgroup G) [FTIsInvariant A G B] :
    ∃ C : Subgroup G, IsCompl B C ∧ FTIsInvariant A G C := by
  classical
  ...
  have hBinv : η B ∈ ρ.invtSubmodule := by
    ...
    simpa [ρ, η] using
      (FTIsInvariant.invariant (A := A) (G := G) (H := B) a (Additive.toMul x)).1 hxB
  ...
  have hCinv : FTIsInvariant A G C := by
    refine ⟨?_⟩
    intro a g
    constructor
    · intro hg
  ...
  | h n ih =>
    ...
    · have hdercyc : IsCyclic (derivedSubgroup R) :=
      ...
      have hquot_cyc : IsCyclic (R ⧸ S) := by
        ...
        haveI : FTIsInvariant A R (ΩR ⊔ S) := isInvariant_sup ΩR S
        haveI : FTIsInvariant A (R ⧸ S) B0 := by
          simpa [B0, qS] using
            isInvariant_map_quotient (A := A) (G := R) (N := S) (H := ΩR ⊔ S)
        haveI : FTIsInvariant A ΩQ B := by
        ...
        have hC_bot : C = ⊥ := by
          ...
          have hS_le_X : S ≤ X := by
            ...
            simp [Cbar, hs1]
          haveI : FTIsInvariant A ΩQ C := hCinv
          haveI : FTIsInvariant A (R ⧸ S) Cbar := by
            simpa [Cbar, ΩQ] using isInvariant_map_subtype (A := A) (G := R ⧸ S) ΩQ C
          haveI : FTIsInvariant A R X := by
