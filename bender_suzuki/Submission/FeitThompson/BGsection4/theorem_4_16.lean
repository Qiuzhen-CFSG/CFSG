...
public theorem theorem_4_16 {R A : Type*} [Group R] [Finite R] [Nontrivial R] [Group A] [Finite A]
  ...
  · obtain ⟨hΩexp, hΩcard⟩ :=
    ...
    · have hS_normal : S.Normal := by
      ...
      have hS_inv : FTIsInvariant A R S :=
        isInvariant_of_characteristic (A := A) (G := R) S
      letI : FTIsInvariant A R S := hS_inv
      have htop_inv : FTIsInvariant A R (⊤ : Subgroup R) :=
        isInvariant_of_characteristic (A := A) (G := R) (⊤ : Subgroup R)
      letI : FTIsInvariant A R (⊤ : Subgroup R) := htop_inv
      have hT_inv : FTIsInvariant A R T := by
        simpa [T] using
          isInvariant_commutator (A := A) S (⊤ : Subgroup R)
      letI : FTIsInvariant A R T := hT_inv
      have hCT_inv : FTIsInvariant A R C_T := by
        simpa [C_T] using isInvariant_centralizer (A := A) T
      letI : FTIsInvariant A R C_T := hCT_inv
      ...
      have hD_eq_comm : D = ⁅S, S⁆ := by
        change (derivedSubgroup S).map S.subtype = ⁅S, S⁆
        rw [derivedSubgroup, derivedSeries_one, Subgroup.map_subtype_commutator]
      have hD_inv : FTIsInvariant A R D := by
        rw [hD_eq_comm]
        simpa using isInvariant_commutator (A := A) S S
      letI : FTIsInvariant A R D := hD_inv
      have hD_le_C : D ≤ C := by
        rw [hD_eq_ZS]
        intro x hx
        rcases Subgroup.mem_map.mp hx with ⟨z, hz, rfl⟩
      ...
      have hS_action_not_le_T :
        ...
        have hgen_mem :
          ...
          · intro x hx
            ...
            · have haxCT : a • x ∈ C_T :=
                (FTIsInvariant.invariant (A := A) (G := R) (H := C_T) a x).1 hxCT
      ...
      have hBq_inv : FTIsInvariant A (R ⧸ C) Bq := by
        simpa [Bq, qC] using
      ...
      have hB_inv : FTIsInvariant A Q B := by
        letI : FTIsInvariant A (R ⧸ C) Bq := hBq_inv
      ...
      letI : FTIsInvariant A Q B := hB_inv
      obtain ⟨Y, hBY, hY_inv⟩ :=
        exists_isCompl_isInvariant_of_elementaryAbelian_coprime
          (G := Q) (A := A) (p := p) hcop B
      letI : FTIsInvariant A Q Y := hY_inv
      let Ybar : Subgroup (R ⧸ C) := Y.map Q.subtype
      have hYbar_le_Q : Ybar ≤ Q := by
        exact Subgroup.map_subtype_le Y
      have hYbar_inv : FTIsInvariant A (R ⧸ C) Ybar := by
        refine ⟨?_⟩
        intro a g
        constructor
        · rintro ⟨yq, hyY, rfl⟩
          refine ⟨a • yq, (FTIsInvariant.invariant (A := A) (G := Q) (H := Y) a yq).1 hyY, ?_⟩
          rfl
        · rintro ⟨yq, hyY, hg⟩
          refine ⟨a⁻¹ • yq, (FTIsInvariant.invariant (A := A) (G := Q) (H := Y) a⁻¹ yq).1 hyY, ?_⟩
          change ((a⁻¹ • yq : Q) : R ⧸ C) = g
      ...
      letI : FTIsInvariant A (R ⧸ C) Ybar := hYbar_inv
      let X : Subgroup R := Ybar.comap qC
      have hX_inv : FTIsInvariant A R X := by
        refine isInvariant_comap_quotient
          (A := A) (G := R) (N := C) Ybar ?_
        intro a g
        simp [MulAction.Quotient.smul_mk]
      letI : FTIsInvariant A R X := hX_inv
      have hC_le_X : C ≤ X := by
      ...
      have hαyS_mem : α • y ∈ S :=
        (FTIsInvariant.invariant (A := A) (G := R) (H := S) α y).1 hyS
      ...
      have hαzT_mem : α • z ∈ T :=
        (FTIsInvariant.invariant (A := A) (G := R) (H := T) α z).1 hzT
      ...
      have hαxX_mem : α • x ∈ X :=
        (FTIsInvariant.invariant (A := A) (G := R) (H := X) α x).1 hxX_mem
      ...
      have hDsubT_inv : FTIsInvariant A T DsubT := by
        simpa [DsubT] using isInvariant_subgroupOf D T
      letI : FTIsInvariant A T DsubT := hDsubT_inv
      ...
      have hTsub_inv : FTIsInvariant A S Tsub := by
        simpa [Tsub] using isInvariant_subgroupOf T S
      letI : FTIsInvariant A S Tsub := hTsub_inv
