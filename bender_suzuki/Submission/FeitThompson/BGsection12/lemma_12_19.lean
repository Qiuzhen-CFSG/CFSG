...
private theorem section12_invariant_hall_fixed_by_ambientDerivedSubgroup
    ...
    (hHinv : FTIsInvariant (ambientDerivedSubgroup E) (section10Msigma M) H) :
  ...
  have hSylow_fixed :
    ...
    letI : FTIsInvariant D S H := hHinv
    have hHinvQ : FTIsInvariant (Q : Subgroup D) S H := by
      refine ⟨?_⟩
      intro a y
      change y ∈ H ↔ (a : D) • y ∈ H
      exact FTIsInvariant.invariant (A := D) (G := S) (H := H) (a : D) y
    have hLinvQ : FTIsInvariant (Q : Subgroup D) S L :=
...
private theorem section12_exists_invariant_hall_betaCompl_msigma
    ...
    ∃ H : Subgroup (section10Msigma M),
      IsHallSubgroup (section10BetaPrimes M)ᶜ H ∧
        FTIsInvariant (ambientDerivedSubgroup E) (section10Msigma M) H := by
  classical
  let D : Subgroup G := ambientDerivedSubgroup E
  let S : Subgroup G := section10Msigma M
  have hD_le_E : D ≤ E := by
