...
public theorem section12_exists_isCompl_isInvariant_of_elementaryAbelian_coprime
    ...
    (hcop : Nat.Coprime p (Nat.card A)) (B : Subgroup V) [FTIsInvariant A V B] :
    ∃ C : Subgroup V, IsCompl B C ∧ FTIsInvariant A V C := by
  classical
  ...
  have hBinv : η B ∈ ρ.invtSubmodule := by
    ...
    simpa [ρ, η] using
      (FTIsInvariant.invariant (A := A) (G := V) (H := B) a (Additive.toMul x)).1 hxB
  ...
  have hCinv : FTIsInvariant A V C := by
    refine ⟨?_⟩
    intro a v
    constructor
    · intro hv
...
private theorem section12_isInvariant_map_quotient_local
    ...
    FTIsInvariant A (G ⧸ N) (H.map (QuotientGroup.mk' N)) := by
  letI : MulDistribMulAction A (G ⧸ N) :=
    quotientMulDistribMulAction (A := A) (G := G) N inferInstance
  refine ⟨?_⟩
  intro a q
...
private theorem section12_isInvariant_comap_quotient_local
    ...
    {N : Subgroup G} [N.Normal] [FTIsInvariant A G N]
    (H : Subgroup (G ⧸ N))
    [MulDistribMulAction A (G ⧸ N)] [FTIsInvariant A (G ⧸ N) H]
    (hq : ∀ a : A, ∀ g : G,
      a • ((QuotientGroup.mk' N) g) = (QuotientGroup.mk' N) (a • g)) :
    FTIsInvariant A G (H.comap (QuotientGroup.mk' N)) := by
  refine ⟨?_⟩
  intro a g
  constructor
  · intro hg
...
public theorem section12_CA_msigma_complement_in_E2
  ...
  letI : FTIsInvariant E₁ (E₂ ⧸ Φ) Q := hQinv
  have hqcompat : ∀ a : E₁, ∀ g : E₂, a • q g = q (a • g) := by
    intro a g
    simp [q, MulAction.Quotient.smul_mk]
  have hP₀subInv : FTIsInvariant E₁ E₂ P₀sub := by
    simpa [P₀sub, q] using
  ...
  · have hforward (a : E₁) {x : G} (hx : x ∈ P₀) :
      ...
      · exact (FTIsInvariant.invariant (A := E₁) (G := E₂) (H := P₀sub) a y).1 hyP₀sub
