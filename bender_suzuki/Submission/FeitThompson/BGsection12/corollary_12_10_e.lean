...
public theorem corollary_12_10_e
  ...
  have hxM_inv : FTIsInvariant PUnit.{1} M ((Subgroup.zpowers x).subgroupOf M) :=
    ⟨fun _ _ => ⟨id, id⟩⟩
  ...
  have hx₁E_pi : IsPiSubgroup (G := E) (section12Tau2Primes M)
    ...
    have hp_mem : p ∈ subgroupPrimeSet (Subgroup.zpowers x₁) := by
      dsimp [subgroupPrimeSet]; exact hp
    exact hx₁_π hp_mem
  have hx₁E_inv : FTIsInvariant PUnit.{1} E ((Subgroup.zpowers x₁).subgroupOf E) :=
    ⟨fun _ _ => ⟨id, id⟩⟩
  ...
  have hA_inv : FTIsInvariant PUnit.{1} E (A.subgroupOf E) := ⟨fun _ _ => ⟨id, id⟩⟩
  rcases proposition_1_5_b hE_solv hcopE (section12Tau2Primes M)
