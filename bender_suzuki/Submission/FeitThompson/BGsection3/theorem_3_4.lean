...
public instance instMulDistribMulAction_subtype_local {G A : Type*} [Group G] [Group A]
  ...
  smul a x := ⟨a • x.1, (FTIsInvariant.invariant (A := A) (G := G) (H := H) a x.1).1 x.2⟩
  one_smul x := by
    ext
    change ((1 : A) • (x : G)) = x
    simp
...
theorem proper_characteristic_le_fixedPointSubgroup_local
  ...
  have hRinv : ∀ r : R, ∀ h ∈ H', (r : G) * h * (r : G)⁻¹ ∈ H' := by
    letI : H.Characteristic := hH_char
    haveI : FTIsInvariant (↥R) (↥K) H := isInvariant_of_characteristic (A := ↥R) (G := ↥K) H
    intro r h hh
    rcases hh with ⟨x, hx, rfl⟩
    refine ⟨r • x, (FTIsInvariant.invariant (A := ↥R) (G := ↥K) (H := H) r x).1 hx, ?_⟩
...
theorem theorem_3_4_quotient_center_fixfree_local
  ...
  have hZinv : FTIsInvariant (↥R) (↥K) Z :=
...
public theorem subgroupCentralizerIn_map_mk'_eq_map_of_solvable_coprime
  ...
  haveI : Subgroup.Normalizes R H := ⟨hRnormH⟩
  have hXsub_inv : FTIsInvariant (↥R) (↥H) Xsub := by
    refine ⟨?_⟩
    intro a x
    constructor
    · intro hx
  ...
  letI : FTIsInvariant (↥R) (↥H) Xsub := hXsub_inv
...
theorem elementCentralizerIn_map_mk'_eq_map_center_of_solvable_coprime
  ...
  have hNsub_inv : FTIsInvariant (↥(Subgroup.zpowers r)) (↥K) Nsub := by
    refine ⟨?_⟩
    intro a x
    constructor
    · intro hx
  ...
  letI : FTIsInvariant (↥(Subgroup.zpowers r)) (↥K) Nsub := hNsub_inv
