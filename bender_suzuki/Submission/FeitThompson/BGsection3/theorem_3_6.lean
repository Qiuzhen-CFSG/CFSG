...
public theorem theorem_3_6_reduce_eq_commutator {G : Type uG} [Group G] [Finite G]
  ...
  letI : FTIsInvariant (↥R) (↥H) C := commutatorAction_isInvariant (G := ↥H) (A := ↥R)
  have hRinv : ∀ r : R, ∀ x ∈ N, (r : G) * x * (r : G)⁻¹ ∈ N := by
    ...
    have hsmul : r • xH ∈ C := (FTIsInvariant.invariant (A := ↥R) (G := ↥H) (H := C) r xH).1 hxH
...
public theorem theorem_3_6_characteristic_quotient_step {G : Type uG} [Group G] [Finite G]
  ...
  have hXinv : ∀ r : R, ∀ x ∈ X, (r : G) * x * (r : G)⁻¹ ∈ X := by
    ...
    haveI : FTIsInvariant (↥R) (↥H) XH := isInvariant_of_characteristic (A := ↥R) (G := ↥H) XH
    intro r x hx
    rcases hx with ⟨xH, hxH, rfl⟩
    refine ⟨r • xH, (FTIsInvariant.invariant (A := ↥R) (G := ↥H) (H := XH) r xH).1 hxH, ?_⟩
...
noncomputable instance {H : Type*} [Group H] [Finite H] {x : H} {V : Subgroup H} [V.Normal] : MulDistribMulAction (↥(Subgroup.zpowers x)) (↥V ⧸ frattini V) := by
  have hfrattini_inv : FTIsInvariant (↥(Subgroup.zpowers x)) (↥V) (frattini V) :=
...
public theorem theorem_3_6_invariant_complement_in_fitting_preimage
    ...
    ∃ K : Subgroup H,
      ...
      Disjoint V K ∧
      FTIsInvariant (↥R) (↥H) K ∧
      FTIsInvariant (↥R) (↥H) (Subgroup.normalizer K) := by
  letI : Fact p.Prime := ⟨hp⟩
  ...
  have hV_inv : FTIsInvariant (↥R) (↥H) V :=
    isInvariant_of_characteristic (A := ↥R) (G := ↥H) V
  letI : FTIsInvariant (↥R) (↥H) V := hV_inv
  ...
  have hU_inv : FTIsInvariant (↥R) (↥H) U :=
    isInvariant_of_characteristic (A := ↥R) (G := ↥H) U
  letI : FTIsInvariant (↥R) (↥H) U := hU_inv
  ...
  have hVsub_inv : FTIsInvariant (↥R) (↥U) (V.subgroupOf U) := by
    refine ⟨?_⟩
    intro a x
    change ((x : H) ∈ V) ↔ ((((a • x : U) : U) : H) ∈ V)
    have hx := FTIsInvariant.invariant (A := ↥R) (G := ↥H) (H := V) a (x : H)
    change ((x : H) ∈ V) ↔ ((((a • x : U) : U) : H) ∈ V) at hx
    exact hx
  letI : FTIsInvariant (↥R) (↥U) (V.subgroupOf U) := hVsub_inv
  ...
  letI : FTIsInvariant (↥R) (↥U) K0 := hK0_inv
  ...
  have hK_inv : FTIsInvariant (↥R) (↥H) K := by
    simpa [K] using isInvariant_map_subtype (A := ↥R) (G := ↥H) U K0
  letI : FTIsInvariant (↥R) (↥H) K := hK_inv
  have hNK_inv : FTIsInvariant (↥R) (↥H) (Subgroup.normalizer K) := by
...
public theorem theorem_3_6_normalizer_sup_top
    ...
    ∃ K : Subgroup H,
      ...
      Disjoint V K ∧
      FTIsInvariant (↥R) (↥H) K ∧
      FTIsInvariant (↥R) (↥H) (Subgroup.normalizer K) ∧
      V ⊔ Subgroup.normalizer K = ⊤ := by
  letI : Fact p.Prime := ⟨hp⟩
  let hRnormH : R ≤ Subgroup.normalizer H := Subgroup.le_normalizer_of_normal (H := H)
  let V : Subgroup H := fittingSubgroup H
...
private theorem exists_invariant_pSubgroup_of_isInvariant
    ...
    {H : Subgroup G} [FTIsInvariant A G H] {p : ℕ} [Fact p.Prime]
    (hsolvH : IsSolvable ↥H) (hcopAH : Nat.Coprime (Nat.card A) (Nat.card H)) :
    ∃ P : Subgroup H, IsPGroup p P ∧ ¬ p ∣ P.index ∧ FTIsInvariant A H P := by
...
public theorem theorem_3_6_exists_normalizer_invariant_pSubgroup_with_nontrivial_commutator
    ...
    (hVK_sup :
      fittingSubgroup H ⊔ K =
        (fittingSubgroup (↥H ⧸ fittingSubgroup H)).comap
          (QuotientGroup.mk' (fittingSubgroup H)))
    (hNK_inv : FTIsInvariant (↥R) (↥H) (normalizerOf K))
    (hVNK_sup : fittingSubgroup H ⊔ normalizerOf K = ⊤) :
    ∃ P : Subgroup (normalizerOf K),
      IsPGroup p P ∧
      ¬ p ∣ P.index ∧
      FTIsInvariant (↥R) (↥(normalizerOf K)) P ∧
...
private theorem normal_map_subtype_of_normal_and_isInvariant
  ...
  have hRinv :
    ...
    have hsmul : r • xH ∈ S := (FTIsInvariant.invariant (A := ↥R) (G := ↥H) (H := S) r xH).1 hxH
    have hmem : (((r • xH : H) : G)) ∈ N := by
...
private theorem isInvariant_subgroupCentralizerIn_of_isInvariant
    ...
    (V K : Subgroup G) [FTIsInvariant A G V] [FTIsInvariant A G K] :
    FTIsInvariant A G (subgroupCentralizerIn V K) := by
  have hcent_inv : FTIsInvariant A G (Subgroup.centralizer (K : Set G)) :=
...
public theorem theorem_3_6_fitting_commutator_eq_fitting_and_centralizer_eq_bot
    ...
    FTIsInvariant (↥R) (↥H) K →
    FTIsInvariant (↥R) (↥H) (normalizerOf K) →
...
public theorem theorem_3_6_normalizer_complement_structure
  ...
  have hV_char : V.Characteristic := by
    ...
    infer_instance
  letI : V.Characteristic := hV_char
  have hV_inv : FTIsInvariant (↥R) (↥H) V := isInvariant_of_characteristic (A := ↥R) (G := ↥H) V
  letI : FTIsInvariant (↥R) (↥H) V := hV_inv
  have hV_elem : IsElementaryAbelian p ↥V := by
    simpa [V] using
      theorem_3_6_fitting_elementaryAbelian H R R₀ p hind hsolvG hodd hHR hcopHR
...
public theorem theorem_3_6_r0_centralizing_complement_forces_cyclic
  ...
  have hNK_invR : FTIsInvariant (↥R) (↥H) NK := by
    simpa [NK] using hNK_inv
  have hNK_inv₀ : FTIsInvariant (↥R₀) (↥H) NK := by
    refine ⟨?_⟩
    intro a x
    have hx := FTIsInvariant.invariant (A := ↥R) (G := ↥H) (H := NK)
      ⟨(a : G), hR₀_le a.2⟩ x
    change (x ∈ NK) ↔ (a • x ∈ NK) at hx
    exact hx
  letI : FTIsInvariant (↥R₀) (↥H) NK := hNK_inv₀
  ...
  have hKsub_inv₀ : FTIsInvariant (↥R₀) (↥NK) (K.subgroupOf NK) := by
    refine ⟨?_⟩
    intro a x
    constructor
    · intro hx
    ...
    · intro hx
      ...
      exact hxa ▸ hx
  letI : FTIsInvariant (↥R₀) (↥NK) (K.subgroupOf NK) := hKsub_inv₀
...
public theorem theorem_3_6_r0_commutator_nontrivial
  ...
  have hV_char : V.Characteristic := by
    ...
    infer_instance
  letI : V.Characteristic := hV_char
  letI : FTIsInvariant (↥R) (↥H) V := isInvariant_of_characteristic (A := ↥R) (G := ↥H) V
  letI : V.Normal := by
    dsimp [V]
    infer_instance
...
public theorem theorem_3_6_centralizer_KR₀_on_fitting_eq_bot
  ...
  have hR₀_le_normKg : R₀ ≤ Subgroup.normalizer (Kg : Set G) := by
    ...
    · intro hx
      ...
      have hy' : (⟨a, ha⟩ : R₀) • y ∈ K := (FTIsInvariant.invariant (A := ↥R₀) (G := ↥H)
          (H := K) ⟨a, ha⟩ y).1 hyK
      refine Subgroup.mem_map.mpr ?_
    ...
    · intro hx
      ...
      have hy' : (⟨a⁻¹, ha_inv⟩ : R₀) • y ∈ K := (FTIsInvariant.invariant (A := ↥R₀) (G := ↥H)
...
public theorem theorem_3_6_pSubgroup_eq_commutator_with_R₀
    ...
    (hK_inv : FTIsInvariant (↥R) (↥H) K) →
    FTIsInvariant (↥R) (↥H) (normalizerOf K) →
    FTIsInvariant (↥R) (↥H) (normalizerSubtypeMap K P) →
    fittingSubgroup H ⊔ normalizerOf K = ⊤ →
    fittingSubgroup H ⊓ normalizerOf K = ⊥ →
    (K.subgroupOf (normalizerOf K)) = fittingSubgroup (normalizerOf K) →
    subgroupCentralizerIn (⊤ : Subgroup H) K ≤ K →
  ...
  let hR₀normH : R₀ ≤ Subgroup.normalizer H := hR₀_le.trans hRnormH
  haveI : Subgroup.Normalizes R₀ H := ⟨hR₀normH⟩
  have hPsub_inv₀ : FTIsInvariant (↥R₀) (↥H) Psub := by
    refine ⟨?_⟩
    intro a x
    have hx := FTIsInvariant.invariant (A := ↥R) (G := ↥H) (H := Psub)
      ⟨(a : G), hR₀_le a.2⟩ x
  ...
  have hR₀normPsubg : R₀ ≤ Subgroup.normalizer Psubg := by
    ...
    · intro hx
      ...
      refine Subgroup.mem_map.mpr ?_
      refine ⟨(⟨a, ha⟩ : R₀) • y, ?_, ?_⟩
      · exact (FTIsInvariant.invariant (A := ↥R₀) (G := ↥H) (H := Psub) ⟨a, ha⟩ y).1 hy
      · simp [Subgroup.conjMulDistribMulActionOfLeNormalizer_smul_coe]
    · intro hx
      ...
      have hxPsub_smul : (⟨a, ha⟩ : R₀) • xH ∈ Psub := by
        ...
        simpa [hyx] using hy
      have hxPsub : xH ∈ Psub :=
        (FTIsInvariant.invariant (A := ↥R₀) (G := ↥H) (H := Psub) ⟨a, ha⟩ xH).2 hxPsub_smul
...
public theorem theorem_3_6_local_subambient_step
  ...
  let hR₀normH : R₀ ≤ Subgroup.normalizer H := hR₀_le.trans hRnormH
  haveI : Subgroup.Normalizes R₀ H := ⟨hR₀normH⟩
  have hP_inv₀ : FTIsInvariant (↥R₀) (↥H) Psub := by
    refine ⟨?_⟩
    intro a x
    have hx := FTIsInvariant.invariant (A := ↥R) (G := ↥H) (H := Psub)
      ⟨(a : G), hR₀_le a.2⟩ x
    change (x ∈ Psub) ↔ (a • x ∈ Psub) at hx
    exact hx
  have hX_inv₀ : FTIsInvariant (↥R₀) (↥H) X := by
    refine ⟨?_⟩
    intro a x
    have hx := FTIsInvariant.invariant (A := ↥R) (G := ↥H) (H := X)
      ⟨(a : G), hR₀_le a.2⟩ x
  ...
  have hV_inv₀ : FTIsInvariant (↥R₀) (↥H) V :=
    isInvariant_of_characteristic (A := ↥R₀) (G := ↥H) V
  have hY_inv₀ : FTIsInvariant (↥R₀) (↥H) Y := by
    letI : FTIsInvariant (↥R₀) (↥H) X := hX_inv₀
    letI : FTIsInvariant (↥R₀) (↥H) Psub := hP_inv₀
  ...
  have hN_inv₀ : FTIsInvariant (↥R₀) (↥H) N := by
    letI : FTIsInvariant (↥R₀) (↥H) V := hV_inv₀
    letI : FTIsInvariant (↥R₀) (↥H) Y := hY_inv₀
  ...
  have hR₀invNg : ∀ r : R₀, ∀ x ∈ Ng, (r : G) * x * (r : G)⁻¹ ∈ Ng := by
    ...
    rcases Subgroup.mem_map.mp hx with ⟨y, hy, rfl⟩
    refine Subgroup.mem_map.mpr ?_
    refine ⟨r • y, (FTIsInvariant.invariant (A := ↥R₀) (G := ↥H) (H := N) r y).1 hy, ?_⟩
    simp [Subgroup.conjMulDistribMulActionOfLeNormalizer_smul_coe]
...
public theorem theorem_3_6_H_eq_VKP_R_eq_R₀
  ...
  let Psub : Subgroup H := normalizerSubtypeMap K P
  have hPsub_inv : FTIsInvariant (↥R) (↥H) Psub := by
    letI : FTIsInvariant (↥R) (↥H) (normalizerOf K) := hNK_inv
    letI : FTIsInvariant (↥R) (↥(normalizerOf K)) P := hP_inv
  ...
  have hN_top_of_ambient_top :
    ...
    have hY_inv₀ : FTIsInvariant (↥R₀) (↥H) Y := by
      letI : FTIsInvariant (↥R₀) (↥H) X := hX_inv₀
      letI : FTIsInvariant (↥R₀) (↥H) Psub := hPsub_inv₀
      simpa [Y] using isInvariant_sup_of_le_normalizer X Psub hPsub_normX
    have hY_le_normV : Y ≤ Subgroup.normalizer (V : Set H) :=
      Subgroup.le_normalizer_of_normal (H := V)
    have hN_inv₀ : FTIsInvariant (↥R₀) (↥H) N := by
...
public theorem theorem_3_6_K_is_q_group
  ...
  have hproper_fix :
    ...
    letI : FTIsInvariant (↥R) (↥K) L := isInvariant_of_characteristic (A := ↥R) (G := ↥K) L
    have hLmap_invR : FTIsInvariant (↥R) (↥H) Lmap := by
      simpa [Lmap] using isInvariant_map_subtype (A := ↥R) (G := ↥H) K L
    letI : FTIsInvariant (↥Psub) (↥K) L := isInvariant_of_characteristic (A := ↥Psub) (G := ↥K) L
    have hPsub_normLmap : Psub ≤ Subgroup.normalizer (Lmap : Set H) := by
      refine subgroup_le_normalizer_of_conj_mem Lmap Psub ?_
      intro a x hx
      rcases Subgroup.mem_map.mp hx with ⟨y, hy, rfl⟩
      have hy' : a • y ∈ L := (FTIsInvariant.invariant (A := ↥Psub) (G := ↥K) (H := L) a y).1 hy
...
public theorem theorem_3_6_K_quotient_commutator_fixed_eq_bot
    ...
    let hKcomm_inv : FTIsInvariant (↥Psub) (↥K) (commutator (↥K)) :=
...
public theorem theorem_3_6_K_quotient_commutator_p_faithful
  ...
  let hcomm_inv_zpow : FTIsInvariant (↥(Subgroup.zpowers a)) (↥K) (commutator (↥K)) :=
  ...
  let hPhi_inv_zpow : FTIsInvariant (↥(Subgroup.zpowers a)) (↥K) (frattini (↥K)) :=
...
public theorem theorem_3_6_K_quotient_commutator_r_fixed_ne_bot
    ...
    (∀ X : Subgroup H, X ≤ K →
      ...
      ⁅X, normalizerSubtypeMap K P⁆ = ⊥) →
    R = R₀ →
    let _ : FTIsInvariant (↥R) (↥H) K := hK_inv
    let hKcomm_invR : FTIsInvariant (↥R) (↥K) (commutator (↥K)) :=
  ...
  letI : FTIsInvariant (↥R) (↥H) K := hK_inv
  letI : FTIsInvariant (↥R) (↥H) Psub := by
  ...
  haveI : Fact (IsPGroup qK ↥K) := ⟨hqK_pgroup⟩
  haveI : Subgroup.Normalizes Psub K := ⟨hPsub_normK⟩
  let hKcomm_invP : FTIsInvariant (↥Psub) (↥K) (commutator (↥K)) :=
    isInvariant_of_characteristic (A := ↥Psub) (G := ↥K) (commutator (↥K))
  letI : MulAction.QuotientAction (↥Psub) (commutator (↥K)) :=
  ...
  have hsub_sup : (Psubg.subgroupOf Sg) ⊔ R.subgroupOf Sg = ⊤ := by
    calc
      ...
      _ = ⊤ := by simp [Sg]
  have hK_invS : FTIsInvariant (↥Sg) (↥H) K := by
    refine ⟨?_⟩
  ...
  letI : FTIsInvariant (↥Sg) (↥H) K := hK_invS
  let hKcomm_invS : FTIsInvariant (↥Sg) (↥K) (commutator (↥K)) :=
...
public theorem theorem_3_6_K_commutatorAction_abelian
  ...
  have hRnormKg : R ≤ Subgroup.normalizer (Kg : Set G) := by
    ...
    · intro hx
      rcases Subgroup.mem_map.mp hx with ⟨y, hyK, rfl⟩
      have hy' : (⟨a, ha⟩ : R) • y ∈ K :=
        (FTIsInvariant.invariant (A := ↥R) (G := ↥H) (H := K) ⟨a, ha⟩ y).1 hyK
      exact Subgroup.mem_map_of_mem H.subtype hy'
    · intro hx
      have ha_inv : a⁻¹ ∈ R := R.inv_mem ha
      rcases Subgroup.mem_map.mp hx with ⟨y, hyK, hyx⟩
      have hy' : (⟨a⁻¹, ha_inv⟩ : R) • y ∈ K :=
        (FTIsInvariant.invariant (A := ↥R) (G := ↥H) (H := K) ⟨a⁻¹, ha_inv⟩ y).1 hyK
      refine Subgroup.mem_map.mpr ?_
  ...
  have hKcomm_invR : FTIsInvariant (↥R) (↥K) Kcomm :=
  ...
  have hRnormKcommg : R ≤ Subgroup.normalizer (Kcommg : Set G) := by
    ...
    rcases Subgroup.mem_map.mp hx with ⟨y, hy, rfl⟩
    have hy' : a • y ∈ Kcomm :=
      (FTIsInvariant.invariant (A := ↥R) (G := ↥K) (H := Kcomm) a y).1 hy
...
public theorem theorem_3_6_K_elementaryAbelian
  ...
  have hKcomm_not_Pinvariant :
    ...
    have hX_inv : FTIsInvariant (↥R) (↥H) X := by
      have hKcomm_inv : FTIsInvariant (↥R) (↥K) Kcomm :=
        commutatorAction_isInvariant (G := ↥K) (A := ↥R)
    ...
    have hPhi_eq : frattini (↥K) = commutator (↥K) :=
      theorem_3_6_K_frattini_eq_commutator H R R₀ p hind hsolvG hodd hHR hcopHR
        hR₀_le hR₀_prime hp hCZ hcomm_eq hbad K P hVK_sup hVK_disj hK_inv hPsub_inv
        hKsub_fit hCH_le_K hP_p hPK_ne_bot hproper
    let hKcomm_inv : FTIsInvariant (↥R) (↥K) (commutator (↥K)) :=
      isInvariant_of_characteristic (A := ↥R) (G := ↥K) (commutator (↥K))
    letI : MulAction.QuotientAction (↥R) (commutator (↥K)) :=
    ...
    have hquot_triv_comm : ActsTrivially (A := ↥R) (G := ↥K ⧸ commutator (↥K)) := by
      ...
      simpa using hfix
    let hPhi_inv : FTIsInvariant (↥R) (↥K) (frattini (↥K)) :=
    ...
    have hRnormKg : R ≤ Subgroup.normalizer (Kg : Set G) := by
      ...
      exact Subgroup.mem_map_of_mem H.subtype <|
        (FTIsInvariant.invariant (A := ↥R) (G := ↥H) (H := K) a y).1 hy
    haveI : Subgroup.Normalizes R Kg := ⟨hRnormKg⟩
  ...
  · have hZ_index_ne_one : Z.index ≠ 1 := by
    ...
    have hRnormKg : R ≤ Subgroup.normalizer ((K.map H.subtype : Subgroup G) : Set G) := by
      ...
      · intro hx
        ...
        refine ⟨(⟨a⁻¹, ha_inv⟩ : R) • yK,
          (FTIsInvariant.invariant (A := ↥R) (G := ↥H) (H := K) ⟨a⁻¹, ha_inv⟩ yK).1 hyK, ?_⟩
    ...
    have hR_le_normPsubg : R ≤ Subgroup.normalizer (Psubg : Set G) := by
      ...
      have hy' : (⟨a, a.2⟩ : R) • y ∈ Psub :=
        (FTIsInvariant.invariant (A := ↥R) (G := ↥H) (H := Psub) ⟨a, a.2⟩ y).1 hy
      exact Subgroup.mem_map_of_mem H.subtype hy'
    have hSg_le_normPsubg : Sg ≤ Subgroup.normalizer (Psubg : Set G) := by
      exact sup_le Psubg.le_normalizer hR_le_normPsubg
    have hSg_le_normH : Sg ≤ Subgroup.normalizer H := by
    ...
    have hK_invS : FTIsInvariant (↥Sg) (↥H) K := by
      ...
      · intro hx
        ...
        simpa [smul_smul] using h
    letI : FTIsInvariant (↥Sg) (↥H) K := hK_invS
    let hZ_invS : FTIsInvariant (↥Sg) (↥K) Z :=
...
private theorem theorem_3_6_fixedPointSubgroup_subgroup_eq_bot_of_disjoint
    ...
    (U : Subgroup M) [FTIsInvariant A M U]
...
private theorem theorem_3_6_fixedPointSubgroup_sup_eq_bot_of_disjoint
    ...
    (U W : Subgroup M) [FTIsInvariant A M U] [FTIsInvariant A M W] [FTIsInvariant A M (U ⊔ W)]
    (hdisj : Disjoint U W)
  ...
  have hu_fix : ∀ a : A, a • u = u := by
    intro a
    have hau : a • u ∈ U := (FTIsInvariant.invariant (A := A) (G := M) (H := U) a u).1 huU
    have haw : a • w ∈ W := (FTIsInvariant.invariant (A := A) (G := M) (H := W) a w).1 hwW
...
private theorem theorem_3_6_final_contradiction
  ...
  have hK_smul_V_coe (a : K) (x : V) :
    ...
    exact
      Subgroup.conjMulDistribMulActionOfLeNormalizer_smul_coe_explicit
        (G := H) K V hKnormV a x
  let hV_invR : FTIsInvariant (↥R) (↥H) V :=
    isInvariant_of_characteristic (A := ↥R) (G := ↥H) V
  letI : FTIsInvariant (↥R) (↥H) V := hV_invR
  have hfixK_V_bot :
      fixedPointSubgroup (↥K) (↥V) = ⊥ := by
    simpa [V] using
  ...
  have hF_Kinv : ∀ Y : Ωsub, FTIsInvariant (↥K) (↥V) (F Y) := by
    intro Y
  ...
  have hK_invS : FTIsInvariant (↥Sg) (↥H) K := by
    ...
    · intro hx
      ...
      simpa [smul_smul] using h
  letI : FTIsInvariant (↥Sg) (↥H) K := hK_invS
  let hV_invS : FTIsInvariant (↥Sg) (↥H) V :=
    isInvariant_of_characteristic (A := ↥Sg) (G := ↥H) V
  letI : FTIsInvariant (↥Sg) (↥H) V := hV_invS
  ...
  have hF_restrict_inv (Y Z : Ωsub) : FTIsInvariant Y.1 (↥V) (F Z) := by
    refine ⟨?_⟩
    intro a x
    exact (hF_Kinv Z).invariant (a : K) x
  have hF_sup_inv : ∀ s : Finset Ωsub, ∀ Y : Ωsub, FTIsInvariant Y.1 (↥V) (s.sup F) := by
    intro s
    ...
    | insert i s hi ih =>
        intro Y
        letI : FTIsInvariant Y.1 (↥V) (F i) := hF_restrict_inv Y i
        letI : FTIsInvariant Y.1 (↥V) (s.sup F) := ih Y
        have hs_le_norm : s.sup F ≤ Subgroup.normalizer (F i : Set V) :=
          hsub_le_normalizer (X := F i) (Y := s.sup F)
        have hsup_inv :
            FTIsInvariant Y.1 (↥V) (F i ⊔ s.sup F) :=
  ...
  have hF_supIndep_and_fix :
    ...
    | insert i s hi ih =>
        ...
        · intro Y hYins
          ...
          letI : FTIsInvariant Y.1 (↥V) (F i) := hF_restrict_inv Y i
          letI : FTIsInvariant Y.1 (↥V) (s.sup F) := hF_sup_inv s Y
          ...
          letI : FTIsInvariant Y.1 (↥V) (F i ⊔ s.sup F) := by
            exact
  ...
  have hWV_invK (Y : Ωsub) : FTIsInvariant (↥K) (↥V) (WV Y) := by
    refine ⟨?_⟩
    intro a x
    constructor
    · intro hx
  ...
  have hWV_invS (Y : Ωsub) : FTIsInvariant (↥Sg) (↥V) (WV Y) := by
    refine ⟨?_⟩
    intro a x
    constructor
    · intro hx
  ...
  have hWV_invR (Y : Ωsub) : FTIsInvariant (↥R) (↥V) (WV Y) := by
    refine ⟨?_⟩
    intro a x
  ...
  have hWH_invR (Y : Ωsub) : FTIsInvariant (↥R) (↥H) (WH Y) := by
    letI : FTIsInvariant (↥R) (↥V) (WV Y) := hWV_invR Y
  ...
  have hR_fixed_eq_Kcomm {Y : Ωsub} (hY_fix : ∀ a : R, a • Y = Y) : Y.1 = Kcomm := by
    have hFY_invR : FTIsInvariant (↥R) (↥V) (F Y) := by
      ...
      · intro hx
        have hx' : a⁻¹ • (a • x) ∈ F Y := hforward a⁻¹ hx
        simpa [inv_smul_smul] using hx'
    letI : FTIsInvariant (↥R) (↥V) (F Y) := hFY_invR
    letI : FTIsInvariant (↥K) (↥V) (F Y) := hF_Kinv Y
  ...
  · have hp_dvd_H : p ∣ Nat.card H := by
    ...
    have hFY_invP : FTIsInvariant (↥Psub) (↥V) (F Y) := by
      ...
      · intro hx
        have hx' : a⁻¹ • (a • x) ∈ F Y := hforward a⁻¹ hx
        simpa [inv_smul_smul] using hx'
    letI : FTIsInvariant (↥Psub) (↥V) (F Y) := hFY_invP
    letI : FTIsInvariant (↥K) (↥V) (F Y) := hF_Kinv Y
    let CY : Subgroup K := actionCentralizerIn (A := ↥K) (G := ↥(F Y)) (⊤ : Subgroup K)
    have hY_le_CY : Y.1 ≤ CY := by
      intro y hy
