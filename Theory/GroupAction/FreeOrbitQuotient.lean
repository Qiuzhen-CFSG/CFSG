...
noncomputable def orbitRelQuotientSubgroupEquivProd
    ...
    : _root_.MulAction.orbitRel.Quotient H X
      ≃ _root_.MulAction.orbitRel.Quotient G X × (G ⧸ H) :=
  (_root_.MulAction.equivSubgroupOrbits X H).trans
    ((Equiv.sigmaCongrRight
        fun (omega : _root_.MulAction.orbitRel.Quotient G X) =>
          ...
          _root_.MulAction.equivSubgroupOrbitsQuotientGroup
            (x :=
              (⟨
                  omega.out,
                  _root_.MulAction.orbitRel.Quotient.mem_orbit.mpr
                    (Quotient.out_eq' omega)
                ⟩
