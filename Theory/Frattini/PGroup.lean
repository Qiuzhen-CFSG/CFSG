...
lemma commutator_le_frattini_of_isPGroup {R : Type*} [Group R] [Finite R]
  ...
  let : IsCyclic (R ⧸ M) := isCyclic_of_prime_card (α := R ⧸ M)
  ...
  · let B : Subgroup R := Subgroup.zpowers x
    ...
    have hC_coatom : IsCoatom C := by
      ...
      · intro D hCD
        ...
        have : Fact (Nat.card B).Prime := ⟨by simpa [hB_card] using (Fact.out : Nat.Prime p)⟩
        have hB_le_D : B ≤ D :=
...
theorem frattini_eq_bot_iff_isElementaryAbelian {R : Type*} [Group R] [Finite R]
  ...
  · intro hElem
    let : IsElementaryAbelian p R := hElem
...
theorem frattini_eq_closure_commutator_union_powers {R : Type*} [Group R] [Finite R]
  ...
  let : K.Normal := by
    ...
    · rw [← conj_inv]
      exact K.inv_mem ihx
  have : Fact (IsPGroup p (R ⧸ K)) := ⟨(Fact.out : IsPGroup p R).to_quotient K⟩
