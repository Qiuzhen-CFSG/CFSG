...
theorem IsExtraspecial.quotient_isPGroup (p : ℕ) (G : Type*) [Group G]
    [IsExtraspecial p G]
...
theorem IsPGroup.of_normal_qotient {p : ℕ} {G : Type*} [Group G] (N : Subgroup G)
    [hN : N.Normal] (hN : IsPGroup p N) (hQ : IsPGroup p (G ⧸ N))
    : IsPGroup p G :=
  fun g => by
