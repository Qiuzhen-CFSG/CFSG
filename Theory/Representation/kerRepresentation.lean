...
theorem kerLift'_injective : Injective (kerLift' φ) := fun a b =>
...
theorem kerRepresentation_apply (g : G) : Representation.kerRepresentation ρ (mk' _ g) = ρ g := by rfl
...
def kerRepresentationOrderIso : Subrepresentation (kerRepresentation ρ) ≃o Subrepresentation ρ := {
...
theorem kerRepresentation_irreducible_iff : IsIrreducible (kerRepresentation ρ) ↔ IsIrreducible ρ :=
  OrderIso.isSimpleOrder_iff (kerRepresentationOrderIso ρ)

instance kerRepresentation_irreducible [IsIrreducible ρ] : IsIrreducible (kerRepresentation ρ) := (kerRepresentation_irreducible_iff ρ).mpr inferInstance
