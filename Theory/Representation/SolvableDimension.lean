...
noncomputable def conj_diff_equiv
  ...
  have e' : τ ≃ₗ conjugateRep τ (g⁻¹ * x) := by
    refine e.trans ?_
    simpa [mul_assoc] using (conj_assoc_equiv (ρ := ρ) (a := g⁻¹ * x) (b := g)).symm
  let h :=
    conj_equiv_mul_left (ρ := τ) (a := g⁻¹ * x) (g := g⁻¹) e'
  let e1' : conjugateRep ρ (1 : G) ≃ₗ conjugateRep τ g⁻¹ := by
    simpa [mul_assoc] using (conj_assoc_equiv (ρ := ρ) (a := g⁻¹) (b := g)).symm
...
noncomputable def conjugateAut
  ...
  toFun h :=
    ...
    ⟩
  invFun h :=
    ⟨g * h * g⁻¹, Subgroup.Normal.conj_mem (inferInstance : H.Normal) h h.prop g⟩
  left_inv h := by
    apply Subtype.ext
    simp [mul_assoc]
  right_inv h := by
...
noncomputable def coindCosetEquiv
  ...
  · intro f₁ f₂ hfg
    ...
    · have hmem : x * g⁻¹ ∈ H := by
      ...
      have hf₁ :
        ...
        simpa [coindEval, hmem, div_eq_mul_inv, mul_assoc]
  ...
  · intro v
    refine ⟨
      ⟨coindBaseFunctionAt (ρ := ρ) g v, coindBaseFunctionAt_mem_coset (ρ := ρ) g v⟩,
      ?_
    ⟩
...
theorem coind_apply_mem_coset_shift
    ...
    (hf : f ∈ (coindCosetSubrep (ρ := ρ) q).toSubmodule)
    : coindRep (ρ := ρ) x f
      ∈ (coindCosetSubrep (ρ := ρ) (q / (x : G ⧸ H))).toSubmodule := by
  intro y hy
  change f.1 (y * x) = 0
  apply hf
  intro hEq
  ...
  · right
    ...
    have hP_unique (q' : G ⧸ H) (hq' : q' ≠ q) : P q' = 0 := by
      ...
      have : IsSimpleOrder (Subrepresentation ((coindCosetSubrep (ρ := ρ) q).toRepresentation)) := by
        simpa [q]
          using (coindCosetSubrep_irreducible (ρ := ρ) (g : G ⧸ H)
                  : IsSimpleOrder
                      (Subrepresentation
                        ((coindCosetSubrep (ρ := ρ) ((g : G ⧸ H))).toRepresentation)))
      ...
      let eNq :
        ...
        exact (show N.toRepresentation
                    ≃ₗ (coindCosetSubrep (ρ := ρ) q).toRepresentation
                from repEquivOfNeZeroOfSimple
                  (V₁ := N.toSubmodule)
...
theorem coindRep_irreducible_of_noNontrivialConj
    ...
    (hnconj
      : ∀ x : G, (x : G ⧸ H) ≠ 1 → ¬ Nonempty (ρ ≃ₗ Representation.conjugateRep ρ x))
    : IsSimpleOrder (Subrepresentation (coindRep (ρ := ρ))) := by
  classical
  let : Fintype (G ⧸ H) := Fintype.ofFinite (G ⧸ H)
  let : Nontrivial V := Subrepresentation.irreducible_module_nontrivial ρ
...
theorem coindEval_coindMapFromRepMapAux
    ...
    : coindEval (ρ := ρ) g (coindMapFromRepMapAux σ ρ π w) = π (σ g w) :=
  rfl
...
noncomputable def coindMapFromRepMapAuxOfSubrepAux
    ...
    (hchar
      : ringChar F = 0 ∨ (Nat.Prime (ringChar F) ∧ Nat.Coprime (ringChar F) (Nat.card H)))
  ...
  let : ComplementedLattice (Subrepresentation σH) := by
    exact (Representation.isSemisimpleRepresentation_iff_isSemisimpleModule_asModule
            (ρ := σH)).2
      hσHcr
  ...
  have hproj_intertwining (h : H) :
    ...
    have hproj_hw : proj ((σH h) w) = 0 := by
      simpa [proj]
        using Submodule.projectionOnto_apply_right hcompl_sub ⟨(σH h) w, hw_mem⟩
    apply Subtype.ext
    calc
      (((proj.comp (σH h)) v : M.toSubmodule) : V)
          = ((proj ((σH h) u + (σH h) w) : M.toSubmodule) : V) := by
        rw [LinearMap.comp_apply, ← huw, map_add]
      _ = ((proj ((σH h) u) : M.toSubmodule) : V)
          + ((proj ((σH h) w) : M.toSubmodule) : V) := by
        simp [map_add]
      _ = (σH h) u + 0 := by
...
theorem coindMapFromRepMapAuxOfSubrepAuxAux_eval_one
    ...
    : coindEval (ρ := M.toRepresentation) (1 : G)
        (coindMapFromRepMapAuxOfSubrepAux σ hchar M m)
      = m := by
  classical
...
set_option backward.isDefEq.respectTransparency false in
theorem semisimple_le_ker_of_forall_simple_submodule_le_ker {G : Type*} [Group G]
...
theorem irreducible_subrepresentation_of_simple_asModuleSubmodule {G : Type*} [Group G]
    {F : Type*} [Field F] {V : Type*} [AddCommGroup V] [Module F V]
    (ρ : Representation F G V) {m : Submodule (MonoidAlgebra F G) ρ.asModule}
...
noncomputable def coindEquivOfSubrep_noNontrivialConj
    ...
    (hnconj
      : ∀ x : G,
          (x : G ⧸ H) ≠ 1
          → ¬ Nonempty
                (M.toRepresentation ≃ₗ Representation.conjugateRep M.toRepresentation x))
  ...
  intro g
  ext v x
  simpa [LinearMap.comp_apply, eLin]
    using congrArg
...
noncomputable def coindPiEquiv
  ...
  invFun ψ :=
    ∑ q : G ⧸ H, coindBaseFunctionAt (ρ := ρ) (quotientSection (G := G) (H := H) q) (ψ q)
  left_inv f := by
    ext x
    calc
      (∑ q : G ⧸ H,
          coindBaseFunctionAt (ρ := ρ) (quotientSection (G := G) (H := H) q)
            (coindEval (ρ := ρ) (quotientSection (G := G) (H := H) q) f)).1
            x
    ...
    calc
      ...
      _ = ψ q := by
        ...
        · intro q' _ hq'
          ...
          simpa [g]
            using (coindEval_of_ne_coset (ρ := ρ)
                    (x := g)
...
lemma lemma_2_3_algClosed
  ...
  | h n ih =>
      ...
      · let : Nontrivial G := by
        ...
        have hx_ne_one : (x : G ⧸ H) ≠ 1 := by simpa [x, hxq] using hq0_ne
        ...
        · let shiftMap : ∀ g : G, conjugateRep σ g⁻¹ →ₗ ρH :=
            fun g => by
              refine RepMap.mk ?_ ?_
              · exact {
                  toFun := fun v => ρ g v
          ...
          have hA_disj :
              Pairwise fun q r : G ⧸ H => Disjoint (A q).toSubmodule (A r).toSubmodule :=
            by
              intro q r hqr
          ...
          have hmap_A (y : G) (q : G ⧸ H) :
              Submodule.map (ρ y) (A q).toSubmodule ≤ (A ((y : G ⧸ H) * q)).toSubmodule :=
            by
              intro w hw
              let gq : G := quotientSection (G := G) (H := H) q
              have hgq : (gq : G ⧸ H) = q := quotientSection_spec (G := G) (H := H) q
              ...
              have ha_mem : s⁻¹ * y * gq ∈ H := by
                simpa [div_eq_mul_inv, mul_assoc]
                  using Subgroup.Normal.conj_mem (inferInstance : H.Normal) ((y * gq) / s)
                    hdiv s⁻¹
          ...
          have hA_indep : iSupIndep (fun q : G ⧸ H => (A q).toSubmodule) := by
            ...
            have hA_disjoint_biSup :
              ...
              · let : Module F[H] (Representation.asModule ρH) :=
                ...
                have hUmod_eq : U.asSubmodule = Umod := by
                  calc
                    U.asSubmodule
                        = (Subrepresentation.subrepresentationSubmoduleOrderIso (ρ := ρH))
                            U :=
                      rfl
                  ...
                  simpa [sSet, Umod]
                    using (sSup_range
                            (f :=
                              fun r : s =>
                                ((A r).asSubmodule
                                  : Submodule F[H] (Representation.asModule ρH))))
                ...
                have heqA : (A q).toRepresentation ≃ₗ (A r).toRepresentation := by
                  ...
                  calc
                    ρH.asModuleEquiv ↑(eS (((A q).toRepresentation h) v))
                        = ρH.asModuleEquiv
                            ↑(eS ((MonoidAlgebra.single h (1 : F)) • v')) := by
                      have hv' :
                    ...
                    _ = ρH.asModuleEquiv
                          ↑(((MonoidAlgebra.single h (1 : F)) • eS v'
                              : ((A r).asSubmodule
                                  : Submodule F[H] (Representation.asModule ρH)))) := by
            ...
            · intro q s hq ih v hv hv0 i hi
              ...
              have hvq_zero : v q = 0 := by
                ...
                have : v q ∈ (⊥ : Submodule F W) := by simpa [hdisj.eq_bot] using hmem
                simpa using this
          ...
          have hA_internal : DirectSum.IsInternal (fun q : G ⧸ H => (A q).toSubmodule) :=
            by
              refine (DirectSum.isInternal_submodule_iff_iSupIndep_and_iSup_eq_top
                        (R := F) (ι := G ⧸ H) (M := W)
                        (A := fun q : G ⧸ H => (A q).toSubmodule)).2
                ?_
          ...
          have hfin :
            ...
            calc
              Module.finrank F W = Module.finrank F (⨁ q : G ⧸ H, (A q).toSubmodule) := by
                symm
