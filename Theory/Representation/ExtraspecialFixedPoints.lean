...
theorem invariants_eq_bot_iff_fixedVectors_eq_zero
  ...
  · intro hfix
    ...
    intro v hv
    have hv' : ∀ h : H, ρ h v = v := by simpa [Representation.mem_invariants] using hv
  ...
  have hsfix :
      ∀ s : Finset (Submodule (MonoidAlgebra F G) ρ.asModule),
        ↑s ⊆ {m : Submodule (MonoidAlgebra F G) ρ.asModule |
          IsSimpleModule (MonoidAlgebra F G) m} →
        ∀ x : ρ.asModule, x ∈ ⨆ m ∈ s, m → ρ h (ρ.asModuleEquiv x) = ρ.asModuleEquiv x :=
    by
      intro s hs' x hx
      ...
      | @insert q t hqt ih =>
          ...
          have hx' : x ∈ q ⊔ ⨆ m ∈ t, m := by simpa [hqt] using hx
          ...
          calc
            ρ h (ρ.asModuleEquiv (xq + xt))
                = ρ h (ρ.asModuleEquiv xq + ρ.asModuleEquiv xt) := by
              simp
...
set_option backward.isDefEq.respectTransparency false in
theorem exists_simple_submodule_nontrivial_of_not_le_ker {G : Type*} [Group G] {F : Type*}
    [Field F] {V : Type*} [AddCommGroup V] [Module F V] (ρ : Representation F G V)
    [IsSemisimpleModule (MonoidAlgebra F G) ρ.asModule] (H : Subgroup G)
    (hH : ¬ H ≤ ρ.ker)
    : ∃ m : Submodule (MonoidAlgebra F G) ρ.asModule,
...
theorem invariants_ofSubmodule'_eq_bot_of_invariants_eq_bot {G : Type*} [Group G]
    ...
    : Representation.invariants
        ((Subrepresentation.ofSubmodule' m).toRepresentation.comp R.subtype)
      = ⊥ := by
  rw [Submodule.eq_bot_iff]
  intro v hv
  apply Subtype.ext
...
theorem irreducible_of_ofSubmodule'_simple {G : Type*} [Group G] {F : Type*} [Field F]
    ...
    : Representation.IsIrreducible
        (Subrepresentation.ofSubmodule' m).toRepresentation := by
...
theorem center_le_of_normal_ne_bot_of_commutator_le_center_of_prime_center_local
  ...
  · let C : Subgroup K := ⁅N, (⊤ : Subgroup K)⁆
    ...
    have hC_le_center : C ≤ Subgroup.center K := by
      exact (Subgroup.commutator_mono (show N ≤ (⊤ : Subgroup K) by exact le_top)
              le_rfl).trans
        hcomm
    have hC_ne_bot : C ≠ ⊥ := by
      intro hC_eq_bot
...
theorem span_range_eq_top_of_irreducible_isAlgClosed
  ...
  have hspan :
      Subalgebra.toSubmodule (Algebra.adjoin F (Set.range (ρ : G → Module.End F V))) =
        Submodule.span F (Set.range (ρ : G → Module.End F V)) := by
    simpa [MonoidHom.mclosure_range (ρ : G →* Module.End F V), MonoidHom.coe_mrange]
      using (Algebra.adjoin_eq_span (R := F) (s := Set.range (ρ : G → Module.End F V)))
  calc
    Submodule.span F (Set.range (ρ : G → Module.End F V))
        = Subalgebra.toSubmodule
            (Algebra.adjoin F (Set.range (ρ : G → Module.End F V))) := by
...
theorem trace_eq_zero_of_not_mem_center_of_faithful_irreducible_isExtraspecial
  ...
  have hρ_conj :
    ...
    calc
      (ρ (y * x * y⁻¹) : Module.End F V)
        ...
        simp
      _ = ((ρ y : Module.End F V) * (ρ x : Module.End F V))
          * (ρ y⁻¹ : Module.End F V) := by
        simp
      _ = (ρ y : Module.End F V)
          * ((ρ x : Module.End F V) * (ρ y⁻¹ : Module.End F V)) := by
        rw [mul_assoc]
  have hρ_zx :
      (ρ (y * x * y⁻¹) : Module.End F V) =
  ...
  have htrace_conj :
    ...
    calc
      ...
      _ = LinearMap.trace F V
            (((ρ x : Module.End F V) * (ρ y⁻¹ : Module.End F V))
              * (ρ y : Module.End F V)) := by
        exact
  ...
  have ht_eq : t = a * t := by simpa [t] using htrace_conj.symm.trans htrace_smul
...
theorem finrank_eq_primePow_of_faithful_irreducible_isExtraspecial
  ...
  have hspan_b : Submodule.span F (Set.range b) = ⊤ := by
    ...
    have htop_le : (⊤ : Submodule F (Module.End F V)) ≤ Submodule.span F (Set.range b) :=
      by
        rw [← hspan_rho]
  ...
  have hl_ker : ∀ c : Q →₀ F, l c = 0 → c = 0 := by
    ...
    have htq_b (r : Q) : tq (b r) = if r = s then (Module.finrank F V : F) else 0 := by
      ...
      · subst r
        have htrace_eq : tq (b s) = (Module.finrank F V : F) := by
          ...
          calc
            LinearMap.trace F V
                  ((ρ (repQ s) : Module.End F V) * (ρ ((repQ s)⁻¹) : Module.End F V))
                = LinearMap.trace F V
                    (((ρ ((repQ s)⁻¹) : Module.End F V)
                      * (ρ (repQ s) : Module.End F V))) := by
              exact
                LinearMap.trace_mul_comm
                  (R := F)
      ...
      · have hnotcenter : repQ r * (repQ s)⁻¹ ∉ Subgroup.center K := by
        ...
        have htrace_eq : tq (b r) = 0 := by
          calc
            LinearMap.trace F V
                  ((ρ (repQ r) : Module.End F V) * (ρ ((repQ s)⁻¹) : Module.End F V))
    ...
    have hsum :
      ...
      · have hcs : c s = 0 := by simpa [Finsupp.mem_support_iff] using hs
    ...
    have hzero : tq (l c) = 0 := by
      simp [hc0]
    have hcoeff : c s * (Module.finrank F V : F) = 0 := by simpa [hcalc] using hzero
  ...
  have hcardQ : Nat.card Q = q ^ (2 * n) := by
    have hmul : Nat.card K = Nat.card Q * Nat.card (Subgroup.center K) := by
      simpa [Q]
        using (Subgroup.card_eq_card_quotient_mul_card_subgroup (Subgroup.center K))
    have hcenter : Nat.card (Subgroup.center K) = q := IsExtraspecial.center_order_p q K
    have hmul' : q ^ (2 * n + 1) = Nat.card Q * q := by simpa [hcard, hcenter] using hmul
...
theorem theorem_2_5_exists_generator {H : Type*} [Group H] [IsCyclic H]
...
theorem theorem_2_5_exists_faithful_irreducible
    ...
    : ∃ (V : Type (max uP uF)) (_ : AddCommGroup V) (_ : Module F V)
        (_ : FiniteDimensional F V) (ρ : Representation F P V),
        IsIrreducible ρ ∧ Function.Injective ρ := by
  classical
...
noncomputable def theorem_2_5_quotientKerRightHomEquiv
    ...
    {H : Type*} [Group H]
    {φ : H →* MulAut P}
    : (SemidirectProduct P H φ
        ⧸ (SemidirectProduct.rightHom (N := P) (G := H) (φ := φ)).ker)
      ≃* H :=
...
theorem theorem_2_5_exists_extension
    ...
    : ∃ σ : Representation F (SemidirectProduct P H φ) V,
        σ.comp SemidirectProduct.inl = ρ := by
  classical
  let _ := hH
  let _ := hh
  ...
  have hcycK : IsCyclic (G ⧸ K) := by
    let eQ : G ⧸ K ≃* H := by
      let hK :
        ...
        simpa [G, K]
          using (SemidirectProduct.range_inl_eq_ker_rightHom (N := P) (G := H) (φ := φ))
  ...
  have E : ∀ g : G, ρK ≃ₗ conjugateRep ρK g := by
    ...
    have hcenter :
      ...
      intro z hz
      have hzcomm : ∀ y : P, y * z = z * y := by simpa [Subgroup.mem_center_iff] using hz
      have hαgz : αg z = z := by
...
theorem theorem_2_5_generator_linearEquiv
    ...
    : ∃ g : V ≃ₗ[F] V,
        (g : Module.End F V) = σ (SemidirectProduct.inr x) ∧ g ^ h = 1 := by
  have hxord : orderOf x = h := by
    simpa [hH] using orderOf_eq_card_of_forall_mem_zpowers hxgen
  have hxpow : x ^ h = 1 := by simpa [hxord] using pow_orderOf_eq_one x
  ...
  have hxinr_pow : ((SemidirectProduct.inr x : SemidirectProduct P H φ) ^ h) = 1 := by
    calc
      ((SemidirectProduct.inr x : SemidirectProduct P H φ) ^ h)
        ...
        simp
      _ = 1 := by simp [hxpow]
  have hu_pow : u ^ h = 1 := by simpa [u] using congrArg σ.asGroupHom hxinr_pow
...
theorem theorem_2_5_hE_phiQ_fixed_eq_one
    ...
    (hq
      : theorem_2_5_hE_phiQ
          (P := P) (H := H) (φ := φ)
          (semidirectProduct_center_fixed (p := p) (P := P) (H := H) (φ := φ)
            hcentralizer) y q
        = q)
    : q = 1 := by
  ...
  have hyh : y ^ h = 1 := by simpa [hH] using (pow_card_eq_one' (x := y))
  have hzh : (z : P) ^ h = 1 := by
    have htop := hpow h
  ...
  have hp0_center : p0 ∈ Subgroup.center P := by
    have hp0_fix : p0 ∈ {p : P | φ y p = p} := by simpa [p0] using hyfix
...
theorem theorem_2_5_hE_char_not_dvd_card
  ...
  have hchar_ne_zero : ringChar F ≠ 0 := by
    intro hzero
    have : Nat.card P = 0 := by simpa [hzero] using hcharP
  ...
  have hchar_dvd_p : ringChar F ∣ p := by
    have : ringChar F ∣ p ^ (2 * n + 1) := by simpa [hp] using hcharP
  ...
  let f : ρ.IntertwiningMap ρ := Representation.IntertwiningMap.centralMul ρ (z : P) <| by
    rw [Submonoid.mem_center_iff]
    intro g
  ...
  have hz_dvd : orderOf (z : P) ∣ p := by
    simpa [IsExtraspecial.center_order_p p P]
      using (Subgroup.orderOf_dvd_natCard (Subgroup.center P) z.2)
  have hz_order : orderOf (z : P) = p := by
    ...
    · exact hpz
  have hzpow : (z : P) ^ p = 1 := by simpa [hz_order] using pow_orderOf_eq_one (z : P)
  ...
  have ha_eq_one : a = 1 := by
    have hpowchar : a ^ ringChar F = 1 := by simpa [hchar_eq_p] using hapow
...
theorem theorem_2_5_hE
    ...
    have htop_le : (⊤ : Submodule F (Module.End F V)) ≤ Submodule.span F (Set.range b) :=
      by
        ...
        have hrho :
          ...
          calc
            (ρ p0 : Module.End F V)
                = (ρ ((p0 / repQ q0) * repQ q0) : Module.End F V) := by
              congr 1
              simp [div_eq_mul_inv, mul_assoc]
  ...
  have hS_single (y : H) (q : Q) :
    ...
    have hT :
      ...
      calc
        T y (b q) = τ y ∘ₗ b q ∘ₗ τ y⁻¹ := by
          simp [T, τ, Representation.linHom_apply]
        _ = τ y ∘ₗ (ρ (repQ q) : Module.End F V) ∘ₗ τ y⁻¹ := by
          rfl
        _ = (σ (SemidirectProduct.inr y) : Module.End F V)
            ∘ₗ (ρ (repQ q) : Module.End F V)
            ∘ₗ (σ (SemidirectProduct.inr y)⁻¹ : Module.End F V) := by
          simp [τ]
        _ = (σ (SemidirectProduct.inr y) : Module.End F V)
            ∘ₗ σ (SemidirectProduct.inl (repQ q))
            ∘ₗ (σ (SemidirectProduct.inr y)⁻¹ : Module.End F V) := by
          rw [← hσq]
        _ = σ
              (SemidirectProduct.inr y
                * SemidirectProduct.inl (repQ q)
        ...
        · have hrepQ' : repQ q = Quotient.out q := by
          ...
          calc
            ...
            _ = (((φ y (Quotient.out q) : P) : Q)) := by
              change (QuotientGroup.map (Subgroup.center P) (Subgroup.center P) (φ y)
                        (by
                          intro z hz
                          have hzfixed : φ y z = z := hcenter_fixed y ⟨z, hz⟩
                          simpa [hzfixed] using hz)
    ...
    have hρ :
      ...
      calc
        (ρ (φ y (repQ q)) : Module.End F V)
            = (ρ ((φ y (repQ q) / repQ ((φQ y) q)) * repQ ((φQ y) q))
                : Module.End F V) := by
          congr 1
          simp [div_eq_mul_inv, mul_assoc]
    ...
    have hmain :
      ...
      apply e.injective
      calc
        e ((e.symm.conj (T y)) (Finsupp.single q (1 : F)))
            = T y (e (Finsupp.single q (1 : F))) := by
      ...
      have hρ_ne_zero : (ρ (φ y (repQ q)) : Module.End F V) ≠ 0 := by
        ...
        have hone : (1 : Module.End F V) = 0 := by
          calc
            (1 : Module.End F V)
                = (ρ ((φ y (repQ q))⁻¹) : Module.End F V)
                  * (ρ (φ y (repQ q)) : Module.End F V) := by
              simpa using
  ...
  have hS_one : S (Finsupp.single (1 : Q) (1 : F)) = Finsupp.single (1 : Q) 1 := by
    apply e.injective
    calc
      e (S (Finsupp.single (1 : Q) (1 : F)))
          = T x (e (Finsupp.single (1 : Q) (1 : F))) := by
  ...
  have ha_pow_ne_fix_of_ne_one {q : Q} (hq : q ≠ 1) {k : ℕ} (hk0 : 0 < k) (hkh : k < h) :
    ...
    have hfix' : φQ (x ^ k) q = q := by simpa [ha_apply_pow] using hfix
    exact hq <|
  ...
  have hcard_orbit_r (ω : Ω') :
    ...
    calc
      Nat.card (MulAction.orbit (Subgroup.zpowers a) (r ω))
          = Fintype.card (MulAction.orbit (Subgroup.zpowers a) (r ω)) := by
        simp
      _ = Function.minimalPeriod ((a) • ·) (r ω) := by
        symm
  ...
  have hcard_orbit_one :
    ...
    calc
      Nat.card (MulAction.orbit (Subgroup.zpowers a) (1 : Q))
          = Fintype.card (MulAction.orbit (Subgroup.zpowers a) (1 : Q)) := by
        simp
      _ = Function.minimalPeriod ((a) • ·) (1 : Q) := by
        symm
  ...
  have horbit_eq_of_pow_eq {ω ω' : Ω'} (hneq : ω ≠ ω') {k : ℕ} (hk : k < h) :
    ...
    calc
      ...
      _ = Quotient.mk'' (((a ^ k) (r ω)) : Q) := by
        ...
        <| (MulAction.mem_orbit_iff).2
          ⟨
            (⟨a, Subgroup.mem_zpowers a⟩ ^ k)⁻¹,
            by
              rw [inv_smul_eq_iff]
  ...
  have horbitVec_mem (ω : Ω') (i : Fin h) :
    ...
    have hcoeff0 :
      ...
      calc
        ...
        _ = ε ^ (((i : ℕ) * h : ℕ) : ℤ) := by
          ...
          exact_mod_cast this
        _ = (ε ^ h) ^ (i : ℕ) := by
          rw [zpow_natCast, pow_mul]; exact pow_right_comm ε (↑i) h
    ...
    calc
      ...
      _ = (∑ n ∈ Finset.range (h - 1),
        ...
        calc
          ∑ n ∈ Finset.range h, (ε ^ (((i : ℕ) * (h - 1 - n) : ℕ) : ℤ)) • S ((S ^ n) δ)
              = ∑ n ∈ Finset.range ((h - 1) + 1),
                  (ε ^ (((i : ℕ) * (h - 1 - n) : ℕ) : ℤ)) • S ((S ^ n) δ) := by
            rw [hh_succ]
          _ = (ε ^ (((i : ℕ) * (h - 1 - (h - 1)) : ℕ) : ℤ)) • S ((S ^ (h - 1)) δ)
              + ∑ n ∈ Finset.range (h - 1),
                  (ε ^ (((i : ℕ) * (h - 1 - n) : ℕ) : ℤ)) • S ((S ^ n) δ) := by
            simpa using
  ...
  have hlower_zero : Nat.card Ω' + 1 ≤ Module.finrank F (A 0) := by
    ...
    let L0E : (Option Ω' →₀ F) →ₗ[F] A 0 := L0.codRestrict (A 0) <| by
      intro f
      rw [Finsupp.linearCombination_apply]
  ...
  have hcard_nonzero : Fintype.card {i : Fin h // i ≠ 0} = h - 1 := by
    calc
      Fintype.card {i : Fin h // i ≠ 0}
          = Fintype.card (Fin h) - Fintype.card {i : Fin h // i = 0} := by
        simp
      _ = h - 1 := by simp
  ...
  have hzero_eq : Module.finrank F (A 0) = N + 1 := by
    have hzero_lower : N + 1 ≤ Module.finrank F (A 0) := by simpa [N] using hlower_zero
    rw [hsum0] at hsumA
  ...
  have hfinrank_eq (ii : Fin h) (hii : ii ≠ 0) :
    ...
    have hcard_rest : Fintype.card {j : B // j ≠ j0} = h - 2 := by
      calc
        Fintype.card {j : B // j ≠ j0}
            = Fintype.card B - Fintype.card {j : B // j = j0} := by
          simp
      ...
      have hsum_rest :
        ...
        calc
          ...
          _ ≤ ∑ j : {j : B // j ≠ j0}, Module.finrank F (A j.1.1) := by
            ...
            exact Finset.sum_le_sum (s := (Finset.univ : Finset {j : B // j ≠ j0}))
              (fun j _ =>
                hlower_nonzero j.1.1 <| by
                  intro hj0
                  exact j.2 <| Subtype.ext <| by simpa [j0] using hj0)
      have hzero_ge : N + 1 ≤ Module.finrank F (A 0) := by simpa [N] using hzero_eq.ge
  ...
  have hii_ne_zero : ii ≠ 0 := by
    ...
    have htoNat : (m % h).toNat = 0 := by simpa [hii0] using hval.symm
  ...
  have hAii :
    ...
    simpa [A, ii, gS]
      using eigenspace_eq_intCast (hh := hh2) (g := gS) (h := h) (hε := hε) m
  have hmainA :
      Module.finrank F (A 0) = Module.finrank F (Module.End.eigenspace S (ε ^ m)) + 1 :=
    by
      calc
  ...
  calc
    Module.finrank F (intertwiningSubmodule (τ x) (ε ^ (0 : ℤ) • τ x))
        = Module.finrank F (Module.End.eigenspace S (ε ^ (0 : ℤ))) :=
      hfinrank_intertwine 0
...
theorem theorem_2_5_a {p : ℕ} [hp : Fact p.Prime] {n : ℕ} {P : Type*} [Group P]
    [IsExtraspecial p P] (hp : Nat.card P = p ^ (2 * n + 1)) {h : ℕ} {H : Type*} [Group H]
    [IsCyclic H] (hH : Nat.card H = h) (hh : Nat.Coprime h p) {φ : H →* MulAut P}
    (hcentralizer : ∀ x : H, x ≠ 1 → {p | φ x p = p} = Subgroup.center P)
    : (h ∣ p ^ n + 1) ∨ (h ∣ p ^ n - 1) := by
  classical
  have hpprime : p.Prime := Fact.out
...
theorem theorem_2_5_b_core
  ...
  have hfixR' :
    ...
    have hfixRset' : {w : V' | ∀ r : R, (ρ'.comp R.subtype) r w = w} = ({0} : Set V') := by
      ...
      · intro hw
        ...
        intro r
        simp [hw0]
    exact (invariants_eq_bot_iff_fixedVectors_eq_zero (ρ := ρ') (H := R)).2 hfixRset'
  ...
  have hτPirr : IsIrreducible τP := by
    ...
    have hNPirr : IsIrreducible NP.toRepresentation := by
      simpa [NP]
        using (irreducible_of_ofSubmodule'_simple (ρ := τP) hmPSimple
    ...
    have hNKirr : Representation.IsIrreducible NK.toRepresentation := by
      refine (RepEquiv.irreducible_iff_group_iso
                (ρ := NP.toRepresentation) (σ := NK.toRepresentation) eK ?_).1
        hNPirr
      intro q v
      rfl
    ...
    have hσNK : σNK = NK.toRepresentation := by
      ...
      exact hq'
    have hσNKirr : Representation.IsIrreducible σNK := by simpa [hσNK] using hNKirr
    ...
    have hτKirr : IsIrreducible τK := by
      have fNK : σNK ≃ₗ NK.toRepresentation := by simpa [hσNK] using (RepEquiv.refl σNK)
      have eτK : τK ≃ₗ σNK :=
        proposition_2_2_a K hcycK σNK ENK τ NK fNK
      exact (RepEquiv.irreducible_euqiv (f := eτK)).2 hσNKirr
    exact (RepEquiv.irreducible_iff_group_iso (ρ := τP) (σ := τK) eK (by
      intro q v
      rfl)).2 hτKirr
  ...
  have hcharH' : ¬ ringChar F' ∣ h := by simpa [F', Algebra.ringChar_eq F F'] using hcharH
  have hcharP' : ¬ ringChar F' ∣ Nat.card P := by
    simpa [F', Algebra.ringChar_eq F F'] using hcharP
  obtain ⟨ε, hε⟩ := exists_primitiveRoot_of_isAlgClosed_not_dvd (F := F') hcharH'
  obtain ⟨g, hg, hgpow⟩ :=
  ...
  have hE :
    ...
    have hfin (k : ℤ) :
        Module.finrank F'
          ...
          Module.finrank F'
            ↥(intertwiningSubmodule g.toLinearMap (ε ^ k • g.toLinearMap)) := by
      simpa only [hIntertwine k]
        using congrArg
          (fun N : Submodule F' (_root_.Module.End F' ↥M.toSubmodule) =>
            Module.finrank F' ↥N)
          (hIntertwine k)
    calc
      Module.finrank F'
      ...
      _ = Module.finrank F'
            ↥(intertwiningSubmodule (τ (SemidirectProduct.inr x))
                (ε ^ m • τ (SemidirectProduct.inr x)))
          + 1 :=
        hEτ
      _ = Module.finrank F' ↥(intertwiningSubmodule g.toLinearMap (ε ^ m • g.toLinearMap))
          + 1 := by
        rw [hfin m]
  ...
  · let E0 : Submodule F' ↥M.toSubmodule :=
    ...
    have hE0pos : 0 < Module.finrank F' E0 := by simpa [E0] using hpos
    ...
    have hgR : ∀ r : R, r ∈ Subgroup.zpowers gR := by
      ...
      apply Subtype.ext
      simpa [gR, hy, hm]
        using (MonoidHom.map_zpow (SemidirectProduct.inr : H →* G) x m).symm
...
theorem theorem_2_5_b {p : ℕ} [hp : Fact p.Prime] {n : ℕ} {P : Type*} [Group P]
    [IsExtraspecial p P] (hp : Nat.card P = p ^ (2 * n + 1)) {h : ℕ} {H : Type*} [Group H]
    [IsCyclic H] (hH : Nat.card H = h) (hh : Nat.Coprime h p) {φ : H →* MulAut P}
    (hcentralizer : ∀ x : H, x ≠ 1 → {p | φ x p = p} = Subgroup.center P) {F : Type*}
    [Field F] (hc : ¬ (ringChar F) ∣ h * p ^ (2 * n + 1)) (hhne : h ≠ p ^ n + 1)
    {V : Type*} [AddCommGroup V] [Module F V] [FiniteDimensional F V]
    {ρ : Representation F (SemidirectProduct P H φ) V} [IsIrreducible ρ]
    (hfaithful : Function.Injective ρ)
    : {v | ∀ h : H, ρ ⟨1, h⟩ v = v} ≠ {0} := by
  ...
  have hcharP : ¬ ringChar F ∣ Nat.card P := by
    intro hdivP
    apply hc
    simpa [hp, Nat.mul_comm, Nat.mul_left_comm, Nat.mul_assoc]
      using dvd_mul_of_dvd_right hdivP h
  exact
    theorem_2_5_b_core
      (hp := hp) (hH := hH) (hh := hh) (φ := φ) hcentralizer
