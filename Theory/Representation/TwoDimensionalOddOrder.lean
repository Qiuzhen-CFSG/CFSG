...
lemma top_eq_sylow_zero {G : Type*} [Group G] {p : ℕ} (hp : p = 0) (C : Sylow p G)
    : ⊤ = C.toSubgroup :=
...
lemma ringChar_prime {F : Type*} [Field F] (hc : ¬ ringChar F = 0)
...
lemma card_odd_finite {G : Type*} (ho : Odd (Nat.card G)) : Finite G :=
  Nat.finite_of_card_ne_zero (Nat.ne_of_odd_add ho)

lemma covby_top_of_index_eq_prime {p : ℕ} [Fact p.Prime] {G : Type*} [Group G] [Finite G]
...
lemma pGroup_fix_nonzero_vector
  ...
  exact Exists.casesOn (this n ⊤ (hn.symm ▸ Subgroup.card_top))
    fun w h ↦
  ...
  have hconj :
    ...
    calc
      _ = (⟨(ρ x) w, ψ.apply_mem_toSubmodule x w.2⟩ : ψ.toSubmodule) := by
        simpa [eQ]
          using (Submodule.quotientEquivOfIsCompl_apply_mk_right
  ...
  calc
    LinearMap.det (ρ x)
        = LinearMap.det ((ρ x).restrict (φ.apply_mem_toSubmodule x))
          * LinearMap.det
              (Submodule.mapQ φ.toSubmodule φ.toSubmodule (ρ x)
                (φ.apply_mem_toSubmodule x)) := by
...
lemma exists_central_element_orderOf_eq_prime_of_nontrivial_pgroup_subgroup
  ...
  · have hzpow : z ^ q = 1 := by simpa [hzq] using pow_orderOf_eq_one z
...
theorem theorem_2_6_b
  ...
  intro n hc
  induction n using Nat.strong_induction_on generalizing G V with
  | h n ih =>
      ...
      · rw [add_assoc, one_add_one_eq_two] at hc ih
            ...
            | h m ihN =>
                ...
                · have hNcard_ne_zero : Nat.card N ≠ 0 := Nat.card_pos.ne'
                  ...
                  · let H : Subgroup N := Subgroup.normalizer (Q : Set N)
                    ...
                    have hH_ne_top : H ≠ ⊤ := by simpa [H] using hnormQ
                    ...
                    have hQH_p : IsPGroup q QH := by
                      simpa [QH]
                        using Q.isPGroup'.of_equiv
                          (Subgroup.subgroupOfEquivOfLe (H := (Q : Subgroup N)) (K := H)
                            hQ_le_H).symm
                    have hQH_center : QH ≤ Subgroup.center H := by
                      ...
                      · have : Fact (Nat.Prime (ringChar F)) := ringChar_prime hchar0
                        ...
                        have hxy_left : ⁅x, y⁆ ∈ QH := by
                          exact (Subgroup.commutator_le_left (H₁ := QH)
                                  (H₂ := (⊤ : Subgroup H)))
                        ...
                        have hxy_bot : ⁅x, y⁆ ∈ (⊥ : Subgroup H) := by
                          simpa [hQH_disj_C.eq_bot]
                            using show ⁅x, y⁆ ∈ QH ⊓ (C : Subgroup H) from ⟨
                              hxy_left,
                              hxy_right
                            ⟩
                    ...
                    have hM'_ne_bot : M' ≠ ⊥ := by
                      intro hM'_bot
                      apply hM_ne_bot
                      exact (Subgroup.map_eq_bot_iff_of_injective (H := M)
                              (f := N.subtype) N.subtype_injective).mp
                        hM'_bot
                    have hcardM'_eq : Nat.card M' = Nat.card M := by
                      simpa [M']
                        using (Subgroup.card_map_of_injective (K := M) (f := N.subtype)
                                N.subtype_injective)
            ...
            conj_mem := by
              ...
              · intro h hh
                ...
                calc
                  ...
                  _ = g * ((g⁻¹ * h * g) * k) := by
                    rw [hk.2.1 _ (Subgroup.Normal.conj_mem' hqC h hh g)]
                  _ = _ := by group
        ...
        · have : Fact q.Prime := ⟨hq1⟩
          ...
          have hcomm_le_ker_U : commutator G ≤ rhoU.ker := by
            ...
            calc
              rhoU (g1 * g2 * g1⁻¹ * g2⁻¹)
                  = rhoU g1 * (rhoU g2 * rhoU g1⁻¹) * rhoU g2⁻¹ := by
          ...
          have hcomm_le_ker_Q : commutator G ≤ rhoQ.ker := by
            ...
            calc
              rhoQ (g1 * g2 * g1⁻¹ * g2⁻¹)
                  = rhoQ g1 * (rhoQ g2 * rhoQ g1⁻¹) * rhoQ g2⁻¹ := by
                simp [map_mul, mul_assoc]
            ...
            have hsub : (ρ' c) ^ q - 1 = 0 := by
              calc
                (ρ' c) ^ q - 1 = e ^ q := by
                  symm
                  simpa [e]
                    using (sub_pow_expChar_of_commute (R := Module.End F' V') (p := q)
          ...
          have hcomm_rho_C : ∀ y z : C, ρ' (y * z) = ρ' (z * y) := by
            intro y z
            apply linearMap_eq_of_isCompl_of_eq_on_each (e := ρ' (y * z))
              (f := ρ' (z * y)) (φ := U) (ψ := W) hcomplUW
            · intro u
              calc
        ...
        · have : Fact q.Prime := ⟨hq1⟩
          ...
          have hab : a ≠ b := by
            ...
            have hsq : a ^ 2 = 1 := by simpa [habEq] using hsq'
          ...
          have hU1 : Module.finrank F' U = 1 := by simpa [U] using hφ1
          have hW1 : Module.finrank F' W = 1 := by simpa [W] using hψ1
          have hcomplUW : IsCompl U W := by simpa [U, W] using hcompl_sub
          have hUx : ∀ u : U, ρ' x u = a • u := by simpa [U] using hφx
          have hWx : ∀ w : W, ρ' x w = b • w := by simpa [W] using hψx
          have hU_ne_bot : U ≠ ⊥ := by
            intro hUbot
            ...
            · have hWgU : W.map (ρ' g) = U := by
                ...
                · exfalso
                  have htopW : (⊤ : Submodule F' V') = W := by
                    calc
                      ⊤ = (⊤ : Submodule F' V').map (ρ' g) := by
                        simpa using (hmap_top g).symm
          ...
          have hcommρ : ∀ g h : G, ρ' (g * h) = ρ' (h * g) := by
            ...
            apply linearMap_eq_of_isCompl_of_eq_on_each (e := ρ' (g * h))
              (f := ρ' (h * g)) (φ := U) (ψ := W) hcomplUW
            · intro u
