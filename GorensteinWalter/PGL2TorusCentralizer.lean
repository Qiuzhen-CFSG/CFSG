...
/-- The standard split torus involution has dihedral normalizer: its
centralizer is `U ⊔ ⟨w⟩`. -/
public theorem pgl2_split_torus_centralizer_data
  ...
  have hC_le : Subgroup.centralizer ({s} : Set (PGL2 K)) ≤
    ...
    have hmul : Matrix.ProjGenLinGroup.mk (A * diagHom (-1 : Kˣ)) =
      ...
      simpa [s, torus, map_mul] using hcomm
  ...
  have hcross : Odd (Nat.card K) →
    ...
    simpa [diagHom, diagGL, Matrix.det_diagonal, Fin.prod_univ_two, k]
      using hsq
  exact ⟨U, s, w, hUcyclic, hUcard, hsU, hssq, hsne, hw_not_mem, hw_sq,
    (by intro x hx; rcases hx with ⟨k, rfl⟩; exact hweyl_torus k),
    hC_le.antisymm hC_ge, hcross⟩
...
private lemma char_ne_two_of_odd_card (K : Type u) [Field K] [Finite K]
    (hodd : Odd (Nat.card K)) : ringChar K ≠ 2 := by
  let _ : Fintype K := Fintype.ofFinite K
  intro h2
  have heven : Fintype.card K % 2 = 0 :=
    (FiniteField.even_card_iff_char_two (F := K)).mp h2
  have hodd' : Odd (Fintype.card K) := by
    simpa [Nat.card_eq_fintype_card] using hodd
  exact hodd'.not_two_dvd_nat (Nat.dvd_of_mod_eq_zero heven)
...
centralizer is `U ⊔ ⟨w⟩`. -/
public theorem pgl2_nonsplit_torus_centralizer_data
    (K : Type u) [Field K] [Finite K] (hodd : Odd (Nat.card K)) :
    ∃ U : Subgroup (PGL2 K), ∃ s w : PGL2 K,
      IsCyclic U ∧ Nat.card U = Nat.card K + 1 ∧
      s ∈ U ∧ s * s = 1 ∧ s ≠ 1 ∧
  ...
  classical
  let _ : Fintype K := Fintype.ofFinite K
  have hchar : ringChar K ≠ 2 := char_ne_two_of_odd_card K hodd
  obtain ⟨lam, hlam⟩ := FiniteField.exists_nonsquare hchar
  have hlam0 : lam ≠ 0 := by
    intro h
    subst h
    exact hlam IsSquare.zero
  let _ : Fact (Irreducible (X ^ 2 - C lam : K[X])) := ⟨
    (X_pow_sub_C_irreducible_iff_of_prime (K := K) Nat.prime_two).2 (by
      intro b hb
      exact hlam ⟨b, by simpa [pow_two] using hb.symm⟩)⟩
  let f : K[X] := X ^ 2 - C lam
  let E : Type u := AdjoinRoot f
  let _ : Field E := inferInstance
  ...
  let U : Subgroup (PGL2 K) := torus.range
  let _ : Fintype E := Fintype.ofEquiv (Fin 2 → K) b2.equivFun.toEquiv.symm
  let _ : Finite E := inferInstance
  have hUcyclic : IsCyclic U := by
  ...
  have hw_not_mem : w ∉ U := by
    ...
    have h01' : (mulGL x : Matrix (Fin 2) (Fin 2) K) 0 1 = 0 := by
      rw [← ha]
      simp [wGL, Matrix.GeneralLinearGroup.scalar, Matrix.mul_apply,
        Fin.sum_univ_two]
  ...
  have hC_le : Subgroup.centralizer ({s} : Set (PGL2 K)) ≤
    ...
    have hμsq_c : c * (μ : K) ^ 2 = c := by
      ...
      calc
        ...
        _ = b * (μ : K) := by rw [h11]
        _ = c * lam := by simpa [mul_comm] using h00
  ...
  have hcross : Odd (Nat.card K) →
    ...
    exact hsq
  exact ⟨U, s, w, hUcyclic, hrange_card, hsU, hssq, hsne, hw_not_mem, hw_sq,
    (by intro x hx; rcases hx with ⟨u, rfl⟩; exact hweyl_torus u),
    le_antisymm hC_le hC_ge, hcross⟩
