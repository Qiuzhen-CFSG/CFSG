...
lemma card_not_dvd_sylow_eq_bot
    ...
    C.toSubgroup = ⊥ := by
  apply (Subgroup.eq_bot_iff_card C.toSubgroup).2
  rw [C.card_eq_multiplicity, Nat.factorization_eq_zero_of_not_dvd hp, pow_zero]

lemma ringChar_prime
    {F : Type*} [Field F]
    (hc : ¬ ringChar F = 0) :
    Fact (ringChar F).Prime :=
  ⟨(CharP.char_is_prime_or_zero F (ringChar F)).resolve_right hc⟩
...
lemma covby_top_of_index_eq_prime {p : ℕ} [Fact p.Prime] {G : Type*} [Group G]
    [Finite G] (h : IsPGroup p G) {H : Subgroup G} (h_idx : H.index = p) :
    CovBy H ⊤ ∧ H.Normal := by
  have hp : p.Prime := Fact.out
  have hcov : CovBy H ⊤ := by
    ...
    · have h₃ := Subgroup.relIndex_mul_index h₁.le
      ...
      · rw [Subgroup.relIndex_eq_one] at h₅
        exact h₁.not_ge h₅
  letI : Group.IsNilpotent G := h.isNilpotent
  exact ⟨hcov, Subgroup.NormalizerCondition.normal_of_coatom H
    Group.normalizerCondition_of_isNilpotent (covBy_top_iff.mp hcov)⟩
...
public theorem center_cyclic_of_representation_faithful_irreducible
  ...
  letI : Nontrivial ρ.asModule := Subrepresentation.irreducible_module_nontrivial ρ
  have : IsDomain k := {
    toIsCancelMulZero := by
      refine isCancelMulZero_iff_noZeroDivisors.mpr ?_
      rw [_root_.noZeroDivisors_iff]
      intro a b hab
      letI : DecidableEq (End F[G] ρ.asModule) := Classical.typeDecidableEq _
      letI : IsSimpleModule F[G] ρ.asModule :=
        (irreducible_iff_isSimpleModule_asModule ρ).mp inst
...
lemma pGroup_fix_nonzero_vector
  ...
  let p := ringChar F
  letI : Fact p.Prime := ringChar_prime hc
  let pr : ℕ → Prop := fun n ↦
    (H : Subgroup G) → Nat.card H = p ^ n → ∃ v : V, v ≠ 0 ∧ ∀ g ∈ H, ρ g • v = v
  have (n : ℕ) : pr n := by
    induction n with
    | zero =>
      intro H hH
    ...
    | succ n ih =>
      ...
      letI : Nontrivial ↥φ.toSubmodule := by
        ...
        suffices v ≠ 0 by
          ...
          exact this
        exact hv1
      have hpow : (φ.toRepresentation y) ^ p = 1 := by
        ext w
      ...
      have : (φ.toRepresentation y - 1) ^ p = 0 := by
        letI : CharP (End F φ.toSubmodule) p := by
          ...
          have hcast : (ringChar F : F) • w.val = 0 := by
      ...
      have : ∃ n : ℕ, ∃ m ∈ (M.map H.subtype), g = m * y ^ n := by
        ...
        use n, (h * (y⁻¹ ^ n)).val, (by
          simp only [Subgroup.mem_map, Subgroup.subtype_apply, Subtype.exists, exists_and_right,
            exists_eq_right]
...
lemma moduleEnd_mul_comm_of_apply_eq_smul
    {F V : Type*} [Field F] [AddCommGroup V] [Module F V]
    (e f : Module.End F V) (a b : F)
    (he : ∀ v, e v = a • v) (hf : ∀ v, f v = b • v) :
    e * f = f * e := by
  ext v
  simp only [Module.End.mul_apply, hf, he, smul_smul, mul_comm]
...
lemma eigenspace_le_of_isCompl_of_eq_left
  ...
  rcases Submodule.existsUnique_add_of_isCompl hcompl z with ⟨u, w, huw, -⟩
  ...
  obtain ⟨_u1, _w1, _hu1w1, huniq⟩ :=
    Submodule.existsUnique_add_of_isCompl hcompl (e z)
...
lemma commutator_le_ker_of_representation_mul_comm
    {F G V : Type*} [Field F] [Group G] [AddCommGroup V] [Module F V]
    (ρ : Representation F G V) (hcomm : ∀ g h : G, ρ g * ρ h = ρ h * ρ g) :
    commutator G ≤ ρ.ker := by
  rw [commutator_def, Subgroup.commutator_le]
  intro g _ h _
  change ρ (g * h * g⁻¹ * h⁻¹) = 1
  calc
    ρ (g * h * g⁻¹ * h⁻¹) = ρ g * (ρ h * ρ g⁻¹) * ρ h⁻¹ := by
      simp [map_mul, mul_assoc]
    _ = ρ g * (ρ g⁻¹ * ρ h) * ρ h⁻¹ := by rw [hcomm h g⁻¹]
    _ = 1 := by
...
private lemma exists_prime_pCore_ne_bot_of_induction
    {F : Type uF} {G : Type uG} {V : Type uV} [Field F] [Group G]
    [AddCommGroup V] [Module F V] [FiniteDimensional F V]
    (ho : Odd (Nat.card G)) {ρ : Representation F G V}
    (hi : Function.Injective ρ) (S : Subgroup G) (hS : S ≠ ⊥)
    (hsmall :
      ∀ {H : Type uG} [Group H], Nat.card H < Nat.card G → Odd (Nat.card H) →
        {σ : Representation F H V} → Function.Injective σ →
          ∃ C : Sylow (ringChar F) H, IsMulCommutative C ∧ commutator H ≤ C) :
    ∃ q : ℕ, q.Prime ∧ pCore q S ≠ ⊥ := by
  letI := card_odd_finite ho
  have hap (N : Subgroup S) (hne : N ≠ ⊥) : ∃ q : ℕ, q.Prime ∧ pCore q N ≠ ⊥ := by
    have hoGs : Odd (Nat.card S) := Odd.of_dvd_nat ho S.card_subgroup_dvd_card
    ...
    · obtain ⟨n, hn⟩ := hpow
      ...
      have hchar : ringChar F ≠ 0 := by
        intro hchar
        rw [hchar] at hn
        rw [hn] at hN1
        exact (not_lt_of_ge (zero_pow_le_one n)) hN1
      use ringChar F, (ringChar_prime hchar).out
      unfold pCore
      ...
      letI : Nontrivial N := (Subgroup.nontrivial_iff_ne_bot N).mpr hne
      exact top_ne_bot
    · have hNcard_ne_zero : Nat.card N ≠ 0 := Nat.card_pos.ne'
      ...
      · let H : Subgroup N := Subgroup.normalizer (Q : Set N)
        ...
        let ρH : Representation F H V := ρ.comp (S.subtype.comp (N.subtype.comp H.subtype))
        have hiH : Function.Injective ρH := by
          intro x y hxy
          repeat' apply Subtype.ext
          exact hi (by simpa [ρH] using hxy)
        have hcardH_lt_G : Nat.card H < Nat.card G := by
          exact lt_of_lt_of_le hcardH_lt
            (le_trans (Subgroup.card_le_card_group N) (Subgroup.card_le_card_group S))
        obtain ⟨C, hCcomm, hcomm_le_C⟩ :=
          hsmall (H := ↥H) hcardH_lt_G hoH hiH
        ...
        let M' : Subgroup S := M.map N.subtype
  ...
  letI : Nontrivial S := (Subgroup.nontrivial_iff_ne_bot S).mpr hS
  ...
  intro K hK
  have := hq2 (K.map (⊤ : Subgroup S).subtype) (by
    constructor
...
private lemma exists_normal_central_exponent_subgroup
    {G : Type uG} [Group G] (q : ℕ) (Q : Subgroup G) (hQ : Q.Normal) :
    ∃ K : Subgroup G,
      IsElementaryAbelian q K ∧ K.Normal ∧
        ∀ g, g ∈ K ↔ g ∈ Q ∧ (∀ h ∈ Q, g * h = h * g) ∧ g ^ q = 1 := by
  let K : Subgroup G := {
    carrier := {g : G | g ∈ Q ∧ (∀ h ∈ Q, g * h = h * g) ∧ g ^ q = 1}
    mul_mem' := by
      intro g₁ g₂ hg₁ hg₂
      refine ⟨(Subgroup.mul_mem_cancel_left Q hg₁.1).mpr hg₂.1, ?_, ?_⟩
      · intro h hh
    ...
    one_mem' := by
      simp only [Set.mem_setOf_eq, one_mem, one_mul, mul_one, one_pow, and_self,
        implies_true]
    inv_mem' := by
      intro g hg
      refine ⟨Subgroup.inv_mem Q hg.1, ?_, ?_⟩
      · intro h hh
        rw [← commute_iff_eq, Commute.inv_left_iff, commute_iff_eq, hg.2.1 h hh]
      · rw [inv_pow, hg.2.2, inv_one]
  }
  refine ⟨K, ?_, ?_, fun _ ↦ Iff.rfl⟩
  · exact {
      toIsMulCommutative := {
        is_comm := {
  ...
  · exact {
      conj_mem := by
        intro k hk g
        refine ⟨Subgroup.Normal.conj_mem hQ k hk.1 g, ?_, ?_⟩
        · intro h hh
          rw [← mul_left_inj (a := g)]
          calc
            g * k * g⁻¹ * h * g = g * (k * (g⁻¹ * h * g)) := by group
            _ = g * ((g⁻¹ * h * g) * k) := by
              rw [hk.2.1 _ (Subgroup.Normal.conj_mem' hQ h hh g)]
            _ = _ := by group
        · rw [conj_pow, hk.2.2, mul_one, mul_inv_cancel]
    }

private lemma exists_commutative_sylow_of_eq_ringChar
    {k : Type uF} [Field k]
    {G : Type uG} [Group G] [Finite G]
    {V : Type uV} [AddCommGroup V] [Module k V]
    (hdim : Module.finrank k V = 2)
    {ρ : Representation k G V} (hi : Function.Injective ρ)
    (Gs : Subgroup G) [Gs.Normal] {q : ℕ} (hq1 : q.Prime)
    (hq2 : pCore q Gs ≠ ⊥) (hq : q = ringChar k) :
    ∃ C : Sylow q G, IsMulCommutative C ∧ commutator G ≤ C := by
  let qC := (pCore q Gs).map Gs.subtype
  have hqC : qC.Normal := by
    have : (pCore q Gs).Characteristic := pCore_characteristic (G := Gs) (p := q)
    exact ConjAct.normal_of_characteristic_of_normal
  obtain ⟨K, hK1, hK2, hKmem⟩ :=
    exists_normal_central_exponent_subgroup q qC hqC
  letI : IsElementaryAbelian q K := hK1
  letI : K.Normal := hK2
  ...
  obtain ⟨x, hxqC, -, hxcent, hxpow, hxne1⟩ :=
    exists_central_element_orderOf_eq_prime_of_nontrivial_pgroup_subgroup qC hqC_p hqC_ne
  have hxK : x ∈ K := (hKmem x).2 ⟨hxqC, hxcent, hxpow⟩
  have hchar0 : ringChar k ≠ 0 := by
    intro h0
    have : q = 0 := by simpa [hq] using h0
    exact hq1.ne_zero this
  haveI : CharP k q := by
    rw [CharP.charP_iff_prime_eq_zero hq1, hq, ringChar.Nat.cast_ringChar]
  haveI : ExpChar k q := ExpChar.prime hq1
  letI : Fact (Module.finrank k V = 2) := ⟨hdim⟩
  letI : FiniteDimensional k V := FiniteDimensional.of_fact_finrank_eq_two
  letI : Nontrivial V := nontrivial_of_finrank_eq_two hdim
  let sigma : Representation k K V := ρ.comp K.subtype
  have hKp : IsPGroup (ringChar k) K := by
    simpa [hq] using (IsElementaryAbelian.isPGroup q K)
  obtain ⟨v, hv0, hvfix⟩ := pGroup_fix_nonzero_vector (F := k) (G := K) hchar0 hKp sigma
  let U : Submodule k V := Representation.invariants sigma
  have hvU : v ∈ U := by
  ...
  have hU_ne_top : U ≠ ⊤ := by
    ...
    have hxeq : ρ x = ρ 1 := by
      calc
        ...
        _ = ρ 1 := by
          change (1 : Module.End k V) = ρ 1
          simp
    exact hxne1 (hi hxeq)
  have hU_pos : 0 < Module.finrank k U := by
    letI : Nontrivial U := (Submodule.nontrivial_iff_ne_bot).2 hU_ne_bot
    exact Module.finrank_pos (R := k) (M := U)
  have hU_lt : Module.finrank k U < 2 := by
    simpa [hdim] using (Submodule.finrank_lt (K := k) (V := V) hU_ne_top)
  have hU1 : Module.finrank k U = 1 := by
    omega
  ...
  let rhoU : Representation k G U := Representation.subrepresentation ρ U hU_stable
  let rhoQ : Representation k G (V ⧸ U) := Representation.quotient ρ U hU_stable
  letI : Module.Free k U := Module.Free.of_divisionRing k U
  letI : Module.Free k (V ⧸ U) := Module.Free.of_divisionRing k (V ⧸ U)
  have hQ1 : Module.finrank k (V ⧸ U) = 1 := by
    have hsum := Submodule.finrank_quotient_add_finrank (R := k) (M := V) U
    omega
  have hU_data (g : G) :
      ∃ a : k, (∀ u : U, rhoU g u = a • u) ∧ LinearMap.det (rhoU g) = a := by
    obtain ⟨a, ha, _⟩ :=
      LinearMap.existsUnique_eq_smul_id_of_finrank_eq_one hU1 (rhoU g)
    refine ⟨a, ?_, ?_⟩
    · intro u
      simpa using congrArg (fun f : Module.End k U => f u) ha
    · rw [ha, LinearMap.det_smul, LinearMap.det_id, hU1, pow_one, mul_one]
  have hQ_data (g : G) :
      ∃ a : k, (∀ z : V ⧸ U, rhoQ g z = a • z) ∧ LinearMap.det (rhoQ g) = a := by
    obtain ⟨a, ha, _⟩ :=
      LinearMap.existsUnique_eq_smul_id_of_finrank_eq_one hQ1 (rhoQ g)
    refine ⟨a, ?_, ?_⟩
    · intro z
      simpa using congrArg (fun f : Module.End k (V ⧸ U) => f z) ha
  ...
  have hmulU_comm (g1 g2 : G) : rhoU g1 * rhoU g2 = rhoU g2 * rhoU g1 := by
    ...
    exact moduleEnd_mul_comm_of_apply_eq_smul (F := k) (V := U)
      (rhoU g1) (rhoU g2) a1 a2 ha1 ha2
  have hmulQ_comm (g1 g2 : G) : rhoQ g1 * rhoQ g2 = rhoQ g2 * rhoQ g1 := by
    obtain ⟨a1, ha1, -⟩ := hQ_data g1
    obtain ⟨a2, ha2, -⟩ := hQ_data g2
    exact moduleEnd_mul_comm_of_apply_eq_smul (F := k) (V := V ⧸ U)
      (rhoQ g1) (rhoQ g2) a1 a2 ha1 ha2
  have hcomm_le_ker_U : commutator G ≤ rhoU.ker :=
    commutator_le_ker_of_representation_mul_comm (F := k) (G := G) (V := U)
      rhoU hmulU_comm
  have hcomm_le_ker_Q : commutator G ≤ rhoQ.ker :=
    commutator_le_ker_of_representation_mul_comm (F := k) (G := G) (V := V ⧸ U)
      rhoQ hmulQ_comm
  have hcomm_fix_U (c : G) (hc : c ∈ commutator G) : ∀ u : U, ρ c u = u := by
    have hcU : rhoU c = 1 := MonoidHom.mem_ker.mp (hcomm_le_ker_U hc)
    intro u
    exact congrArg Subtype.val (congrArg (fun f : Module.End k U => f u) hcU)
  have hcomm_fix_Q (c : G) (hc : c ∈ commutator G) : ∀ z : V ⧸ U, rhoQ c z = z := by
    have hcQ : rhoQ c = 1 := MonoidHom.mem_ker.mp (hcomm_le_ker_Q hc)
    intro z
    simpa using congrArg (fun f : Module.End k (V ⧸ U) => f z) hcQ
  letI : CharP (Module.End k V) q := by
    refine Module.charP_end (R := k) (M := V) (p := q) ?_
    obtain ⟨w, hw⟩ := exists_ne (0 : V)
    refine ⟨w, ?_⟩
    rw [Ideal.torsionOf_eq_bot_iff_of_noZeroSMulDivisors]
    exact hw
  haveI : ExpChar (Module.End k V) q := ExpChar.prime hq1
  have hcomm_qpow : ∀ c ∈ commutator G, c ^ q = 1 := by
    intro c hc
    obtain ⟨W, hcomplUW⟩ := exists_isCompl U
    let e : Module.End k V := ρ c - 1
    ...
    have hsub : (ρ c) ^ q - 1 = 0 := by
      calc
        (ρ c) ^ q - 1 = e ^ q := by
          symm
          simpa [e] using
            (sub_pow_expChar_of_commute (R := Module.End k V) (p := q)
  ...
  have hscalar_eq_one {a : k} {m : ℕ} (ha : a ^ q ^ m = 1) : a = 1 := by
    have hpow0 : (a - 1) ^ q ^ m = 0 := by
      calc
        (a - 1) ^ q ^ m = a ^ q ^ m - 1 ^ q ^ m := by
          simpa using
            (sub_pow_expChar_pow_of_commute (p := q) (x := a) (y := (1 : k)) (n := m)
              (Commute.one_right _))
  ...
  have hU_fix_C (y : C) : ∀ u : U, ρ y u = u := by
    ...
    rw [hm] at hpow
    have hdetpow := congrArg (fun f : Module.End k U => LinearMap.det f) hpow
  ...
  have hQ_fix_C (y : C) : ∀ z : V ⧸ U, rhoQ y z = z := by
    ...
    rw [hm] at hpow
    have hdetpow :=
      congrArg (fun f : Module.End k (V ⧸ U) => LinearMap.det f) hpow
    rw [map_pow, hdet] at hdetpow
    have ha1 : a = 1 := hscalar_eq_one (by simpa using hdetpow)
    intro z
    calc
...
private lemma isMulCommutative_of_ne_ringChar
    {k : Type uF} [Field k] [IsAlgClosed k]
    {G : Type uG} [Group G] [Finite G]
    {V : Type uV} [AddCommGroup V] [Module k V]
    (ho : Odd (Nat.card G)) (hdim : Module.finrank k V = 2)
    {ρ : Representation k G V} (hi : Function.Injective ρ)
    (Gs : Subgroup G) [Gs.Normal]
    (hdet_one : ∀ g, g ∈ Gs → LinearMap.det (ρ g) = 1)
    {q : ℕ} (hq1 : q.Prime) (hq2 : pCore q Gs ≠ ⊥)
    (hqchar : q ≠ ringChar k) :
  ...
  obtain ⟨x, hxqC, hxord, hxcent, hxpow, hxne1⟩ :=
    exists_central_element_orderOf_eq_prime_of_nontrivial_pgroup_subgroup qC hqC_p hqC_ne
  have hxK : x ∈ K := (hKmem x).2 ⟨hxqC, hxcent, hxpow⟩
  let σ : Representation k K V := ρ.comp K.subtype
  have hσcr : σ.IsCompletelyReducible := by
    ...
    · left
      exact hchar0
    · right
      have hprime : Nat.Prime (ringChar k) := (ringChar_prime hchar0).out
      refine ⟨hprime, ?_⟩
      rcases (IsElementaryAbelian.isPGroup q K).exists_card_eq with ⟨m, hm⟩
      have hnotdvd : ¬ q ∣ ringChar k := by
        intro hdvd
        exact hqchar ((Nat.prime_dvd_prime_iff_eq hq1 hprime).mp hdvd)
      have hcop : Nat.Coprime (ringChar k) (q ^ m) :=
  ...
  letI := hφirr
  have hφ1 : Module.finrank k φ.toSubmodule = 1 := by
    simpa using
      Representation.IsIrreducible.finrank_eq_one_of_isMulCommutative
        (G := K) (k := k) (V := φ.toSubmodule) (ρ := φ.toRepresentation)
  have hψ1 : Module.finrank k ψ.toSubmodule = 1 :=
    finrank_eq_one_of_isCompl_left_of_finrank_eq_two φ.toSubmodule ψ.toSubmodule hcompl_sub
      hφ1 hdim
  let xK : K := ⟨x, hxK⟩
  letI : Module.Free k φ.toSubmodule :=
    Module.Free.of_divisionRing k φ.toSubmodule
  letI : Module.Free k ψ.toSubmodule :=
    Module.Free.of_divisionRing k ψ.toSubmodule
  obtain ⟨a, ha, _⟩ :=
  ...
  have hdetφ :
      (@LinearMap.det φ.toSubmodule φ.toSubmodule.addCommGroup k
  ...
  have hdetψ :
      (@LinearMap.det ψ.toSubmodule ψ.toSubmodule.addCommGroup k
  ...
  have hab1 : a * b = 1 := by
    ...
    rw [show σ xK = ρ x by rfl, hdetφ, hdetψ, hdet_one x (hqC_le_Gs hxqC)] at hdet
    exact hdet.symm
  ...
  have haq : a ^ q = 1 := by
    ...
    have hdet :=
      congrArg (fun f : Module.End k φ.toSubmodule => LinearMap.det f) hpow
  ...
  have hbq : b ^ q = 1 := by
    have hpow : (ψ.toRepresentation xK) ^ q = 1 := by
      rw [← map_pow, hxKpow, map_one]
    have hdet :=
      congrArg (fun f : Module.End k ψ.toSubmodule => LinearMap.det f) hpow
  ...
  have hab : a ≠ b := by
    ...
    have hxid : ρ x = LinearMap.id := by
      apply linearMap_eq_of_isCompl_of_eq_on_each (e := ρ x) (f := LinearMap.id)
    ...
    have hxeq : ρ x = ρ 1 := by
      calc
        ...
        _ = ρ 1 := hρone.symm
    exact hxne1 (hi hxeq)
  let U : Submodule k V := φ.toSubmodule
  let W : Submodule k V := ψ.toSubmodule
  have hU1 : Module.finrank k U = 1 := by
    simpa [U] using hφ1
  ...
  have hU_ne_bot : U ≠ ⊥ := by
    intro hUbot
    have : Module.finrank k U = 0 := by simp [hUbot]
    omega
  have hU_ne_W : U ≠ W := by
    intro hUW
  ...
  have hU_map_or (g : G) : U.map (ρ g) = U ∨ U.map (ρ g) = W := by
    let Ug : Submodule k V := U.map (ρ g)
    have hUg1 : Module.finrank k Ug = 1 := by
      let e : U ≃ₗ[k] Ug :=
    ...
    · by_cases hcb : c = b
      ...
      · have heig_bot : Module.End.eigenspace (ρ x) c = ⊥ :=
        ...
        have : Module.finrank k Ug = 0 := by simp [hUg_bot]
        omega
  have hW_map_or (g : G) : W.map (ρ g) = U ∨ W.map (ρ g) = W := by
    let Wg : Submodule k V := W.map (ρ g)
    have hWg1 : Module.finrank k Wg = 1 := by
      let e : W ≃ₗ[k] Wg :=
    ...
    have hWg_le_eig : Wg ≤ Module.End.eigenspace (ρ x) c := by
      ...
      have hconj_apply :
        ...
        exact congrArg (fun f : Module.End k V => f w) hconj
      calc
    ...
    · by_cases hcb : c = b
      ...
      · have heig_bot : Module.End.eigenspace (ρ x) c = ⊥ :=
        ...
        have : Module.finrank k Wg = 0 := by simp [hWg_bot]
        omega
  have hmap_top (g : G) : (⊤ : Submodule k V).map (ρ g) = ⊤ := by
  ...
  have hU_map_eq (g : G) : U.map (ρ g) = U := by
    ...
    · have hWgU : W.map (ρ g) = U := by
        ...
        · exact hWU
        · exfalso
          have htopW : (⊤ : Submodule k V) = W := by
            calc
              ⊤ = (⊤ : Submodule k V).map (ρ g) := by simpa using (hmap_top g).symm
              _ = (U ⊔ W).map (ρ g) := by rw [hcomplUW.sup_eq_top]
              _ = U.map (ρ g) ⊔ W.map (ρ g) := by rw [Submodule.map_sup]
              _ = W := by rw [hUW, hWW, sup_idem]
          have : Module.finrank k (⊤ : Submodule k V) = 1 := by
            rw [htopW]
            exact hW1
  ...
  have hU_scalar (g : G) : ∃ c : k, ∀ u : U, ρ g u = c • u := by
    let gU : Module.End k U := (ρ g).restrict (hU_stable g)
    obtain ⟨c, hc, _⟩ :=
      LinearMap.existsUnique_eq_smul_id_of_finrank_eq_one hU1 gU
    refine ⟨c, ?_⟩
    intro u
    have hu : gU u = c • u := by
      simpa using congrArg (fun f : Module.End k U => f u) hc
    exact congrArg Subtype.val hu
  have hW_scalar (g : G) : ∃ c : k, ∀ w : W, ρ g w = c • w := by
    let gW : Module.End k W := (ρ g).restrict (hW_stable g)
    obtain ⟨c, hc, _⟩ :=
      LinearMap.existsUnique_eq_smul_id_of_finrank_eq_one hW1 gW
    refine ⟨c, ?_⟩
    intro w
    have hw : gW w = c • w := by
      simpa using congrArg (fun f : Module.End k W => f w) hc
  ...
  exact IsMulCommutative.mk <| Std.Commutative.mk <| fun g h ↦ hi (hcommρ g h)
...
public theorem theorem_2_6_b
  ...
  · rw [add_assoc, one_add_one_eq_two] at hc ih
    ...
    · have h : Subgroup.center G = ⊤ := by
        ...
        exact Subgroup.mem_bot.mp <| hGs_bot ▸ this
      letI : CommGroup G := {
        mul_comm a b := by
    ...
    have hq : ∃ q : ℕ, q.Prime ∧ pCore q Gs ≠ ⊥ := by
      apply exists_prime_pCore_ne_bot_of_induction ho hi0 Gs hGs_bot
      intro H _ hcardH_lt hoH σ hiH
      exact ih (Nat.card H) (by simpa [hc] using hcardH_lt)
        (G := H) hoH (V := V) hdim0 (ρ := σ) hiH rfl
    obtain ⟨q, hq1, hq2⟩ := hq
    by_cases! h : q = ringChar F
    · have hq' : q = ringChar F' := by
        simpa [Algebra.ringChar_eq F F'] using h
      rw [← h]
      exact exists_commutative_sylow_of_eq_ringChar
        (k := F') (G := G) (V := V') hdim hi Gs hq1 hq2 hq'
    · have hqchar : q ≠ ringChar F' := by
        intro hEq
        apply h
        simpa [Algebra.ringChar_eq F F'] using hEq
      letI hcomm : IsMulCommutative G :=
        isMulCommutative_of_ne_ringChar (k := F') (G := G) (V := V') ho hdim hi Gs
          (by
            intro g hg
            simpa [Gs] using hg)
          hq1 hq2 hqchar
