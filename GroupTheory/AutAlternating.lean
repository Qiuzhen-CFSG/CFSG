...
public theorem conjNormal_injective_alternatingGroup (n : ℕ) (hn : 4 ≤ n) :
  ...
  exact injective_of_ker_eq_bot (MulAut.conjNormal (H := alternatingGroup (Fin n)))
    (by
      rw [conjNormal_ker]
...
public theorem isSolvable_Out_of_isAltGroup (n : ℕ) (hn : 5 ≤ n) (hn6 : n ≠ 6) :
  ...
  let c : Perm (Fin n) →* MulAut A := MulAut.conjNormal (H := alternatingGroup (Fin n))
  have hbij : Function.Bijective c := aut_alternatingGroup_bijective_conj n hn hn6
  let e : Perm (Fin n) ≃* MulAut A := MulEquiv.ofBijective c hbij
  -- Inn(Aₙ) = the image of Aₙ under the conjugation map
  have hinn : innerAutGroup A =
      Subgroup.map e.toMonoidHom (alternatingGroup (Fin n) : Subgroup (Perm (Fin n))) := by
    apply le_antisymm
    · intro x hx
      unfold innerAutGroup at hx
      rcases (MonoidHom.mem_range).1 hx with ⟨a, ha⟩
      rw [Subgroup.mem_map]
      refine ⟨(a : Perm (Fin n)), a.2, ?_⟩
      have hx' : x = c (a : Perm (Fin n)) := by
        rw [← ha]
        exact (conjNormal_eq_conj (K := alternatingGroup (Fin n)) a).symm
      rw [hx']
      rfl
    · intro x hx
      ...
      refine ⟨⟨g, hg⟩, ?_⟩
      exact conjNormal_eq_conj (K := alternatingGroup (Fin n)) ⟨g, hg⟩
  -- Out(Aₙ) ≃* Sₙ/Aₙ  (via the iso e and the equality above)
  let e1 : Perm (Fin n) ⧸ (alternatingGroup (Fin n) : Subgroup (Perm (Fin n))) ≃* Out A :=
    QuotientGroup.congr (alternatingGroup (Fin n) : Subgroup (Perm (Fin n)))
      (innerAutGroup A) e hinn.symm
  -- Sₙ/Aₙ ≃* sign.range
  let e2 : Perm (Fin n) ⧸ (alternatingGroup (Fin n) : Subgroup (Perm (Fin n))) ≃*
      ↥((Equiv.Perm.sign (α := Fin n)).range) := by
    exact QuotientGroup.quotientKerEquivRange
      (φ := (Equiv.Perm.sign (α := Fin n) : Perm (Fin n) →* ℤˣ))
  -- sign.range ≤ ℤˣ is abelian, hence solvable
  haveI : IsSolvable ↥((Equiv.Perm.sign (α := Fin n)).range) := by
    haveI : IsSolvable (Units ℤ) := by
      letI : CommGroup (Units ℤ) := inferInstance
      infer_instance
    exact subgroup_solvable_of_solvable (Equiv.Perm.sign (α := Fin n)).range
  let e3 : ↥((Equiv.Perm.sign (α := Fin n)).range) ≃* Out A := e2.symm.trans e1
