...
\qquad V_d=\langle Z_l\mid l\in D(d)\rangle.
\]
...
\begin{enumerate}
\item[(a)] $2^5\le |S|\le2^6$, $G_d/Q_d\cong SL_2(2)$ for every $d$,
  $Q=Q_a$, $O_2(E_a)\cong C_4\times C_4$, and
  $Q_{a+1}\cong C_4\rtimes Q_8$ or $Q_8\rtimes Q_8$;
\item[(b)] $2^8\le |S|\le2^{10}$,
  $G_a/Q_a\cong SL_2(2)$ and
  $G_{a+1}/Q_{a+1}\cong SL_2(2)\times C_2$, with
  $Q/O_2(E_a)$ of order at most $4$, $O_2(E_a)$ special of order $2^6$,
  $V_{a+1}\cong Q_8\rtimes Q_8$, $\Phi(Q_{a+1})=Z_{a+1}$, and an
  elementary abelian normal subgroup $W$ of order $2^4$ in $L$ whose
  normalizer is nonsolvable;
...
\noindent A finite group is said to be of type $G_2(2)'$, $V_6^-(3)$, or
$V_8^+(3)$ if it contains a pair $G_a,G_{a+1}$ with Sylow intersection,
trivial 2-core of their join, and satisfying respectively alternatives
(a), (b), or (c) of (8.6). The groups $G_2(2)$,
$\operatorname{Aut}(V_6^-(3))$, and
$\operatorname{Aut}(V_8^+(3))$ provide examples.
...
\noindent A finite group is of type $V_6^+(2)$ if it contains two
subgroups $G_a,G_{a'}$ with Sylow intersection, trivial 2-core of their
join, and satisfying (9.1)(a)--(c). The group
$V_6^+(2)\cong S_8\cong\operatorname{Aut}(L_4(2))$ is an example.
...
\noindent In this section we assume Hypothesis 2, $G=\langle P_1,P_2\rangle$,
and the notation of Section 7, together with $[Z_a,Z_{a'}]=1$. Thus
$Z_{a'}=\Omega_1(Z(T))\le Z(G_{a'})$ and $Z(G_a)=1$.
...
\item[(a)] $G_a/Q_a\cong SL_2(2)\times C_2$ and
  $G_{a'}/Q_{a'}\cong SL_2(2)$;
\item[(b)] $|S|=2^7$, $Q_a=Z_a$, and $V_{a'}^*\cong Q_8\rtimes Q_8$;
\item[(c)] there is $U\le V_{a'}^*$ of order $2^3$ with
  $N_H(U)/U\cong L_3(2)$.
\end{enumerate}
\end{stm}
...
\begin{proof}
Here $a'=a+1$ and $Z_a\le Q_{a'}$. Applying (7.8) to $A=Z_a$ and
then (1.2), (1.6), and (1.7), one obtains $|Z_a|=4$,
$|S|=2^7$, the two quotient structures in (a), and
$V_{a'}^*\cong Q_8\rtimes Q_8$. The order-$3$ elements in
$E_{a'}$ supply an elementary abelian subgroup $U$ of order $8$;
its two vertex stabilizers give $N_H(U)/U\cong L_3(2)$.
\end{proof}

\noindent A finite group is of type $V_6^+(2)$ if it contains two such
subgroups satisfying (9.1)(a)--(c). The group
$V_6^+(2)\cong S_8\cong\operatorname{Aut}(L_4(2))$ is an example.
...
\begin{proof}
Conjugate to $d=a+1$, $l=a$. Put
$Q=Q_{a+1}\cap Q_m$. By (7.6)(c), $Q\le Q_a$ and
$[Z_a,Q]\ne1$. Applying (1.7) gives $|Z_a|=4$ and
$G_a/C_{G_a}(Z_a)\cong SL_2(2)$; (7.7)(a) then gives
$Q_a=C_{G_a}(Z_a)$.
\end{proof}
...
\begin{proof}
If $|Z_a|>4$, apply (7.8) twice along the critical path. The resulting
subgroups either give a nontrivial normal subgroup of both adjacent
vertex stabilizers or reduce to (9.2); both alternatives contradict
(7.2) or force $|Z_a|=4$. Then (7.7)(a) gives
$G_a/Q_a\cong SL_2(2)$, and transitivity gives the assertion for all
$l\in a^G$.
\end{proof}

\noindent We use the following consequence of (9.3): if $b>1$, then
$Z_a=Z_{a-1}Z_{a+1}$, $|Z_{a+1}|=2$, $S=Q_aQ_{a+1}$, and
$V_{a+1}/Z_{a+1}$ is a faithful $G_{a+1}/Q_{a+1}$-module.

\begin{stm}\label{st:9.4}
Let $b>1$, $d(r,a+1)=2$, $t\in C_{G_{a+1}}(V_r)$,
$x\in[E_{a+1},t]$, and $A\le V_r$ satisfy
\begin{enumerate}
\item[(i)] $[A,t]\le V_{a+1}$;
\item[(ii)] $\langle G_{a+1}\cap G_n,t\rangle=G_{a+1}$ for
  $n\in D(a+1)\cap D(r)$;
...
\begin{proof}
Apply (7.8) to the pair of 2-subgroups
$G_{a+1}\cap G_n$ and $G_{a+1}$, and then use (1.7) on the resulting
dihedral section. Otherwise one obtains a subgroup normal in both
adjacent vertex stabilizers, contrary to (7.2).
\end{proof}
...
\begin{proof}
On $V_a/Z_a$, $t$ is a transvection. Lemmas (1.7) and (7.7)(b)
decompose the module into four-dimensional summands. The relation
$Z_{a-1}\le V_1V_1^w$ for a suitable $w\in O_2(E_{a-1})$ gives
$V_1=V_1^w$ or $V_1\ne V_1^w$, which are exactly (a) and (b).
\end{proof}
...
\begin{proof}
Set $R=[V_{a+1},V_{a'}]$ and
$R_0=[V_{a-1},V_{a'-2}]$. The cases $R_0=1$, $b=3$, and $b>3$
are excluded respectively by (9.4), (7.7), and the critical-pair
symmetry. Hence the index is $2$.
\end{proof}
...
\begin{proof}
Apply (9.5) to an element of $V_{a+1}\setminus Q_{a'}$. If $b>3$,
$[V_{a'},Z_a]$ lies in two incompatible path modules, contradicting
(7.4); hence $b=3$, and (9.5) gives the order and quotient.
\end{proof}

\begin{stm}\label{st:9.8}
Suppose that $Z_{a'}\le V_{a+1}$. Then $b\le3$.
\end{stm}

\begin{proof}
If $b>3$, choose $l\in D(a+1)$. The subgroup $W_l$ is abelian and
centralizes $V_{a+1}$. Applying (7.7) and (7.8) produces
$m\in D(a')$ with $[Z_a,Z_m]\ne1$, contradicting (9.7) or (7.6).
\end{proof}

\begin{stm}\label{st:9.9}
Suppose that $Z_{a+1}\le V_{a'}$. Then $b\le3$.
\end{stm}

\begin{proof}
Interchange $a+1$ and $a'$ in (9.8). If $b>3$, (1.7) gives a subgroup
$V_1\le V_{a'}$ with $[V_1,Z_a]=Z_{a+1}$; a maximal subgroup in
$V_{a-1}$ would then have index at least $4$, contradicting (9.7).
\end{proof}
...
\begin{proof}
If $b>3$, (9.8) and (9.9) give
$Z_{a'}\le V_{a+1}$ and $Z_{a+1}\le V_{a'}$. Applying (7.8) once
more yields the configuration of (9.5)(b), which contradicts (9.7).
\end{proof}
