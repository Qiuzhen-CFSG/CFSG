...
private theorem FeitSibleyData.isZGroup_Q1_of_D_eq_bot
  ...
  have hTI : ∀ g : G, g ∉ d.QInG →
    ...
    simpa [FeitSibleyData.QInG, PFchapter1section1.rightConjugate] using
      d.Q_TI_in_G g⁻¹ hginv
  obtain ⟨K, hfrob⟩ :=
  ...
  have hcomm : commutator G ≤ rho.ker := by
    ...
    calc
      ...
      _ = (rho g1 * rho g1⁻¹) * (rho g2 * rho g2⁻¹) := by
        simp [mul_assoc]
      _ = 1 := by simp [hg1, hg2]
  simpa [derivedSubgroup, derivedSeries_one, commutator] using hcomm
...
private theorem FeitSibleyData.map_derivedSubgroup_Q_eq_S_of_Q1_commutative
  ...
  have hcomm : ⁅d.Q, d.Q⁆ = ⁅d.S, d.S⁆ := by
    ...
    · rw [Subgroup.commutator_le]
      ...
      have hprodComm :
        ...
        ext
        · rfl
        · refine d.Q1.subtype_injective ?_
          calc
            ((⁅px, py⁆).2 : d.H) = (px.2 * py.2 * px.2⁻¹ * py.2⁻¹ : d.H) := by
              simp [commutator]
            _ = ((⁅px.2, py.2⁆ : ↥d.Q1) : d.H) := by
              simp
            _ = (1 : d.H) := by
              simpa using congrArg Subtype.val hright
            _ = ((MonoidHom.inl d.S d.Q1 ⁅px.1, py.1⁆).2 : d.H) := by
              simp
      ...
      have hxy :
        ...
        calc
          ⁅x, y⁆ = Subtype.val (⁅xQ, yQ⁆ : d.Q) := by
            unfold commutatorElement
            simp [xQ, yQ, Subgroup.coe_mul d.Q, Subgroup.coe_inv d.Q]
          _ = Subtype.val (⁅px.1, py.1⁆ : d.S) := by
            -- From heqH.symm: Subtype.val (⁅e px, e py⁆ : d.Q) = ⁅px.1, py.1⁆ : d.H
            -- Since e px = xQ and e py = yQ by px, py definition, we get the result
            have := heqH.symm
            simpa [px, py, xQ, yQ, e, MulEquiv.apply_symm_apply] using this
          _ = ⁅(px.1 : d.H), (py.1 : d.H)⁆ := by
            unfold commutatorElement
            simp [Subgroup.coe_mul d.S, Subgroup.coe_inv d.S]
      rw [hxy]
  ...
  calc
    Subgroup.map d.Q.subtype (derivedSubgroup d.Q) = ⁅d.Q, d.Q⁆ := by
      simpa [derivedSubgroup, derivedSeries_one, commutator, commutator] using
...
private theorem irreducible_degree_sq_le_centerQuotient_index_appendixIV
  ...
  simpa [N, e, derivedSubgroup, derivedSeries_one, commutator] using
...
private theorem actsRegularly_quotient_of_solvable_coprime_appendixIV
  ...
  have hNinvB : IsInvariant B M N := by
    refine ⟨?_⟩
    intro b x
    have htemp : (b : A) • x = b • x :=
      (MulAction.compHom_smul_def B.subtype b x).symm
    simpa [htemp] using
  ...
  have hfixedM_B : fixedPointSubgroup B M = fixedPointSubgroup (↥(Subgroup.zpowers a)) M := by
    ext x
    simp only [fixedPointSubgroup, FixedPoints.mem_subgroup]
    constructor
    · intro h b
      simpa [MulAction.compHom_smul_def B.subtype] using h b
    · intro h b
      simpa [MulAction.compHom_smul_def B.subtype] using h b
  ...
  have hfixed_quot : fixedPointSubgroup B (M ⧸ N) = ⊥ := by
    have hmap_bot : (⊥ : Subgroup M).map (QuotientGroup.mk' N) = ⊥ := Subgroup.map_bot _
    simpa [hmap_bot] using hfixed
  have hfixed_goal : fixedPointSubgroup (↥(Subgroup.zpowers a)) (M ⧸ N) = ⊥ := by
    have htemp : fixedPointSubgroup B (M ⧸ N) = fixedPointSubgroup (↥(Subgroup.zpowers a)) (M ⧸ N) := by
      ext x
      simp only [fixedPointSubgroup, FixedPoints.mem_subgroup]
      constructor
      · intro h b
  ...
  · refine lt_of_le_of_ne hQ2leZ3 ?_
    ...
    have hUqSolv : IsSolvable Uq :=
      isSolvable_of_comm (fun a b => by
        ...
        calc
          eCenter.symm (a * b) = eCenter.symm a * eCenter.symm b := map_mul (eCenter.symm) a b
          _ = eCenter.symm b * eCenter.symm a := Subtype.ext
            (((Subgroup.mem_center_iff.mp (eCenter.symm a).property) (eCenter.symm b : d.Q1 ⧸ N)).symm)
          _ = eCenter.symm (b * a) := (map_mul (eCenter.symm) b a).symm)
    ...
    have hqAbSurj : Function.Surjective qAb := by
      ...
      simp [qAb]
    have hCeq : C = commutator d.Q1 := by
      simp [C, derivedSubgroup, derivedSeries_one, commutator]
    have hAbP_comm : IsPGroup p (d.Q1 ⧸ commutator d.Q1) :=
      hquotP.of_surjective qAb hqAbSurj
    have hQ1P : IsPGroup p d.Q1 :=
      Section6.isPGroup_of_nilpotent_quotient_commutator_isPGroup
  ...
  have hlin :
    ...
    simpa using congrArg
      (fun F : Representation.IntertwiningMap rho rho =>
        (F : Module.End ℂ V)) hc
  have hgoal : rho (MonoidHom.inl A C a) = c • (1 : Module.End ℂ V) := by
    have hf_val : (f : Module.End ℂ V) = rho (MonoidHom.inl A C a) := by
      dsimp [f]
      rfl
    have htemp : (algebraMap ℂ (Representation.IntertwiningMap rho rho) c : Module.End ℂ V) = c • (1 : Module.End ℂ V) := by
      ext v
      simp [Representation.IntertwiningMap.algebraMap_apply, Representation.IntertwiningMap.coe_one]
    calc
      rho (MonoidHom.inl A C a) = (f : Module.End ℂ V) := by
        symm; exact hf_val
      _ = (algebraMap ℂ (Representation.IntertwiningMap rho rho) c : Module.End ℂ V) := hlin.symm
      _ = c • (1 : Module.End ℂ V) := htemp
  exact hgoal
...
private def feitSibleyStep3BaseDegreeData
    {G : Type u} [Group G] [Finite G] (d : FeitSibleyData G)
    (_chars : Finset (ClassFunction d.H)) (_R : Subgroup d.H)
...
private theorem feitSibley_step3_exists_base_companion_core
  ...
  have hpiN (k : NQ) : pi k = k := by
    ...
    have hk :
      ...
      apply e.injective
      have hk_val : (k : d.Q) = ⟨(kEquiv k : d.H), d.Q1_le_Q (kEquiv k).property⟩ := Subtype.ext rfl
      calc
        e (e.symm k) = (k : d.Q) := by simp
        _ = ⟨(kEquiv k : d.H), d.Q1_le_Q (kEquiv k).property⟩ := hk_val
        _ = e (MonoidHom.inr d.S d.Q1 (kEquiv k)) := by
...
public theorem feitSibley_theorem
    ...
    · intro chi psi
      have hdegree (eta : Y) :
        ...
        have hQderivedLe : QderivedH ≤ d.Q := by
          simpa [QderivedH, feitSibleyQderivedH, derivedSubgroup] using
            Subgroup.map_subtype_le (derivedSubgroup d.Q)
  ...
  have hbase_abelian :
    ...
    have hderived : QderivedH = SderivedH := by
      simpa [QderivedH, SderivedH, feitSibleyQderivedH, feitSibleySderivedH, derivedSubgroup] using
  ...
  have step1 :
    ...
    · letI : d.Q1.Normal := d.Q1_normal
      have hQderived :
        ...
        simpa [QderivedH, SderivedH, Q1derivedH, feitSibleyQderivedH, feitSibleySderivedH, feitSibleyQ1derivedH, derivedSubgroup] using
      ...
      · letI : Q2.Normal := hQ2normal
        ...
        have hSderivedLeS : SderivedH ≤ d.S := by
          simpa [SderivedH, feitSibleySderivedH, derivedSubgroup] using
            Subgroup.map_subtype_le (derivedSubgroup d.S)
        have hQ1derivedLeQ1 : Q1derivedH ≤ d.Q1 := by
          simpa [Q1derivedH, feitSibleyQ1derivedH, derivedSubgroup] using
            Subgroup.map_subtype_le (derivedSubgroup d.Q1)
