...
import Submission.BenderSuzuki.External.Huppert.XI.InvariantIndexTwo
import Submission.BenderSuzuki.External.Huppert.XI.NormalComplement
import Submission.BenderSuzuki.External.Huppert.XI.OrderThree
import Submission.BenderSuzuki.External.Huppert.XI.TopResidual
...
private theorem gl2_scalar_smul_onePoint
...
private theorem gl2_mem_center_of_onePoint_trivial
    (K : Type*) [Field K] [DecidableEq K]
    (g : GL (Fin 2) K)
    (hfix : ∀ x : OnePoint K, g • x = x) :
    g ∈ Subgroup.center (GL (Fin 2) K) := by
  have h10 : g 1 0 = 0 :=
    OnePoint.smul_infty_eq_self_iff.mp
      (hfix (OnePoint.infty : OnePoint K))
...
private theorem gl2_onePointPermHom_ker
    (K : Type*) [Field K] [DecidableEq K] :
  ...
  constructor
  · intro hg
    apply gl2_mem_center_of_onePoint_trivial K g
    intro x
    simpa using DFunLike.congr_fun hg x
  · intro hg
    rcases
        Matrix.GeneralLinearGroup.mem_center_iff_val_mem_range_scalar.mp hg with
      ⟨a, ha⟩
    ...
    have hu : Matrix.GeneralLinearGroup.scalar (Fin 2) u = g := by
      apply Units.ext
      change Matrix.scalar (Fin 2) (u : K) =
        (g : Matrix (Fin 2) (Fin 2) K)
      simpa [u] using ha
    rw [← hu]
    exact gl2_scalar_smul_onePoint K u x

private theorem gl2_center_le_onePointPermHom_ker
    (K : Type*) [Field K] [DecidableEq K] :
    Subgroup.center (GL (Fin 2) K) ≤
      (MulAction.toPermHom (GL (Fin 2) K) (OnePoint K)).ker := by
  rw [gl2_onePointPermHom_ker]

private noncomputable def pgl2_onePointPermHom
    (K : Type*) [Field K] [DecidableEq K] :
    Matrix.ProjGenLinGroup (Fin 2) K →*
      Equiv.Perm (OnePoint K) :=
  Matrix.ProjGenLinGroup.lift
    (MulAction.toPermHom (GL (Fin 2) K) (OnePoint K)) (by
      ext u x
      exact gl2_scalar_smul_onePoint K u x)

private theorem pgl2_onePointPermHom_injective
    (K : Type*) [Field K] [DecidableEq K] :
    Function.Injective (pgl2_onePointPermHom K) := by
  intro q1 q2 hq
  ...
  have hperm :
    ...
    simpa [pgl2_onePointPermHom] using hq
  apply (inv_mul_eq_one).mp
  rw [← map_inv, ← map_mul, ← MonoidHom.mem_ker,
    Matrix.ProjGenLinGroup.ker_mk, ← gl2_onePointPermHom_ker,
    MonoidHom.mem_ker]
  change
    MulAction.toPermHom (GL (Fin 2) K) (OnePoint K) (g1⁻¹ * g2) = 1
