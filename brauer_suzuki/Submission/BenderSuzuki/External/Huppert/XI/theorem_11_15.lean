...
import Submission.BenderSuzuki.External.Huppert.XI.theorem_2_6
import Submission.BenderSuzuki.External.Huppert.XI.FixedPointBruhat
import Submission.BenderSuzuki.External.Huppert.XI.ThetaEquationExtraction
...
/-- The conjugacy classes represented by elements of order four. -/
private noncomputable def xi1115_orderFourClasses
    (G : Type*) [Group G] [Finite G]
    [Fintype (ConjClasses G)] [DecidableEq (ConjClasses G)] :
    Finset (ConjClasses G) := by
  classical
  exact Finset.univ.filter fun C =>
    ∃ x : G, orderOf x = 4 ∧ C = ConjClasses.mk x

/-- The local XI.11.7 input needed by XI.11.9: order-four elements of the
Frobenius kernel represent exactly two ambient conjugacy classes. -/
private def XI1115TwoOrderFourClasses
    {H : Type*} [Group H] [Finite H] (F : Subgroup H) : Prop :=
  Nat.card (Set.range fun x : {x : F // orderOf (x : F) = 4} =>
    ConjClasses.mk ((x : F) : H)) = 2

private theorem xi1115_completeFamily_card_eq_conjClasses
    ...
    Fintype.card I = Nat.card (ConjClasses G) := by
  classical
  letI : Fintype G := Fintype.ofFinite G
  rcases Representation.completeFamily_form_basis hchi with ⟨b, _hb⟩
  calc
    Fintype.card I = Module.finrank ℂ (Representation.ClassFunction G) :=
...
/-- XI.5.8's class-number endpoint once its complete character family has
been supplied. -/
private theorem xi1115_classNumber_eq_of_completeFamily
    {G I : Type*} [Group G] [Finite G] [Fintype I]
    (chi : I → Representation.ClassFunction G)
    (hchi : Representation.IsCompleteIrreducibleCharacterFamily chi)
    (q : ℕ) (hcard : Fintype.card I = q + 3) :
    Nat.card (ConjClasses G) = q + 3 := by
  rw [← hcard]
  exact (xi1115_completeFamily_card_eq_conjClasses chi hchi).symm

/-- Arithmetic tail of XI.11.7 from the class number and XI.11.6 partition. -/
private theorem xi1115_orderFourClasses_card_eq_two_of_classNumber_partition
    {G : Type*} [Group G] [Finite G]
    [Fintype (ConjClasses G)] [DecidableEq (ConjClasses G)]
    (q : ℕ) (hclassNumber : Nat.card (ConjClasses G) = q + 3)
    (base : Finset (ConjClasses G))
    (hbaseCard : base.card = q + 1)
    (hdisjoint : Disjoint base (xi1115_orderFourClasses G))
    (hcover : base ∪ xi1115_orderFourClasses G = Finset.univ) :
    (xi1115_orderFourClasses G).card = 2 := by
  have hunionCard :
      (base ∪ xi1115_orderFourClasses G).card =
        base.card + (xi1115_orderFourClasses G).card :=
    Finset.card_union_of_disjoint hdisjoint
  rw [hcover, Finset.card_univ, ← Nat.card_eq_fintype_card,
    hclassNumber, hbaseCard] at hunionCard
  omega

/-- XI.5.8 plus the explicit XI.11.6 class partition leave exactly two
order-four classes. -/
private theorem xi1115_orderFourClasses_card_eq_two_of_completeFamily_partition
    {G I : Type*} [Group G] [Finite G] [Fintype I]
    [Fintype (ConjClasses G)] [DecidableEq (ConjClasses G)]
    (chi : I → Representation.ClassFunction G)
    (hchi : Representation.IsCompleteIrreducibleCharacterFamily chi)
    (q : ℕ) (hcharCard : Fintype.card I = q + 3)
    (base : Finset (ConjClasses G))
    (hbaseCard : base.card = q + 1)
    (hdisjoint : Disjoint base (xi1115_orderFourClasses G))
    (hcover : base ∪ xi1115_orderFourClasses G = Finset.univ) :
    (xi1115_orderFourClasses G).card = 2 := by
  exact xi1115_orderFourClasses_card_eq_two_of_classNumber_partition q
    (xi1115_classNumber_eq_of_completeFamily chi hchi q hcharCard)
    base hbaseCard hdisjoint hcover

/-- An order-four element has the same unique fixed point as its square. -/
private theorem xi1115_order_four_unique_fixed_point
    {G X : Type*} [Group G] [MulAction G X]
    (hInvolutionUnique : ∀ t : G, orderOf t = 2 →
      ∃! c : X, t • c = c)
    (x : G) (hx : orderOf x = 4) :
    ∃! c : X, x • c = c := by
  have hx4 : x ^ 4 = 1 := by
  ...
  have hx2sq : (x ^ 2) ^ 2 = 1 := by
    rw [← pow_mul]
    simpa using hx4
  have hx2order : orderOf (x ^ 2) = 2 :=
    orderOf_eq_prime hx2sq hx2ne
  obtain ⟨c, hc, hcUnique⟩ := hInvolutionUnique (x ^ 2) hx2order
  have hxc : x • c = c := by
    apply hcUnique (x • c)
    calc
      (x ^ 2) • (x • c) = ((x ^ 2) * x) • c := by rw [mul_smul]
      _ = (x * (x ^ 2)) • c := by congr 1; group
      _ = x • ((x ^ 2) • c) := by rw [mul_smul]
      _ = x • c := by rw [hc]
  refine ⟨c, hxc, ?_⟩
  intro y hy
  apply hcUnique y
  rw [pow_two, mul_smul, hy, hy]

/-- A conjugator between elements with the same unique fixed point belongs to
the stabilizer of that point. -/
private theorem xi1115_conjugator_mem_stabilizer_of_unique_fixed_point
    {G X : Type*} [Group G] [MulAction G X]
    (x y : G) (c : X)
    (hxc : x • c = c)
    (hyUnique : ∀ z : X, y • z = z → z = c)
    (hconj : IsConj x y) :
  ...
  have hgc : g • c = c := by
    apply hyUnique (g • c)
    calc
      y • (g • c) = (g * x * g⁻¹) • (g • c) := by rw [hg]
      _ = g • (x • c) := by simp only [mul_smul, inv_smul_smul]
      _ = g • c := by rw [hxc]
  exact ⟨⟨g, hgc⟩, hg⟩

/-- Two explicit nonconjugate order-four elements give the local lower bound
used in the ambient-to-stabilizer transfer. -/
private theorem xi1115_two_le_order_four_class_range_of_not_isConj
    {H : Type*} [Group H] [Finite H]
    (F : Subgroup H) (x y : F)
    (hx : orderOf x = 4) (hy : orderOf y = 4)
    (hxy : ¬ IsConj (x : H) (y : H)) :
    2 ≤ Nat.card
      (Set.range fun z : {z : F // orderOf (z : F) = 4} =>
        ConjClasses.mk ((z : F) : H)) := by
  classical
  let S := {z : F // orderOf (z : F) = 4}
  let cls : S → ConjClasses H := fun z =>
    ConjClasses.mk ((z.1 : F) : H)
  let Local := Set.range cls
  let code : Bool → Local
    | false => ⟨ConjClasses.mk (x : H), ⟨⟨x, hx⟩, rfl⟩⟩
    | true => ⟨ConjClasses.mk (y : H), ⟨⟨y, hy⟩, rfl⟩⟩
  have hcode : Function.Injective code := by
    ...
    · exfalso
      apply hxy
      exact ConjClasses.mk_eq_mk_iff_isConj.mp (congrArg Subtype.val hbd)
    · exfalso
      apply hxy
      exact (ConjClasses.mk_eq_mk_iff_isConj.mp
        (congrArg Subtype.val hbd)).symm
    · rfl
  have hcard : Nat.card Bool ≤ Nat.card Local :=
    Nat.card_le_card_of_injective code hcode
  simpa [Local, cls, S] using hcard

/-- The first sentence of XI.11.9: two ambient order-four classes restrict to
two order-four classes in the point stabilizer. -/
private theorem xi1115_two_order_four_classes_of_ambient
    {G X : Type*} [Group G] [Finite G] [MulAction G X]
    [Fintype (ConjClasses G)] [DecidableEq (ConjClasses G)]
    (c : X) (F : Subgroup (MulAction.stabilizer G c))
    (hInvolutionUnique : ∀ t : G, orderOf t = 2 →
      ∃! z : X, t • z = z)
    (hLocalLower : 2 ≤ Nat.card
      (Set.range fun x : {x : F // orderOf (x : F) = 4} =>
        ConjClasses.mk ((x : F) : MulAction.stabilizer G c)))
    (hAmbient : (xi1115_orderFourClasses G).card = 2) :
    XI1115TwoOrderFourClasses F := by
  classical
  let H := MulAction.stabilizer G c
  let S := {x : F // orderOf (x : F) = 4}
  let clsH : S → ConjClasses H := fun x =>
    ConjClasses.mk ((x.1 : F) : H)
  let Local := Set.range clsH
  let Ambient := {C : ConjClasses G // C ∈ xi1115_orderFourClasses G}
  let rep : Local → S := fun C => Classical.choose C.2
  have hrepClass (C : Local) : clsH (rep C) = C.1 :=
    Classical.choose_spec C.2
  let toAmbient : Local → Ambient := fun C =>
    ⟨ConjClasses.mk ((((rep C).1 : F) : H) : G), by
      simp only [xi1115_orderFourClasses, Finset.mem_filter,
        Finset.mem_univ, true_and]
      exact ⟨((((rep C).1 : F) : H) : G), by
        simpa using (rep C).2, rfl⟩⟩
  have htoAmbientInjective : Function.Injective toAmbient := by
    intro C D hCD
    let xH : H := (((rep C).1 : F) : H)
    let yH : H := (((rep D).1 : F) : H)
    have hxOrder : orderOf (xH : G) = 4 := by
      simpa [xH] using (rep C).2
    have hyOrder : orderOf (yH : G) = 4 := by
      simpa [yH] using (rep D).2
    have hxFix : (xH : G) • c = c := xH.property
    obtain ⟨_yc, _hyc, hyUnique⟩ :=
      xi1115_order_four_unique_fixed_point hInvolutionUnique (yH : G) hyOrder
    have hyFix : (yH : G) • c = c := yH.property
    have hyUniqueAtC : ∀ z : X, (yH : G) • z = z → z = c := by
      intro z hz
      exact (hyUnique z hz).trans (hyUnique c hyFix).symm
    have hconjG : IsConj (xH : G) (yH : G) :=
      ConjClasses.mk_eq_mk_iff_isConj.mp (congrArg Subtype.val hCD)
    obtain ⟨g, hg⟩ :=
      xi1115_conjugator_mem_stabilizer_of_unique_fixed_point
        (xH : G) (yH : G) c hxFix hyUniqueAtC hconjG
    have hconjH : IsConj xH yH := by
      rw [isConj_iff]
      exact ⟨g, Subtype.ext hg⟩
    apply Subtype.ext
    calc
      C.1 = clsH (rep C) := (hrepClass C).symm
      _ = clsH (rep D) :=
        ConjClasses.mk_eq_mk_iff_isConj.mpr hconjH
      _ = D.1 := hrepClass D
  have hLocalLe : Nat.card Local ≤ Nat.card Ambient :=
    Nat.card_le_card_of_injective toAmbient htoAmbientInjective
  have hAmbientCard : Nat.card Ambient = 2 := by
    simpa [Ambient, Nat.card_eq_fintype_card] using hAmbient
  have hLocalCard : Nat.card Local = 2 := by
    have hLower : 2 ≤ Nat.card Local := by
      simpa [Local, clsH, S, H] using hLocalLower
    omega
  simpa [XI1115TwoOrderFourClasses, Local, clsH, S, H] using hLocalCard

/-- Source-faithful XI.5.8/XI.11.7 wrapper from a complete character family,
the XI.11.6 partition, and two explicit local representatives. -/
private theorem xi1115_two_order_four_classes_of_completeFamily_partition
    {G X I : Type*} [Group G] [Finite G] [MulAction G X] [Fintype I]
    [Fintype (ConjClasses G)] [DecidableEq (ConjClasses G)]
    (chi : I → Representation.ClassFunction G)
    (hchi : Representation.IsCompleteIrreducibleCharacterFamily chi)
    (q : ℕ) (hcharCard : Fintype.card I = q + 3)
    ...
    (hInvolutionUnique : ∀ t : G, orderOf t = 2 →
      ∃! z : X, t • z = z)
    (x y : F) (hx : orderOf x = 4) (hy : orderOf y = 4)
    (hxy : ¬ IsConj
      ((x : F) : MulAction.stabilizer G c)
      ((y : F) : MulAction.stabilizer G c)) :
    XI1115TwoOrderFourClasses F := by
  apply xi1115_two_order_four_classes_of_ambient c F hInvolutionUnique
  · exact xi1115_two_le_order_four_class_range_of_not_isConj
      F x y hx hy hxy
  · exact xi1115_orderFourClasses_card_eq_two_of_completeFamily_partition
      chi hchi q hcharCard base hbaseCard hdisjoint hcover

set_option maxHeartbeats 800000 in
private theorem xi1115_theta_fixed_points_of_typeA_centralizer_card
    {P : Type*} [Group P] [Finite P] {n : ℕ}
    (theta : PFAppendixIII.BinaryGaloisField n ≃+*
      PFAppendixIII.BinaryGaloisField n)
    (pairJ : PFAppendixIII.BinaryGaloisField n →
      PFAppendixIII.BinaryGaloisField n → P)
    (hsurj : ∀ x : P, ∃ a z, x = pairJ a z)
    (hinj : ∀ a z b w, pairJ a z = pairJ b w → a = b ∧ z = w)
    (hmul : ∀ a z b w,
      pairJ a z * pairJ b w =
        pairJ (a + b) (z + w + a * theta b))
    (hcentralizerCard :
      Nat.card (Subgroup.centralizer ({pairJ 1 0} : Set P)) =
        2 * Nat.card (PFAppendixIII.BinaryGaloisField n)) :
    ∀ a : PFAppendixIII.BinaryGaloisField n,
      theta a = a → a = 0 ∨ a = 1 := by
  classical
  let K := PFAppendixIII.BinaryGaloisField n
  let FixedTheta := {a : K // theta a = a}
  let y : P := pairJ 1 0
  let toCentralizer : FixedTheta × K →
      Subgroup.centralizer ({y} : Set P) := fun az =>
    ⟨pairJ az.1 az.2, by
      rw [Subgroup.mem_centralizer_singleton_iff]
      dsimp [y]
      rw [hmul, hmul]
      apply congrArg₂ pairJ
      · abel
      · rw [map_one, az.1.property]
        ring⟩
  have hToCentralizerInjective : Function.Injective toCentralizer := by
    intro az bw hab
    have hval := congrArg
      (fun x : Subgroup.centralizer ({y} : Set P) => (x : P)) hab
    have hcoord := hinj az.1 az.2 bw.1 bw.2 hval
    exact Prod.ext (Subtype.ext hcoord.1) hcoord.2
  have hToCentralizerSurjective : Function.Surjective toCentralizer := by
    intro x
    obtain ⟨a, z, hx⟩ := hsurj (x : P)
    have hcomm := Subgroup.mem_centralizer_singleton_iff.mp x.property
    have hcommCoord :
        pairJ a z * pairJ 1 0 = pairJ 1 0 * pairJ a z := by
      simpa [y, hx] using hcomm
    rw [hmul, hmul] at hcommCoord
    have hsecond := (hinj _ _ _ _ hcommCoord).2
    have hfix : theta a = a := by
      have h : z + a = z + theta a := by
        simpa using hsecond
      exact (add_left_cancel h).symm
    refine ⟨(⟨a, hfix⟩, z), ?_⟩
    apply Subtype.ext
    exact hx.symm
  let eCentralizer : FixedTheta × K ≃
      Subgroup.centralizer ({y} : Set P) :=
    Equiv.ofBijective toCentralizer
      ⟨hToCentralizerInjective, hToCentralizerSurjective⟩
  have hFixedCard : Nat.card FixedTheta = 2 := by
    have hcard := Nat.card_congr eCentralizer
    change Nat.card (FixedTheta × K) =
      Nat.card (Subgroup.centralizer ({pairJ 1 0} : Set P)) at hcard
    rw [Nat.card_prod, hcentralizerCard] at hcard
    apply Nat.eq_of_mul_eq_mul_right (Nat.card_pos (α := K))
    exact hcard
  letI : Fintype FixedTheta := Fintype.ofFinite FixedTheta
  let zeroFixed : FixedTheta := ⟨0, map_zero theta⟩
  let oneFixed : FixedTheta := ⟨1, map_one theta⟩
  have hzeroNeOne : zeroFixed ≠ oneFixed := by
    intro h
    exact zero_ne_one (congrArg Subtype.val h)
  let fixedPair : Finset FixedTheta := {zeroFixed, oneFixed}
  have hfixedPairCard : fixedPair.card = 2 := by
    simp [fixedPair, hzeroNeOne]
  have hfixedPairUniv : fixedPair = Finset.univ := by
    apply Finset.eq_univ_of_card
    rw [hfixedPairCard]
    simpa [Nat.card_eq_fintype_card] using hFixedCard.symm
  intro a ha
  let aFixed : FixedTheta := ⟨a, ha⟩
  have haMem : aFixed ∈ fixedPair := by
    rw [hfixedPairUniv]
    exact Finset.mem_univ _
  simp only [fixedPair, Finset.mem_insert, Finset.mem_singleton] at haMem
  rcases haMem with ha0 | ha1
  · exact Or.inl (congrArg Subtype.val ha0)
  · exact Or.inr (congrArg Subtype.val ha1)
...
private theorem xi1115_theta_gf8_fixed_points_and_relation
    (theta : PFAppendixIII.BinaryGaloisField 3 ≃+*
      PFAppendixIII.BinaryGaloisField 3)
    (hnonfixed : ∃ x : PFAppendixIII.BinaryGaloisField 3,
      theta x ≠ x) :
    (∀ x : PFAppendixIII.BinaryGaloisField 3,
      theta x = x → x = 0 ∨ x = 1) ∧
    ((∀ x : PFAppendixIII.BinaryGaloisField 3,
        theta (theta x) = x ^ 2) ∨
      ∀ x : PFAppendixIII.BinaryGaloisField 3,
        theta.symm (theta.symm x) = x ^ 2) := by
  let K := PFAppendixIII.BinaryGaloisField 3
  ...
  have hAutCard : Nat.card (K ≃ₐ[ZMod 2] K) = 3 := by
    calc
      ...
      _ = 3 := by
        simpa [K, PFAppendixIII.BinaryGaloisField] using
          GaloisField.finrank 2 (by norm_num : (3 : ℕ) ≠ 0)
  have hthetaNe : thetaAlg ≠ 1 := by
    intro htheta
    obtain ⟨x, hx⟩ := hnonfixed
    apply hx
    have h := DFunLike.congr_fun htheta x
    simpa [thetaAlg] using h
  have hsigmaOrder : orderOf sigma = 3 := by
    rw [FiniteField.orderOf_frobeniusAlgEquivOfAlgebraic]
    simpa [K, PFAppendixIII.BinaryGaloisField] using
      GaloisField.finrank 2 (by norm_num : (3 : ℕ) ≠ 0)
  have hsigmaNe : sigma ≠ 1 := by
    intro hsigma
    rw [hsigma, orderOf_one] at hsigmaOrder
    omega
  letI : Fact (Nat.Prime 3) := ⟨Nat.prime_three⟩
  have hthetaOrder : orderOf thetaAlg = 3 := by
    have htop : Subgroup.zpowers thetaAlg = ⊤ :=
      zpowers_eq_top_of_prime_card hAutCard hthetaNe
    calc
      orderOf thetaAlg = Nat.card (Subgroup.zpowers thetaAlg) :=
        Nat.card_zpowers thetaAlg |>.symm
      _ = Nat.card (⊤ : Subgroup (K ≃ₐ[ZMod 2] K)) := by rw [htop]
      _ = Nat.card (K ≃ₐ[ZMod 2] K) := Subgroup.card_top
      _ = 3 := hAutCard
  have hsigmaMem : sigma ∈ Submonoid.powers thetaAlg :=
    mem_powers_of_prime_card hAutCard hthetaNe
  obtain ⟨k, hk⟩ :=
    (Submonoid.mem_powers_iff sigma thetaAlg).mp hsigmaMem
  have hkmod : thetaAlg ^ (k % 3) = sigma := by
    have h := (pow_mod_orderOf thetaAlg k).trans hk
    simpa only [hthetaOrder] using h
  have hrem : k % 3 < 3 := Nat.mod_lt k (by norm_num)
  have hthetaOr : thetaAlg = sigma ∨ thetaAlg ^ 2 = sigma := by
    interval_cases h : k % 3
    · simp at hkmod
      exact False.elim (hsigmaNe hkmod.symm)
    · left
      simpa using hkmod
    · right
      simpa using hkmod
  constructor
  · intro x hx
    have hxFrob : sigma x = x := by
      rcases hthetaOr with htheta | htheta
      · rw [← htheta]
        exact hx
      · rw [← htheta]
        change theta (theta x) = x
        rw [hx, hx]
    have hsq : x ^ 2 = x := by
      simpa [sigma, FiniteField.coe_frobeniusAlgEquivOfAlgebraic] using hxFrob
    exact eq_zero_or_one_of_sq_eq_self hsq
  · rcases hthetaOr with htheta | htheta
    · right
      have hsigmaCube : sigma ^ 3 = 1 := by
        have h := pow_orderOf_eq_one sigma
        simpa only [hsigmaOrder] using h
      have hsigmaInv : sigma⁻¹ = sigma ^ 2 := by
        apply mul_left_cancel
        calc
          sigma * sigma⁻¹ = 1 := mul_inv_cancel sigma
          _ = sigma ^ 3 := hsigmaCube.symm
          _ = sigma * sigma ^ 2 := by rw [pow_succ']
      have hInvSq : thetaAlg⁻¹ ^ 2 = sigma := by
        rw [htheta, hsigmaInv]
        calc
          (sigma ^ 2) ^ 2 = sigma ^ (2 * 2) := (pow_mul sigma 2 2).symm
          _ = sigma ^ (3 + 1) := by norm_num
          _ = sigma ^ 3 * sigma ^ 1 := pow_add sigma 3 1
          _ = sigma := by rw [hsigmaCube]; simp
      intro x
      have h := DFunLike.congr_fun hInvSq x
      change theta.symm (theta.symm x) = sigma x at h
      exact h.trans (show sigma x = x ^ 2 by rfl)
    · left
      intro x
      have h := DFunLike.congr_fun htheta x
      change theta (theta x) = sigma x at h
      exact h.trans (show sigma x = x ^ 2 by rfl)
...
private structure XI1115ThetaEquationData
    {K : Type*} [Field K]
    (theta : K ≃+* K) (a : K) where
  xi : K
  mu : K
...
private theorem xi1115_root_embedding_of_typeA_coordinates
    {P : Type*} [Group P]
    (m : ℕ) (hm : 0 < m)
    (theta : PFAppendixIII.BinaryGaloisField (2 * m + 1) ≃+*
      PFAppendixIII.BinaryGaloisField (2 * m + 1))
    (pair : PFAppendixIII.BinaryGaloisField (2 * m + 1) →
      PFAppendixIII.BinaryGaloisField (2 * m + 1) → P)
    (hone : pair 0 0 = 1)
    ...
    (htheta :
      (∀ x, theta (theta x) = x ^ 2) ∨
      (∀ x, theta.symm (theta.symm x) = x ^ 2)) :
    ∃ phi : P →* SuzukiMatrixGroup m, Function.Injective phi := by
  rcases htheta with htheta | htheta
  · rcases xi1115_root_embedding_of_standard_coordinates
      m hm theta htheta pair hone hsurj hinj hmul with ⟨phi, hphi, _hspec⟩
    exact ⟨phi, hphi⟩
  · let pi := theta.symm
    let pair' : PFAppendixIII.BinaryGaloisField (2 * m + 1) →
        PFAppendixIII.BinaryGaloisField (2 * m + 1) → P :=
      fun a z => pair (pi a) (z + a * pi a)
    have hone' : pair' 0 0 = 1 := by
      simpa [pair', pi] using hone
    have hsurj' : ∀ x : P, ∃ a z, x = pair' a z := by
      intro x
      obtain ⟨c, t, hx⟩ := hsurj x
      refine ⟨theta c, t + theta c * c, ?_⟩
      rw [hx]
      ...
      calc
        ...
        _ = t := by rw [CharTwo.add_self_eq_zero, add_zero]
    have hinj' : ∀ a z b w,
        pair' a z = pair' b w → a = b ∧ z = w := by
      intro a z b w hab
      have h := hinj (pi a) (z + a * pi a)
        (pi b) (w + b * pi b) hab
      have hab' : a = b := pi.injective h.1
      subst b
      exact ⟨rfl, add_right_cancel h.2⟩
    have hmul' : ∀ a z b w,
        pair' a z * pair' b w =
          pair' (a + b) (z + w + a * pi b) := by
      intro a z b w
      dsimp only [pair']
      rw [hmul, theta.apply_symm_apply]
      congr 1
      · exact (map_add pi a b).symm
      · rw [map_add]
        ring_nf
        simp only [CharTwo.two_eq_zero, mul_zero, add_zero]
    rcases xi1115_root_embedding_of_standard_coordinates
      m hm pi htheta pair' hone' hsurj' hinj' hmul' with ⟨phi, hphi, _hspec⟩
    exact ⟨phi, hphi⟩
...
private theorem xi1115_standard_bruhat_of_coordinate_embeddings
    {D P : Type*} [Group D] [Group P]
    (m : ℕ) (hm : 0 < m)
    (pi : PFAppendixIII.BinaryGaloisField (2 * m + 1) ≃+*
      PFAppendixIII.BinaryGaloisField (2 * m + 1))
  ...
  rcases xi1115_standard_bruhat_of_coordinate_embeddings_with_weyl_spec
    ...
    ⟨wstd, _hwstd, hwstdSq, hBruhat⟩
  exact ⟨wstd, hwstdSq, hBruhat⟩
...
private theorem xi1115_stronglyReal_centralizer_TI
    {G Omega : Type*} [Group G] [Finite G] [MulAction G Omega]
    [Fintype Omega] [FaithfulSMul G Omega]
    (htwo : MulAction.IsMultiplyPretransitive G Omega 2)
    (hdegreeOdd : Odd (Fintype.card Omega))
...
set_option maxHeartbeats 800000 in
private theorem xi1115_alpha_beta_coordinate
    {G : Type*} [Group G]
    (H : Subgroup G) (F D : Subgroup H)
    [MulDistribMulAction D F] [IsMulCommutative D]
    (s : G) (hss : s * s = 1)
    ...
    (hAlphaNe :
      ∀ x : {x : F // x ≠ 1}, (coord x).2.2 ≠ 1)
    (hBetaNe :
      ∀ x : {x : F // x ≠ 1}, (coord x).1 ≠ 1)
    (x : {x : F // x ≠ 1}) :
    let bx : {x : F // x ≠ 1} := ⟨(coord x).1, hBetaNe x⟩
    (coord bx).2.2 =
      (coord x).2.1⁻¹ • (coord x).2.2⁻¹ := by
  let alphaX : {x : F // x ≠ 1} → {x : F // x ≠ 1} :=
    fun y => ⟨(coord y).2.2, hAlphaNe y⟩
  let betaX : {x : F // x ≠ 1} → {x : F // x ≠ 1} :=
    fun y => ⟨(coord y).1, hBetaNe y⟩
  let invX : {x : F // x ≠ 1} → {x : F // x ≠ 1} :=
    fun y => ⟨y.1⁻¹, inv_ne_one.mpr y.2⟩
  let actX : D → {x : F // x ≠ 1} → {x : F // x ≠ 1} :=
    fun d y => ⟨d • y.1, xi1115_smul_ne_one d y.2⟩
  have hAlphaCoords (y : {x : F // x ≠ 1}) :
      (coord (alphaX y)).1 = (coord y).2.1⁻¹ • (coord y).1⁻¹ ∧
        (coord (alphaX y)).2.1 = (coord y).2.1⁻¹ ∧
          (coord (alphaX y)).2.2 = y.1 := by
    simpa [alphaX] using
      (xi1115_alpha_bruhat_coordinates H F D s hss hAction coord
        hcoord hunique hAlphaNe y)
  have hInvCoords (y : {x : F // x ≠ 1}) :
      (coord (invX y)).1 = (coord y).2.2⁻¹ ∧
        (coord (invX y)).2.1 = (coord y).2.1 ∧
          (coord (invX y)).2.2 = (coord y).1⁻¹ := by
    simpa [invX] using
      (xi1115_inverse_bruhat_coordinates H F D s hss hInverts coord
        hcoord hunique y)
  have hDCoords (d : D) (y : {x : F // x ≠ 1}) :
      (coord (actX d y)).1 = d⁻¹ • (coord y).1 ∧
        (coord (actX d y)).2.1 = d⁻¹ * d⁻¹ * (coord y).2.1 ∧
          (coord (actX d y)).2.2 = d⁻¹ • (coord y).2.2 := by
    simpa [actX] using
      (xi1115_conjugate_bruhat_coordinates H F D s hss hInverts
        hAction coord hcoord hunique d y)
  have hAlphaInvolutive (y : {x : F // x ≠ 1}) :
      alphaX (alphaX y) = y := by
    apply Subtype.ext
    exact (hAlphaCoords y).2.2
  have hBetaAlpha (y : {x : F // x ≠ 1}) :
      betaX (alphaX y) =
        alphaX (actX (coord y).2.1 (invX y)) := by
    apply Subtype.ext
    change (coord (alphaX y)).1 =
      (coord (actX (coord y).2.1 (invX y))).2.2
    calc
      (coord (alphaX y)).1 =
          (coord y).2.1⁻¹ • (coord y).1⁻¹ := (hAlphaCoords y).1
      _ = (coord y).2.1⁻¹ • (coord (invX y)).2.2 := by
        rw [(hInvCoords y).2.2]
      _ = (coord (actX (coord y).2.1 (invX y))).2.2 :=
        (hDCoords (coord y).2.1 (invX y)).2.2.symm
  have hCore (y : {x : F // x ≠ 1}) :
      alphaX (betaX (alphaX y)) =
        actX (coord y).2.1 (invX y) := by
    calc
      alphaX (betaX (alphaX y)) =
          alphaX (alphaX (actX (coord y).2.1 (invX y))) :=
            congrArg alphaX (hBetaAlpha y)
      _ = actX (coord y).2.1 (invX y) :=
        hAlphaInvolutive (actX (coord y).2.1 (invX y))
  have hcore := hCore (alphaX x)
  rw [hAlphaInvolutive x] at hcore
  have hgamma :
      (coord (alphaX x)).2.1 = (coord x).2.1⁻¹ :=
    (hAlphaCoords x).2.1
  rw [hgamma] at hcore
...
private theorem xi1115_bruhatCoords_eq_of_alpha_eq
    {F D : Type*} [Group F] [Group D] [MulDistribMulAction D F]
    (coord₁ coord₂ : {x : F // x ≠ 1} → F × D × F)
    (hAlphaNe₁ : ∀ x, (coord₁ x).2.2 ≠ 1)
    (hBetaNe₁ : ∀ x, (coord₁ x).1 ≠ 1)
    (hBetaNe₂ : ∀ x, (coord₂ x).1 ≠ 1)
    (hInv₁ : ∀ x : {x : F // x ≠ 1},
      (coord₁ ⟨x.1⁻¹, inv_ne_one.mpr x.2⟩).2.2 = (coord₁ x).1⁻¹)
    (hInv₂ : ∀ x : {x : F // x ≠ 1},
      (coord₂ ⟨x.1⁻¹, inv_ne_one.mpr x.2⟩).2.2 = (coord₂ x).1⁻¹)
    (hAlphaBeta₁ : ∀ x : {x : F // x ≠ 1},
      (coord₁ ⟨(coord₁ x).1, hBetaNe₁ x⟩).2.2 =
        (coord₁ x).2.1⁻¹ • (coord₁ x).2.2⁻¹)
    (hAlphaBeta₂ : ∀ x : {x : F // x ≠ 1},
      (coord₂ ⟨(coord₂ x).1, hBetaNe₂ x⟩).2.2 =
        (coord₂ x).2.1⁻¹ • (coord₂ x).2.2⁻¹)
    (hActionFree : ∀ (d e : D) (y : F), y ≠ 1 → d • y = e • y → d = e)
    (hAlpha : ∀ x, (coord₁ x).2.2 = (coord₂ x).2.2) :
    ∀ x, coord₁ x = coord₂ x := by
  intro x
  let xInv : {x : F // x ≠ 1} := ⟨x.1⁻¹, inv_ne_one.mpr x.2⟩
  have hbetaInv : (coord₁ x).1⁻¹ = (coord₂ x).1⁻¹ := by
    calc
      (coord₁ x).1⁻¹ = (coord₁ xInv).2.2 := (hInv₁ x).symm
      _ = (coord₂ xInv).2.2 := hAlpha xInv
      _ = (coord₂ x).1⁻¹ := hInv₂ x
  have hbeta : (coord₁ x).1 = (coord₂ x).1 := by
    exact inv_injective hbetaInv
  let bx₁ : {x : F // x ≠ 1} := ⟨(coord₁ x).1, hBetaNe₁ x⟩
  let bx₂ : {x : F // x ≠ 1} := ⟨(coord₂ x).1, hBetaNe₂ x⟩
  have hbx : bx₁ = bx₂ := Subtype.ext hbeta
  have hactionEq :
      (coord₁ x).2.1⁻¹ • (coord₁ x).2.2⁻¹ =
        (coord₂ x).2.1⁻¹ • (coord₁ x).2.2⁻¹ := by
    calc
      (coord₁ x).2.1⁻¹ • (coord₁ x).2.2⁻¹ =
          (coord₁ bx₁).2.2 := (hAlphaBeta₁ x).symm
      _ = (coord₂ bx₁).2.2 := hAlpha bx₁
      _ = (coord₂ bx₂).2.2 := by rw [hbx]
      _ = (coord₂ x).2.1⁻¹ • (coord₂ x).2.2⁻¹ := hAlphaBeta₂ x
      _ = (coord₂ x).2.1⁻¹ • (coord₁ x).2.2⁻¹ := by rw [hAlpha x]
  have hyNe : (coord₁ x).2.2⁻¹ ≠ 1 :=
    inv_ne_one.mpr (hAlphaNe₁ x)
  have hgammaInv : (coord₁ x).2.1⁻¹ = (coord₂ x).2.1⁻¹ :=
    hActionFree _ _ _ hyNe hactionEq
  have hgamma : (coord₁ x).2.1 = (coord₂ x).2.1 :=
    inv_injective hgammaInv
  exact Prod.ext hbeta (Prod.ext hgamma (hAlpha x))
...
/-- The right root coordinate in the explicit Suzuki Gauss decomposition.
This is the XI.11.13 formula that the source alpha coordinate must match. -/
private def xi1115_standard_alpha_coordinate
    {K F : Type*} [Field K] [Group F]
    (pi : K ≃+* K) (pair : K → K → F) (a z : K) : F :=
  let n : K := a * z + pi a * a ^ 2 + pi z
  pair (n⁻¹ * z) (n⁻¹ * a)

private theorem xi1115_alpha_eq_of_standard_pair_formulas
    {K F D : Type*} [Field K] [Group F] [Group D]
    (pi : K ≃+* K) (pair : K → K → F)
    (coord₁ coord₂ : {x : F // x ≠ 1} → F × D × F)
    (hsurj : ∀ x : F, ∃ a z, x = pair a z)
    (hAlpha₁ : ∀ (a z : K) (hx : pair a z ≠ 1),
      (coord₁ ⟨pair a z, hx⟩).2.2 =
        xi1115_standard_alpha_coordinate pi pair a z)
    (hAlpha₂ : ∀ (a z : K) (hx : pair a z ≠ 1),
      (coord₂ ⟨pair a z, hx⟩).2.2 =
        xi1115_standard_alpha_coordinate pi pair a z) :
    ∀ x, (coord₁ x).2.2 = (coord₂ x).2.2 := by
  intro x
  obtain ⟨a, z, hx⟩ := hsurj x.1
  have hne : pair a z ≠ 1 := by
    simpa [← hx] using x.2
  have hxEq : x = ⟨pair a z, hne⟩ := Subtype.ext hx
  rw [hxEq]
  exact (hAlpha₁ a z hne).trans (hAlpha₂ a z hne).symm

/-- Once XI.11.13 identifies the source and standard alpha coordinates, the
generic coordinate identities and the standard Suzuki swap formula imply the
exact root-swap compatibility needed by the ambient Bruhat transport. -/
private theorem xi1115_aligned_of_standard_alpha
    {K F D T : Type*} [Field K] [Group F] [Group D] [Group T]
    [MulDistribMulAction D F]
    (pi : K ≃+* K) (pair : K → K → F)
    (coordSource coordStandard : {x : F // x ≠ 1} → F × D × F)
    (phiF : F →* T) (phiD : D →* T) (w : T)
    (hsurj : ∀ x : F, ∃ a z, x = pair a z)
    (hSourceAlpha : ∀ (a z : K) (hx : pair a z ≠ 1),
      (coordSource ⟨pair a z, hx⟩).2.2 =
        xi1115_standard_alpha_coordinate pi pair a z)
    (hStandardAlpha : ∀ (a z : K) (hx : pair a z ≠ 1),
      (coordStandard ⟨pair a z, hx⟩).2.2 =
        xi1115_standard_alpha_coordinate pi pair a z)
    (hAlphaNeSource : ∀ x, (coordSource x).2.2 ≠ 1)
    (hBetaNeSource : ∀ x, (coordSource x).1 ≠ 1)
    (hBetaNeStandard : ∀ x, (coordStandard x).1 ≠ 1)
    (hInvSource : ∀ x : {x : F // x ≠ 1},
      (coordSource ⟨x.1⁻¹, inv_ne_one.mpr x.2⟩).2.2 =
        (coordSource x).1⁻¹)
    (hInvStandard : ∀ x : {x : F // x ≠ 1},
      (coordStandard ⟨x.1⁻¹, inv_ne_one.mpr x.2⟩).2.2 =
        (coordStandard x).1⁻¹)
    (hAlphaBetaSource : ∀ x : {x : F // x ≠ 1},
      (coordSource ⟨(coordSource x).1, hBetaNeSource x⟩).2.2 =
        (coordSource x).2.1⁻¹ • (coordSource x).2.2⁻¹)
    (hAlphaBetaStandard : ∀ x : {x : F // x ≠ 1},
      (coordStandard ⟨(coordStandard x).1, hBetaNeStandard x⟩).2.2 =
        (coordStandard x).2.1⁻¹ • (coordStandard x).2.2⁻¹)
    (hActionFree : ∀ (d e : D) (y : F), y ≠ 1 → d • y = e • y → d = e)
    (hStandardSwap : ∀ x : {x : F // x ≠ 1},
      phiF (coordStandard x).1 * phiD (coordStandard x).2.1 * w *
          phiF (coordStandard x).2.2 =
        w * phiF x.1 * w) :
    ∀ x : {x : F // x ≠ 1},
      phiF (coordSource x).1 * phiD (coordSource x).2.1 * w *
          phiF (coordSource x).2.2 =
        w * phiF x.1 * w := by
  have hAlpha : ∀ x,
      (coordSource x).2.2 = (coordStandard x).2.2 :=
    xi1115_alpha_eq_of_standard_pair_formulas pi pair coordSource
      coordStandard hsurj hSourceAlpha hStandardAlpha
  have hcoords : ∀ x, coordSource x = coordStandard x :=
    xi1115_bruhatCoords_eq_of_alpha_eq coordSource coordStandard
      hAlphaNeSource hBetaNeSource hBetaNeStandard hInvSource hInvStandard
      hAlphaBetaSource hAlphaBetaStandard hActionFree hAlpha
  intro x
  rw [hcoords x]
  exact hStandardSwap x

/-- In the direct matrix convention the structure element has root coordinate
`(1,1)`, whose inverse has coordinate `(1,0)`. -/
private theorem xi1115_direct_structure_alignment
...
matrix convention: old `(1,0)` becomes new `(1,1)`. -/
private theorem xi1115_inverse_structure_alignment
    {K F : Type*} [Field K] [CharP K 2] [Group F]
    (theta : K ≃+* K) (pair pair' : K → K → F)
    (hone : pair 0 0 = 1)
    (hmul : ∀ a z b w,
      pair a z * pair b w = pair (a + b) (z + w + a * theta b))
    (hpair' : ∀ a z,
      pair' a z = pair (theta.symm a) (z + a * theta.symm a))
    (j g : F) (hj : j = pair 0 1) (hg : g = pair 1 0) :
    j = pair' 0 1 ∧ g = pair' 1 1 ∧ g⁻¹ = pair' 1 0 := by
  ...
  · calc
      j = pair 0 1 := hj
      _ = pair' 0 1 := by simp [hpair']
  constructor
  · calc
      g = pair 1 0 := hg
      _ = pair' 1 1 := by simp [hpair', CharTwo.add_self_eq_zero]
  · calc
      g⁻¹ = (pair 1 0)⁻¹ := congrArg Inv.inv hg
      _ = pair 1 1 := hpairInv.symm
      _ = pair' 1 0 := by simp [hpair']
...
set_option maxHeartbeats 800000 in
/-- XI.11.13's standard alpha formula on the commutator representatives
produced by the XI.11.11 structure equation. -/
private theorem xi1115_structure_representative_standard_alpha
    ...
    (g : F) (hg : g = pair 1 1)
    (h k : D) (hh : h ≠ 1)
    (hnNe : let lambda : K := (eD h : K)⁻¹
      1 + lambda ^ 2 * theta lambda ≠ 0)
    (hnorm : let lambda : K := (eD h : K)⁻¹
      let kappa : K := (eD k : K)
      kappa⁻¹ * theta kappa⁻¹ =
        lambda ^ 2 * (theta lambda) ^ 2 + lambda * theta lambda)
    (alpha : F → F)
    (halpha : alpha (g⁻¹ * (h⁻¹ • g)) = (k • g⁻¹) * (h • g)) :
    alpha (g⁻¹ * (h⁻¹ • g)) =
      let lambda : K := (eD h : K)⁻¹
      let a : K := 1 + lambda
      let z : K := lambda * theta lambda + theta lambda
      xi1115_standard_alpha_coordinate theta pair a z := by
  let lambda : K := (eD h : K)⁻¹
  let kappa : K := (eD k : K)
  have hlambda : lambda ≠ 0 := inv_ne_zero (eD h).ne_zero
  have hlambdaOne : lambda ≠ 1 := by
    intro hlambdaEq
    have heh : (eD h : K) = 1 := by
      exact inv_eq_one.mp hlambdaEq
    apply hh
    apply eD.injective
    apply Units.ext
    simpa using heh
  ...
  have hnValue :
    ...
    simp only [htwo, hthree, hfour, mul_zero, mul_one, add_zero]
  have hkappa := xi1115_kappa_eq_of_structure_norm theta hthetaSq
    hnormInjective lambda kappa hlambda hlambdaOne
      (by simpa [lambda] using hnNe) (by simpa [lambda, kappa] using hnorm)
  have heh : (eD h : K) = lambda⁻¹ := by simp [lambda]
  rw [halphaPair]
  change pair (kappa + (eD h : K))
      ((eD h : K) * theta (eD h : K) +
        kappa * theta (eD h : K)) =
    xi1115_standard_alpha_coordinate theta pair (1 + lambda)
      (lambda * theta lambda + theta lambda)
  rw [heh]
  dsimp [xi1115_standard_alpha_coordinate]
  rw [hnValue, hkappa]
  apply congrArg₂ pair
  · dsimp [lambda, kappa]
