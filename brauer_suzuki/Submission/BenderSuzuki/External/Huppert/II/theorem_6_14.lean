...
import Submission.BenderSuzuki.External.Huppert.II.theorem_6_11
...
public theorem huppert_II_6_14_a_psl2_card_two
  ...
  have hPcard : Nat.card P = 3 := by
    ...
    rw [huppert_II_6_11_projective_degree K 2]
    norm_num
    simpa [Nat.card_eq_fintype_card] using hKcard
  obtain ⟨rho, hrho, _, _⟩ :=
...
/-- Huppert II.6.14(2): `PSL(2,3)` is the alternating group on four letters. -/
public theorem huppert_II_6_14_b_psl2_card_three
    {K : Type u} [Field K] [Finite K] (hKcard : Nat.card K = 3) :
    Nonempty
      (Matrix.ProjectiveSpecialLinearGroup (Fin 2) K ≃*
  ...
  haveI : CharP K 3 :=
    charP_of_card_eq_prime (by
      simpa [Nat.card_eq_fintype_card] using hKcard)
  have hring_char_ne_two : ringChar K ≠ 2 := by
    rw [ringChar.eq K 3]
    norm_num
  have hneg_one : (-1 : K) ≠ 1 :=
    Ring.neg_one_ne_one_of_char_ne_two hring_char_ne_two
  have hcenter := huppert614_card_center_of_neg_one_ne_one hneg_one
  have hPSLcard := huppert614_card_psl_mul_center (K := K)
  rw [hcenter, hKcard] at hPSLcard
  norm_num at hPSLcard
  have hPSLcard_eq :
      Nat.card (Matrix.ProjectiveSpecialLinearGroup (Fin 2) K) = 12 := by
    rw [Nat.card_eq_fintype_card]
    omega
  let P := ℙ K (Fin 2 → K)
  let PSL := Matrix.ProjectiveSpecialLinearGroup (Fin 2) K
  have hPcard : Nat.card P = 4 := by
    change Nat.card (ℙ K (Fin 2 → K)) = 4
    rw [huppert_II_6_11_projective_degree K 2]
    norm_num
    simpa [Nat.card_eq_fintype_card] using hKcard
  obtain ⟨rho, hrho, _, _⟩ :=
    huppert_II_6_11_projective_action (K := K) 2 (by omega)
  let eP : P ≃ Fin 4 := Finite.equivFinOfCardEq hPcard
  let actFin : PSL →* Equiv.Perm (Fin 4) :=
    (Equiv.permCongrHom eP).toMonoidHom.comp rho
  have hactFin_inj : Function.Injective actFin := by
    intro x y hxy
    apply hrho
    apply (Equiv.permCongrHom eP).injective
    simpa [actFin] using hxy
  let image : Subgroup (Equiv.Perm (Fin 4)) := actFin.range
  let eRange : PSL ≃* image :=
  ...
  have himage_card : Nat.card image = 12 := by
    rw [← Nat.card_congr eRange.toEquiv]
    exact hPSLcard_eq
  have hpermcard : Nat.card (Equiv.Perm (Fin 4)) = 24 := by
    rw [Nat.card_perm, Nat.card_fin]
    norm_num [Nat.factorial]
  have himage_index : image.index = 2 := by
    have hmul := image.index_mul_card
    rw [himage_card, hpermcard] at hmul
    omega
  have himage_alt : image = alternatingGroup (Fin 4) :=
    Equiv.Perm.eq_alternatingGroup_of_index_eq_two himage_index
  exact ⟨eRange.trans (MulEquiv.subgroupCongr himage_alt)⟩
